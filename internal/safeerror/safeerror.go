...
// Error is an error that separates a user-safe message from internal details. In
// production, only SafeMessage reaches the user; in development, the full Error string
// (including wrapped causes) is shown instead.
//
// Any error type can implement Error by adding a single method. The standard error chain
// (errors.Is, errors.As, Unwrap) is unaffected.
type Error interface {
	error

	// SafeMessage returns the user-safe message suitable for HTTP responses and error pages
	// in production.
	//
...
// safeError is the concrete implementation returned by NewError and Errorf.
type safeError struct {
	// cause is the underlying error with internal details.
	cause error

	// safeMessage is the user-safe message suitable for HTTP responses and error pages in
	// production.
	safeMessage string
}
...
//
// Returns string which is the detail that gets logged but never shown to users in
// production.
...
// Unwrap returns the underlying cause, preserving the error chain for errors.Is and
// errors.As.
//
...
// Errorf wraps a formatted internal error with a user-safe message. The internal cause is
// created via fmt.Errorf, so %%w wrapping is supported.
//
// Takes safeMessage (string) which is the message safe for end users.
// Takes format (string) which is the fmt.Errorf format string for the internal cause.
...
// ExtractSafeMessage returns the appropriate error message for the given mode.
//
