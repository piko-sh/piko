...
// node represents a single vector in the HNSW graph. Each node exists at one or more
// layers, with its neighbours stored per layer.
type node[K comparable] struct {
	...
	// neighbours stores the neighbour set for each layer of this node. Index 0 is the base
	// layer, and higher indices are upper layers; each layer is an unsorted slice of node
	// pointers.
	neighbours [][]*node[K]
	...
	// id is a unique number used to track visited nodes. The graph sets this when a node is
	// added and reuses it when removed.
	id uint32
}
...
//
// Takes neighbours ([]*node[K]) which is the slice of neighbour nodes to search.
