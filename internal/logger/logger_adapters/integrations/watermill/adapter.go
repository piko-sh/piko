...
// Adapter connects Piko's logger to Watermill's LoggerAdapter interface. It allows
// Watermill components to use Piko's logging system.
type Adapter struct {
...
// Info logs an info-level message with fields. Watermill's internal logs are framework
// internals, so we map them to Internal.
//
// Takes message (string) which is the message to log.
...
// Debug logs a debug-level message with fields. Watermill's debug logs are framework
// internals, so we map them to Internal.
//
// Takes message (string) which is the message to log.
// Takes fields (watermill.LogFields) which contains structured data for the log entry.
func (a *Adapter) Debug(message string, fields watermill.LogFields) {
	a.logger.Internal(message, a.convertFields(fields)...)
}

// Trace logs a trace-level message with fields. Watermill's trace logs are deep framework
// internals, so we map them to Trace.
//
// Takes message (string) which is the message to log.
// Takes fields (watermill.LogFields) which contains structured data for the log entry.
...
// With returns a new logger with the given fields added to all subsequent log calls.
//
// Takes fields (watermill.LogFields) which specifies the key-value pairs to add to each
// log entry.
//
// Returns watermill.LoggerAdapter which is a new adapter that includes the additional
// fields.
...
//
// Takes fields (watermill.LogFields) which contains the Watermill log fields to convert.
// Takes extraFields (...logger_domain.Attr) which provides additional fields to append to
// the result.
//
...
// NewAdapter creates a new Watermill logger adapter that wraps a Piko logger. Watermill's
// LoggerAdapter interface has no context.Context, so the adapter must hold a concrete
// logger instance.
//
