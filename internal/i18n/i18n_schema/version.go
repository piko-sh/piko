...
var (
	...
	// SchemaHash is the SHA-256 hash of i18n.fbs, computed at init time. This hash changes
	// whenever the schema file is modified, so the cache invalidates automatically when the
	// schema evolves.
	SchemaHash = fbs.ComputeSchemaHash(schemaContent)
)

// Pack wraps a serialised i18n FlatBuffer with the schema version hash. The returned
