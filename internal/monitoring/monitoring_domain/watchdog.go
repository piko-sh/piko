...
const (
	// defaultCheckInterval is the period between watchdog evaluation ticks. Short intervals
	// improve detection speed at negligible CPU cost since each tick only reads cached
	// metrics and performs integer comparisons.
	defaultCheckInterval = 500 * time.Millisecond

	// defaultHeapThresholdPercent is the fraction of GOMEMLIMIT above which heap allocation
	// triggers a profile capture.
	defaultHeapThresholdPercent = 0.85

	// defaultHeapThresholdBytes is the absolute heap allocation threshold in bytes, used
	// when GOMEMLIMIT is not set.
	defaultHeapThresholdBytes = 512 * 1024 * 1024

	// defaultGoroutineThreshold is the goroutine count above which a goroutine profile
	// capture is triggered.
	defaultGoroutineThreshold = 10000

	// defaultGoroutineSafetyCeiling is the goroutine count above which profile captures are
	// suppressed to avoid destabilising the runtime.
	defaultGoroutineSafetyCeiling = 100000

	// defaultGCPressureThreshold is the GCCPUFraction above which a GC pressure warning is
	// emitted.
	defaultGCPressureThreshold = 0.50

	// defaultCooldown is the minimum duration between consecutive captures of the same
	// profile type.
	defaultCooldown = 2 * time.Minute

	// defaultMaxCapturesPerWindow is the maximum number of profile captures permitted within
	// a single sliding capture window.
	defaultMaxCapturesPerWindow = 5

	// defaultCaptureWindow is the duration of the sliding window used for global rate
	// limiting of profile captures.
	defaultCaptureWindow = 15 * time.Minute

	// defaultMaxWarningsPerWindow is the maximum number of warning-only notifications
	// permitted within a single CaptureWindow. Warnings (GC pressure, heap trend, FD
	// pressure, scheduler latency) do not consume the capture budget so a real heap leak
	// still gets profiled even when other rules are flapping.
	defaultMaxWarningsPerWindow = 10

	// defaultHighWaterResetCooldown is the minimum duration after the high-water mark was
	// last set before it may be reset to the initial threshold.
	defaultHighWaterResetCooldown = 10 * time.Minute

	// defaultWarmUpDuration is the period after startup during which the watchdog suppresses
	// all evaluations to allow the runtime to stabilise.
	defaultWarmUpDuration = 30 * time.Second

	// defaultMaxProfilesPerType is the maximum number of profile files retained per profile
	// type before the oldest are rotated out.
	defaultMaxProfilesPerType = 5

	// defaultMaxProfileSizeBytes is the maximum number of bytes a single profile capture may
	// produce before being truncated.
	defaultMaxProfileSizeBytes = 50 * 1024 * 1024

	// defaultRSSThresholdPercent is the fraction of the cgroup memory limit above which RSS
	// triggers a profile capture.
	...
	// watchdog emits a scheduler-latency warning. 10 ms is a commonly-used "user-visible
	// latency" floor; sustained higher values indicate goroutine starvation, GC
	// interference, or CPU contention.
	defaultSchedulerLatencyP99Threshold = 10 * time.Millisecond

	// defaultCrashLoopWindow is the time window over which the startup history is inspected
	// for crash-loop detection. A burst of unclean exits within this window triggers a
	// CrashLoopDetected event.
	defaultCrashLoopWindow = 60 * time.Second

	// defaultCrashLoopThreshold is the minimum number of unclean exits within
	// defaultCrashLoopWindow that signal a crash loop.
	defaultCrashLoopThreshold = 3

	// defaultContinuousProfilingInterval is the period between routine profile captures in
	// continuous-profiling mode (opt-in).
	defaultContinuousProfilingInterval = 10 * time.Minute

	// defaultContinuousProfilingRetention is the number of routine profile files retained
	// per type. Older files are rotated out.
	defaultContinuousProfilingRetention = 6

	// minContinuousProfilingInterval is the lower bound on the routine capture interval.
	// Faster than this would stress the heap profiler without adding much forensic value.
	minContinuousProfilingInterval = 1 * time.Minute

	// maxContinuousProfilingRetention is the upper bound on routine profile retention to
	...
	// defaultContentionDiagnosticWindow is the duration mutex/block profiling stays enabled
	// during a contention diagnostic before being disabled and the profiles captured.
	...
	// defaultContentionDiagnosticConsecutiveTrigger is the number of scheduler-latency
	// events within ContentionDiagnosticTriggerWindow that triggers an auto-fire diagnostic
	// when AutoFire is enabled.
	defaultContentionDiagnosticConsecutiveTrigger = 3

	// defaultContentionDiagnosticTriggerWindow is the rolling window over which
	// scheduler-latency events are counted for auto-fire.
	defaultContentionDiagnosticTriggerWindow = 15 * time.Minute

	// defaultContentionDiagnosticCooldown is the minimum interval between consecutive
	// diagnostics so the runtime is not under continuous profiling overhead.
	defaultContentionDiagnosticCooldown = 30 * time.Minute

	// minContentionDiagnosticWindow is the smallest acceptable diagnostic window. Shorter
	// than this risks empty profiles.
	minContentionDiagnosticWindow = time.Second

	// maxContentionDiagnosticWindow caps the diagnostic window to keep the extra profiling
	// overhead bounded.
	maxContentionDiagnosticWindow = 5 * time.Minute

	// contentionDiagnosticOverheadPad is the additional time budget added on top of the
	// window duration when computing the auto-fire context timeout, to account for the
	// synchronous capture step that runs after the window expires.
	contentionDiagnosticOverheadPad = 30 * time.Second

	// defaultEventRingSize bounds the number of recent watchdog events retained in memory
	// for inspector queries. Sized to comfortably cover one capture window (15 min) of event
	// chatter at the configured warning-budget rate.
	defaultEventRingSize = 256

	// eventSubscriberBuffer is the per-subscriber channel buffer for streaming watchdog
	// events. A subscriber that does not drain fast enough drops oldest pending events
	// rather than blocking the watchdog evaluation loop.
	eventSubscriberBuffer = 64

	// maxEventSubscribers caps the number of concurrent streaming subscribers attached to
	// the watchdog. Exceeding the cap returns ErrEventSubscriberCapExceeded so a runaway
	// client cannot grow the subscriber slice unbounded.
	...
	// preDeathSnapshotTimeout is the maximum time allowed for the pre-death diagnostic
	// snapshot before it is abandoned.
	preDeathSnapshotTimeout = 10 * time.Second

	// notificationTimeout is the maximum time allowed for a single watchdog notification
	// delivery.
...
var (
	// ErrInvalidWatchdogConfig is returned when the watchdog configuration contains invalid
	// values.
	ErrInvalidWatchdogConfig = errors.New("invalid watchdog configuration")

	// ErrContentionDiagnosticInProgress is returned by RunContentionDiagnostic when a
	// diagnostic is already running.
	ErrContentionDiagnosticInProgress = errors.New("contention diagnostic already in progress")

	// ErrContentionDiagnosticCooldown is returned by RunContentionDiagnostic when the
	// cooldown since the last diagnostic has not yet elapsed.
	ErrContentionDiagnosticCooldown = errors.New("contention diagnostic cooldown active")

	// ErrProfilingControllerNil is returned by RunContentionDiagnostic when no profiling
	// controller has been configured.
	ErrProfilingControllerNil = errors.New("profiling controller not available")

	// ErrWatchdogStopped is returned by RunContentionDiagnostic when the watchdog has
	// already been stopped.
	ErrWatchdogStopped = errors.New("watchdog stopped")

	// ErrEventSubscriberCapExceeded is logged by SubscribeEvents when the configured maximum
	// number of concurrent subscribers has already been reached. Existing subscribers
	// continue to receive events; the new subscription receives a pre-closed channel and a
	// no-op cancel func.
	ErrEventSubscriberCapExceeded = errors.New("watchdog event subscriber cap exceeded")
)

var (
	_ WatchdogInspector = (*Watchdog)(nil)
)

// WatchdogConfig holds the configuration for the runtime watchdog service.
type WatchdogConfig struct {
	// ProfileDirectory is the filesystem path where captured profile files are stored. Must
	// be set before the watchdog is started.
	ProfileDirectory string

	// ContinuousProfilingTypes is the list of profile types captured each interval; default
	// ["heap"], allowed values heap, goroutine, and allocs. CPU, trace, block, and mutex are
	// deliberately disallowed in routine mode because they are duration-based and would
	// block the loop.
	ContinuousProfilingTypes []string

	// FDPressureThresholdPercent is the soft FD limit fraction above which the watchdog
	// emits an FD pressure warning; default 0.80, zero disables. The watchdog only alerts
	// and does not capture a profile because there is nothing useful to capture for FD
	// exhaustion.
	FDPressureThresholdPercent float64

	// CaptureWindow is the duration of the sliding window used for global rate limiting.
	CaptureWindow time.Duration

	// HighWaterResetCooldown is the minimum duration after the heap high-water mark was last
	// set before it can be reset to the initial threshold.
	HighWaterResetCooldown time.Duration

	// WarmUpDuration is the period after startup during which all evaluations are
	// suppressed.
	WarmUpDuration time.Duration

	// HeapThresholdBytes is the absolute heap allocation threshold in bytes. Used when
	// GOMEMLIMIT is not configured.
	HeapThresholdBytes uint64

	// HeapThresholdPercent is the fraction of GOMEMLIMIT above which heap allocation
	// triggers a profile capture. Only used when GOMEMLIMIT is set.
	HeapThresholdPercent float64

	// GCPressureThreshold is the GCCPUFraction above which a GC pressure warning is emitted.
	GCPressureThreshold float64

	// RSSThresholdPercent is the fraction of the cgroup memory limit above which RSS
	// triggers a profile capture.
	//
	// Only effective when the cgroup memory limit is available. Default: 0.85.
	RSSThresholdPercent float64

	// TrendEvaluationInterval is the period between heap trend regression computations.
	// Default: 30 seconds.
	TrendEvaluationInterval time.Duration

	// TrendWarningHorizon is the projected time-to-breach below which a heap trend warning
	// is emitted. Default: 5 minutes.
	TrendWarningHorizon time.Duration

	// TrendWindowSize is the number of heap samples to retain for linear regression
	// analysis. Default: 120 (60 seconds at 500ms interval).
	TrendWindowSize int
	...
	// GoroutineSafetyCeiling is the goroutine count above which captures are suppressed to
	// avoid worsening an already unstable runtime.
	GoroutineSafetyCeiling int

	// MaxCapturesPerWindow is the maximum number of profile captures allowed within a single
	// CaptureWindow.
	MaxCapturesPerWindow int

	// MaxWarningsPerWindow is the maximum number of warning-only events permitted within a
	// single CaptureWindow (GC pressure, heap trend, FD pressure, and scheduler-latency
	// rules share this budget).
	//
	// Defaults to 10. Warnings live in their own budget so flapping warnings cannot crowd
	// out real heap captures.
	MaxWarningsPerWindow int

	// SchedulerLatencyP99Threshold is the p99 scheduler latency above which the watchdog
	// emits a scheduler-latency warning, sourced from runtime/metrics
	// /sched/latencies:seconds; default 10ms, zero disables the rule.
	SchedulerLatencyP99Threshold time.Duration

	// Cooldown is the minimum duration between consecutive captures of the same profile
	// type.
	Cooldown time.Duration

	// MaxProfileSizeBytes is the maximum number of bytes a single profile capture may
	// produce.
	//
	// Captures exceeding this limit are discarded to prevent the watchdog from worsening
	// memory pressure. Default: 50 MiB.
	MaxProfileSizeBytes int64

	// CrashLoopThreshold is the minimum number of unclean exits within CrashLoopWindow that
	// triggers a CrashLoopDetected event. Default: 3.
	...
	// ContinuousProfilingInterval is the period between routine captures; default 10
	// minutes, validation enforces a minimum of 1 minute.
	ContinuousProfilingInterval time.Duration

	// CheckInterval is the period between watchdog evaluation ticks. Shorter intervals
	// detect anomalies faster at negligible CPU cost.
	CheckInterval time.Duration

	// ContinuousProfilingRetention is the maximum number of routine profile files retained
	// per type. Default: 6 (~1 hour of history at 10-minute cadence).
	ContinuousProfilingRetention int

	// ContentionDiagnosticCooldown is the minimum interval between two consecutive
	// diagnostics. Default: 30 minutes.
	ContentionDiagnosticCooldown time.Duration

	// ContentionDiagnosticWindowDuration is the period during which block + mutex profiling
	// are active before the diagnostic captures the resulting profiles and disables them;
	// default 60s, allowed range 1s to 5m.
	ContentionDiagnosticWindowDuration time.Duration

	// ContentionDiagnosticBlockProfileRate is the value passed to
	// runtime.SetBlockProfileRate for the duration of the diagnostic. Default: 1e6 (one
	// sample per 1ms of blocking).
	ContentionDiagnosticBlockProfileRate int

	// ContentionDiagnosticMutexProfileFraction is the value passed to
	// runtime.SetMutexProfileFraction for the duration of the diagnostic. Default: 100 (one
	// in 100 mutex contentions sampled).
	ContentionDiagnosticMutexProfileFraction int

	// CrashLoopWindow is the duration over which the startup history is inspected for
	// crash-loop detection; default 60s, zero disables startup-history processing entirely
	// (no crash-loop alerts and no previous-crash classification).
	CrashLoopWindow time.Duration

	// ContentionDiagnosticConsecutiveTrigger is the number of scheduler-latency events
	// within ContentionDiagnosticTriggerWindow that trigger auto-fire. Default: 3.
	ContentionDiagnosticConsecutiveTrigger int

	// ContentionDiagnosticTriggerWindow is the rolling window for counting scheduler-latency
	// events for auto-fire. Default: 15 minutes.
	ContentionDiagnosticTriggerWindow time.Duration

	// ContinuousProfilingNotify enables informational notifications for each routine
	...
	// ContentionDiagnosticAutoFire enables automatic diagnostic firing when
	// scheduler-latency events repeat. Default: false (manual only).
	ContentionDiagnosticAutoFire bool

	// DeltaProfilingEnabled enables storing a baseline heap profile alongside each capture
	// so the user can compute a diff. Default: false.
	DeltaProfilingEnabled bool

	// IncludeGoroutineStacks toggles per-goroutine stack capture. When enabled, each
	// goroutine profile firing also writes a human-readable .txt sidecar containing the full
	// stack of every goroutine (pprof debug=2), alongside the existing aggregated .pb.gz
	// binary profile.
	//
	// Useful when investigating goroutine leaks where you need to know the exact call site
	// or closure-captured arguments (e.g. which channel a publisher is parked on). Disabled
	// by default because the sidecar can be tens of megabytes per dump for processes with
	// many thousand goroutines.
	IncludeGoroutineStacks bool

	// Enabled controls whether the watchdog is active. When false, Start is a no-op.
	Enabled bool
}

// Watchdog monitors runtime metrics and automatically captures diagnostic profiles when
// anomalies are detected. It periodically reads system statistics from a SystemCollector
// and evaluates heap usage, goroutine counts, and GC pressure against configurable
// thresholds.
//
// Safe for concurrent use; mutable state is protected by a mutex and the stop channel
// coordinates shutdown. Fields that are only accessed from the single loop goroutine
// (heapTrendBuffer, lastTrendEvaluation, lastGoroutineLeakCheck) are safe by goroutine
// confinement.
type Watchdog struct {
	// lastContentionDiagnosticAt is the most recent successful diagnostic completion
	// timestamp; used to enforce the cooldown.
	lastContentionDiagnosticAt time.Time

	// heapHighWaterSetAt is the timestamp when heapHighWater was last updated, used to
	// determine when the high-water mark may be reset.
	heapHighWaterSetAt time.Time

	// lastTrendEvaluation is the timestamp of the most recent heap trend regression
	// computation. Confined to the loop goroutine.
	lastTrendEvaluation time.Time

	// lastGoroutineLeakCheck is the timestamp of the most recent goroutine leak profile
	// evaluation. Confined to the loop goroutine.
	lastGoroutineLeakCheck time.Time

	// startedAt records when the watchdog was started, used to enforce the warm-up period.
	// Written once in Start before the loop goroutine begins.
	startedAt time.Time

	// profileUploader uploads captured profiles to a remote storage backend for preservation
	// across pod restarts.
	//
	// May be nil when remote storage is not configured. Set before Start and not changed
	// afterwards.
	profileUploader WatchdogProfileUploader

	// notifier delivers event notifications to external systems such as Slack or PagerDuty.
	//
	// May be nil when notifications are not configured. Set before Start and not changed
	// afterwards.
	...
	//
	// May be nil if profiling is not enabled. Set before Start via SetProfilingController.
	...
	// heapTrendBuffer holds recent heap allocation samples for linear regression-based trend
	// detection.
	//
	// Nil when trend detection is disabled. Confined to the loop goroutine.
	heapTrendBuffer *heapTrendBuffer

	// lastCaptureTime records the most recent capture timestamp per profile type, used for
	// per-type cooldown enforcement.
	lastCaptureTime map[string]time.Time

	// lastWarningTime records the most recent warning timestamp per rule type, used for
	// per-rule warning cooldown enforcement. Distinct from lastCaptureTime so warning rules
	// and capture rules cannot starve each other.
	lastWarningTime map[string]time.Time

	// systemCollector provides the current system statistics for evaluation.
	systemCollector *SystemCollector

	// hostname is the machine hostname, cached at construction time for use in notifications
	// and upload metadata.
	hostname string

	// captureTimestamps is a sliding window of all capture timestamps, used for global rate
	// limiting.
	captureTimestamps []time.Time

	// schedulerLatencyEvents is a small bounded ring of recent scheduler-latency event
	// timestamps. Used by the contention diagnostic auto-fire to detect "N events in M
	// minutes".
	schedulerLatencyEvents []time.Time

	// eventRing is a bounded ring buffer of recently emitted watchdog events that
	// sendNotification appends to so the inspector surface (ListEvents and SubscribeEvents)
	// can serve recent history; older entries are evicted when the ring is full.
	eventRing []WatchdogEventInfo

	// eventSubscribers tracks active streaming subscribers receiving newly emitted events.
	// Subscribers that fall behind drop pending events rather than blocking the watchdog.
	eventSubscribers []*watchdogEventSubscriber

	// warningTimestamps is a sliding window of warning emission timestamps used for the
	// warning-budget global rate limit. Tracked separately so chatty warning rules cannot
	// consume the capture budget.
	warningTimestamps []time.Time

	// previousHeapProfile stores the compressed bytes of the last captured heap profile,
	// used to provide a diff baseline for the next capture.
	//
	...
	// backgroundWG tracks in-flight notification and upload goroutines so that Stop can wait
	// for all background work to complete.
	backgroundWG sync.WaitGroup

	// captureWG tracks in-flight profile capture goroutines so that Stop can wait for them
	// to finish before closing the profile store.
	captureWG sync.WaitGroup

	// initialHeapThreshold is the heap allocation threshold computed at startup from
	// GOMEMLIMIT or the configured byte threshold.
	initialHeapThreshold uint64

	// heapHighWater is the current heap allocation level that must be exceeded before a new
	// heap capture is triggered. Starts at initialHeapThreshold and escalates with each
	// capture.
	heapHighWater uint64

	// gomemlimit is the Go runtime memory limit, or math.MaxInt64 when no limit is set.
	gomemlimit int64

	// cgroupMemoryLimit is the cached cgroup memory limit in bytes captured once at Start.
	// The cgroup limit is stable for the lifetime of the process; reading it on every tick
	// would be wasted work.
	cgroupMemoryLimit uint64

	// contentionMu serialises access to the contention diagnostic; only one diagnostic runs
	// at a time so block/mutex profile rates are not stamped on by concurrent runs.
	contentionMu sync.Mutex

	// mu guards mutable state including heapHighWater, lastCaptureTime, captureTimestamps,
	// profilingController, previousHeapProfile, and stopped.
	mu sync.Mutex

	// goroutineBaseline is the goroutine count at startup, used to avoid spurious captures
	// during normal operation. Accessed atomically.
	goroutineBaseline atomic.Int32

	// stopped indicates whether Stop has been called.
	stopped bool

	// started indicates whether Start has already been called so a second invocation does
	...
	// rules and heap/allocs continuous captures are skipped.
...
// DefaultWatchdogConfig returns a WatchdogConfig populated with production defaults.
//
// Returns WatchdogConfig which contains sensible default values for all watchdog
// settings.
...
// NewWatchdog creates a new runtime watchdog that monitors system metrics and captures
// diagnostic profiles when thresholds are exceeded.
//
// Takes config (WatchdogConfig) which provides the threshold and timing configuration.
// The struct is intentionally passed by value because callers commonly compose it from
// DefaultWatchdogConfig at the call site; the 296-byte copy on a once-per-process
// constructor path is negligible.
// Takes systemCollector (*SystemCollector) which provides the system statistics for
// periodic evaluation.
// Takes opts (...WatchdogOption) which provides optional configuration functions to
// customise clock and sandbox dependencies.
//
...
// WithWatchdogClock sets the clock used by the watchdog for time operations. This is
// intended for testing with a mock clock.
//
...
// WithWatchdogSandbox sets the sandbox used by the watchdog's profile store. This is
// intended for testing with a controlled filesystem.
//
// Takes sandbox (safedisk.Sandbox) which provides filesystem access for profile storage.
//
// Returns WatchdogOption which configures the profile store sandbox when applied.
...
// WithWatchdogNotifier sets the notification delivery mechanism for watchdog events. This
// is intended for integration with external alerting systems.
//
...
// WithWatchdogProfileUploader sets the remote storage backend for profile uploads.
// Profiles are uploaded after being written to local disk.
//
...
// SetProfilingController sets the profiling controller used for on-demand profile
// captures. This must be called before Start for captures to succeed.
//
...
// Start begins periodic monitoring by spawning a background goroutine that evaluates
// system metrics on each tick. If the watchdog is disabled via config, the call is a
// no-op.
//
// The background goroutine runs until Stop is called or the context is cancelled.
...
// CapturePreDeathSnapshot captures a final diagnostic snapshot before the process
// terminates.
//
// It captures heap and goroutine profiles synchronously and sends a notification if
// configured. This is intended to be called from a shutdown hook when SIGTERM is
// received. The method respects the provided context's deadline to stay within the
...
// Stop signals the watchdog loop to exit and closes the profile store. Safe to call
// multiple times; only the first call closes the stop channel.
//
// Before closing the profile store, Stop marks the most recent startup-history entry as
// cleanly stopped so the next process start can distinguish clean exits from crashes.
//
// Safe for concurrent use; protected by the watchdog's mutex.
...
// ListProfiles returns metadata for all stored profile files, sorted by timestamp
...
// DownloadSidecar reads the JSON sidecar paired with the named profile and returns its
// bytes.
//
// Takes profileFilename (string) which is the .pb.gz profile filename; the sidecar is
// resolved by swapping the extension.
//
...
// Returns error when the profile filename is empty or the read fails for reasons other
// than absence.
...
// order (oldest first). A missing or unreadable history file yields a nil slice (and
// counts the read error in OTel).
//
// Returns []WatchdogStartupHistoryEntry which is the parsed history.
// Returns error when an existing file cannot be parsed (corruption); a missing file is
// not an error.
...
// DownloadProfile writes the raw compressed bytes of the named profile file to the
// provided writer. The caller is responsible for decompression.
//
...
// PruneProfiles removes stored profile files. When profileType is empty, all profiles are
// removed; otherwise only profiles of the specified type are removed.
//
// Takes profileType (string) which filters deletion to a specific profile category. Pass
// empty string to delete all profiles.
//
...
//
// Safe for concurrent use; acquires the watchdog's mutex to read mutable fields.
...
// wired. Without one, every capture path (continuous profiling, threshold-triggered
// captures, pre-death snapshot, contention diagnostic) silently no-ops, leaving operators
// with no profile artefacts when problems occur.
//
// The fix is to add piko.WithMonitoringProfiling() to the WithMonitoring option set,
// which constructs and wires the controller.
//
...
//
// The system-collector value is stable for the lifetime of the process, so reading it on
// every tick would be wasted work; the corresponding OTel gauge is recorded once here
// rather than per tick. The same call also records the soft FD limit gauge once because
// RLIMIT_NOFILE is also stable for the process.
...
// crash loops, then appends a new entry for the current process. The whole flow is
// best-effort: failures are logged plus counted but never block startup, since the
// watchdog must come up even when the profile directory is unwritable. Set
// CrashLoopWindow to zero to disable history processing entirely.
...
// loop runs the periodic evaluation loop until stopped or the context is cancelled. Each
// tick reads system statistics and evaluates them against the configured thresholds, then
// records the heartbeat.
//
// Recovered panics in the evaluation path are reported via OTel and the notifier; the
// loop deliberately does not auto-restart, in line with the "let it fail visibly"
// philosophy. A stale heartbeat plus an incremented panic counter is the externally
// visible signal that the watchdog stopped.
...
//
// Increments the loop-panic counter, logs the failure with stack trace, and emits a
// critical event so external monitoring can see that runtime monitoring is no longer
// active. It deliberately does not re-panic, but does not restart the loop either; by
// design the watchdog stays dead so an absent heartbeat signals the failure.
//
// Takes recovered (any) which is the value returned by recover() in the loop's deferred
// recovery handler.
...
// evaluate runs all rule evaluators against the provided system statistics. During the
// warm-up period, evaluations are suppressed to allow the runtime to stabilise after
// startup.
//
...
// triggerCapture spawns a background goroutine that captures a profile of the given type
// and stores it to disk along with its sidecar metadata. Errors are logged and counted
// but do not propagate to the caller.
//
// Takes profileType (string) which identifies the profile to capture (e.g. "heap",
// "goroutine").
// Takes capCtx (captureContext) which describes the firing rule and observed values for
...
//
// Stop acquires w.mu before calling captureWG.Wait or backgroundWG.Wait, so the stopped
// check inside the same lock is the only way to avoid the "WaitGroup.Go after Wait
// returned" panic that would otherwise occur if a rule fires concurrently with shutdown.
//
...
//
// Captures a profile via the profiling controller and writes the compressed data to the
// profile store. After a successful write it persists a JSON sidecar containing rule
// attribution and a snapshot of system stats so a future operator can reconstruct what
// triggered the capture without opening the profile in pprof first.
//
...
// activeProfilingController returns the configured profiling controller when the watchdog
// is still running.
//
// Takes profileType (string) which identifies the profile capture being attempted (used
// as a log attribute when the controller is missing).
//
// Returns ProfilingController which is the wired controller on success.
// Returns bool which is false when the watchdog has stopped or no controller is wired in.
//
...
// collectProfileBytes drives the controller capture and bounds the resulting payload by
// MaxProfileSizeBytes so an oversized profile never causes allocator pressure.
//
// Takes controller (ProfilingController) which performs the capture.
// Takes profileType (string) which identifies the profile to capture and is used as a log
// attribute on failure paths.
//
// Returns []byte which is the compressed profile on success, nil on failure or when
// discarded for size.
// Returns bool which is true on success and false when the capture failed or the result
// was discarded.
...
// maybeWriteGoroutineStacks writes a debug=2 per-goroutine stacks sidecar alongside a
// goroutine profile capture when WatchdogConfig.IncludeGoroutineStacks is enabled. The
...
// unconditionally on the capture path.
// Takes timestamp (string) which is the timestamp portion of the profile filename, used
// to keep the stacks sidecar paired with its binary.
...
// processStoredProfile handles post-storage actions for a captured profile including
// upload, delta baseline storage, and flight recorder snapshots.
//
...
// captureFlightRecorderSnapshot writes the current rolling execution trace buffer to the
// profile store.
//
// This provides scheduling and GC event context alongside pprof profiles. Silently
// returns when the flight recorder is not enabled.
//
// Safe for concurrent use; acquires the watchdog's mutex to read the profiling
// controller.
...
// compute a diff between consecutive captures using `go tool pprof -diff_base
// heap-baseline-*.pb.gz heap-*.pb.gz`.
//
// Takes currentProfileData ([]byte) which is the current heap profile to store as the
// next baseline.
//
// Safe for concurrent use; acquires the watchdog's mutex to swap the previous heap
// profile.
...
//
// If no notifier is set, this is a no-op. The notification is sent in a background
// goroutine tracked by backgroundWG so Stop can wait for it to complete.
//
...
// uploadProfile sends the captured profile data to the configured remote storage backend.
//
