...
const (
	...
	// profileFileExtension is the file extension used for compressed profile files.
	profileFileExtension = ".pb.gz"

	// profileSidecarExtension is the file extension used for sidecar JSON metadata that
	// pairs with each captured profile.
	profileSidecarExtension = ".json"

	// profileStacksExtension is the file extension used for the human-readable per-goroutine
	// stacks sidecar (pprof debug=2 output).
	profileStacksExtension = ".stacks.txt"

	// errFormatListingProfileDirectory is the error format string used when reading the
	// profile directory fails.
	errFormatListingProfileDirectory = "listing profile directory: %w"

	// startupHistoryFilename is the file in the profile directory that holds the
	// crash-history ring used to detect crash loops and unclean shutdowns across process
	// restarts.
	startupHistoryFilename = "startup_history.json"

	// maxStartupHistoryEntries caps the number of entries retained in the startup history
	// ring. The oldest entries are evicted when the cap is exceeded.
	maxStartupHistoryEntries = 10

	// profileNameSeparator separates the profile-type prefix from the timestamp portion of a
	// profile filename (e.g. "heap-20260418T120000.pb.gz").
	profileNameSeparator = "-"

	// maxStartupHistoryFileBytes caps the in-memory copy of startup_history.json. The file
	// is bounded by maxStartupHistoryEntries at write time; the cap here defends against an
	// attacker or corrupted disk producing an oversized file the watchdog would otherwise
	// json.Unmarshal in one shot.
	maxStartupHistoryFileBytes int64 = 256 * 1024

	// maxSidecarFileBytes caps the size of sidecar JSON read into memory. Sidecars are
	// watchdog-authored and small (<8 KiB typical); the cap prevents a malicious or corrupt
	// sidecar from dominating memory.
	maxSidecarFileBytes int64 = 1 * 1024 * 1024

	// maxProfileFileBytes caps the size of compressed profile bytes read into memory before
	// streaming to a remote inspector. pprof profiles are small in practice; the cap stops a
	// hand-edited file in the profile directory from exhausting memory.
	maxProfileFileBytes int64 = 256 * 1024 * 1024
)

var (
	// errEmptyFilename is returned when a read operation receives an empty filename.
	errEmptyFilename = errors.New("filename must not be empty")
)

// captureMetadata is the JSON sidecar stored next to each profile. It records the firing
// rule, observed value, and system stats at capture time so an operator can reconstruct
// what triggered the capture without opening the profile in pprof.
type captureMetadata struct {
	// CapturedAt is the instant the capture decision was made.
	CapturedAt time.Time `json:"captured_at,omitzero"`

	// RuntimeMetricsSnapshot is the curated map of runtime/metrics values at capture time.
	// May be nil when not collected.
	...
	// RuleFired identifies which evaluator triggered the capture as a free-form snake-case
	// string (for example "heap_high_water", "goroutine_threshold", "rss", "goroutineleak",
	// "pre_death", or "routine").
	RuleFired string `json:"rule_fired"`

	// ProfileType is the profile category captured (matches the profile filename prefix).
	...
	// GomemlimitBytes is the effective Go runtime memory limit in bytes. math.MaxInt64
	// indicates no limit.
	GomemlimitBytes int64 `json:"gomemlimit_bytes"`

	// ObservedValue is the metric value that triggered the capture, in the rule's natural
	// units (bytes for heap and RSS rules, count for the goroutine rule, and so on); zero
	// when the rule does not carry a single observed value.
	...
	// CgroupLimitBytes is the cgroup memory limit at capture time (zero when unknown).
	...
	// SchedulerLatencyP99Nanos is the runtime/metrics scheduler latency p99 derived from
	// /sched/latencies:seconds.
	SchedulerLatencyP99Nanos int64 `json:"scheduler_latency_p99_ns,omitempty"`

	// GCPauseP99Nanos is the runtime/metrics GC pause p99 derived from /gc/pauses:seconds.
...
// captureContext is passed from rule evaluators down through triggerCapture to
// captureAndStoreProfile so the sidecar metadata can attribute the capture to the rule
// that fired.
type captureContext struct {
	// Rule is a stable, snake-case identifier for the firing rule. Used as the value of
	// captureMetadata.RuleFired in the sidecar.
	Rule string

	// Observed is the metric value that triggered the capture (zero when the rule has no
...
// startupHistoryEntry records a single process start/stop in the history ring. Entries
// are appended at Start and patched with StoppedAt at Stop; a zero StoppedAt at the next
// Start signals an unclean exit (panic, OOM kill, SIGKILL).
type startupHistoryEntry struct {
	// StartedAt is the wall-clock instant the watchdog began monitoring this process.
	StartedAt time.Time `json:"started_at,omitzero"`

	// StoppedAt is the wall-clock instant of clean shutdown. Zero when the process exited
	// uncleanly.
	...
	// Reason is a free-form reason recorded at stop time ("clean", "unclean", "panic").
	// Empty when the process is still running.
...
type startupHistoryFile struct {
	// Entries is the bounded ring of historical process start/stop records, oldest first.
	// Capped to maxStartupHistoryEntries.
	Entries []startupHistoryEntry `json:"entries"`
}

// readHistory loads the startup history ring from disk. A missing file is not an error;
// callers distinguish "first run" from "read succeeded" via the bool return.
//
...
type profileEntry struct {
	...
	// Type is the profile category extracted from the file name prefix (e.g. "heap",
	// "goroutine").
	Type string
	...
	// HasSidecar reports whether a paired JSON sidecar with the same base name is present
	// alongside the profile.
	HasSidecar bool
}

// profileStore manages compressed pprof profile files on disk with automatic rotation. It
// writes gzip-compressed profiles and removes the oldest files when the per-type limit is
// exceeded.
//
...
type profileStore struct {
	...
	// maxProfilesPerType is the maximum number of profile files to retain per profile type
	// before the oldest are deleted.
	maxProfilesPerType int

	// mu guards all filesystem operations to prevent concurrent writes from corrupting file
	// state or racing during rotation.
	mu sync.Mutex
}

// newProfileStore creates a new profile store rooted at the given directory.
//
// Takes directory (string) which is the absolute path where profile files are stored. The
// directory is created if it does not exist.
// Takes maxPerType (int) which is the maximum number of profile files to keep per profile
// type before rotating.
// Takes clk (clock.Clock) which provides the time source for file name timestamps.
//
...
// write compresses the provided profile data with gzip and writes it to disk with a
// timestamped file name. After writing, it rotates old profiles of the same type to stay
// within the configured limit.
//
// Takes profileType (string) which identifies the profile category (e.g. "heap",
// "goroutine") and is used as the file name prefix.
// Takes data ([]byte) which is the raw pprof profile data to compress and store.
//
// Returns string which is the timestamp portion of the written filename so the caller can
// pair a sidecar metadata file with the same timestamp.
...
// writeMetadata persists the sidecar JSON for a profile capture. The sidecar shares the
// same base name as the profile so consumers can pair them by stripping the .pb.gz /
// .json extensions.
//
// Takes profileType (string) which identifies the profile category.
// Takes timestamp (string) which is the timestamp portion of the filename (use the value
...
// writeStacks persists the human-readable per-goroutine stacks sidecar that pairs with a
// goroutine profile capture. The file shares the same base name as the profile so
// consumers can pair them by stripping the .pb.gz / .stacks.txt extensions.
//
// Takes profileType (string) which identifies the profile category (only "goroutine" is
// meaningful today, but the helper is type-agnostic).
// Takes timestamp (string) which is the timestamp portion of the filename (use the value
// returned from write to pair correctly).
...
// writeWithRetention is the routine-mode counterpart to write. It accepts an explicit
// retention parameter so continuous-profiling rotation can be independent from the
// per-type cap used by threshold-triggered captures.
//
// Takes profileType (string) which identifies the profile category and is used as the
// file name prefix.
// Takes data ([]byte) which is the raw pprof profile data to compress and store.
...
// rotateProfilesWithMax is the underlying rotation routine accepting an explicit max-file
// count. rotateProfiles delegates to this with the store's default per-type limit.
//
...
// rotateProfiles removes the oldest profile files for the given type when the count
// exceeds maxProfilesPerType. Files are sorted by name (which embeds a timestamp) so the
// oldest sort first.
//
...
// list returns all profile entries in the store, sorted by timestamp descending (newest
// first). Each entry's type and timestamp are parsed from the file name.
//
...
// indexSidecarPresence returns a set of profile basenames (without extension) that have a
// paired JSON sidecar in the same directory. Pre-indexing avoids a per-profile stat call
// during listing.
//
// Takes entries ([]fs.DirEntry) which is the directory listing to scan.
//
// Returns map[string]struct{} keyed by sidecar basename (sidecar extension stripped) for
// fast membership tests during list().
...
// profileEntryFromDirEntry parses a directory entry into a profileEntry, returning
// ok=false for non-profile files, malformed names, or unreadable metadata. Filtering
// happens here so list() stays under the cognitive complexity budget.
//
// Takes entry (fs.DirEntry) which is the directory entry under inspection.
// Takes sidecars (map[string]struct{}) which is the precomputed set of basenames that
// have a paired sidecar.
//
// Returns profileEntry which describes the profile when the entry is a well-formed
// profile file.
...
// readSidecar returns the raw bytes of the JSON sidecar paired with the named profile
// file. Resolves the sidecar by stripping the profile extension and appending the sidecar
// extension.
//
...
// than absence (e.g. permission denied).
//
// Safe for concurrent use; protected by the store's mutex.
...
// read returns the raw compressed bytes of the named profile file. The caller is
...
// deleteByType removes all profile files whose name starts with the given profile type
// prefix.
//
// Takes profileType (string) which identifies the profile category to delete (e.g.
// "heap", "goroutine").
//
