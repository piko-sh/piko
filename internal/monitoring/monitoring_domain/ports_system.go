...
type SystemStats struct {
	...
	// Process holds the process-level statistics such as PID, thread count, resource count,
	...
	// Schedule contains scheduler latency, goroutine counts, and GOMAXPROCS as reported by
	// runtime/metrics. Histogram-derived percentiles are zero when no samples are present
	// yet.
	...
	// TimestampMs is the Unix timestamp in milliseconds when the stats were collected.
	TimestampMs int64
...
type GCInfo struct {
	// RecentPauses contains recent GC pause durations in nanoseconds. Derived from the
	// /gc/pauses:seconds histogram by retaining a small ring of the most recent pauses
	// observed across collector ticks.
	RecentPauses []uint64
	...
	// NextGC is the heap size target for the next garbage collection cycle in bytes.
	NextGC uint64
	...
	// PauseP50 is the p50 GC pause derived from the runtime histogram. Zero when no samples
	// are present yet.
...
type SchedulerInfo struct {
	...
	// GoroutineCount mirrors the value of /sched/goroutines:goroutines and is reported
	// alongside SystemStats.NumGoroutines for symmetry with other runtime/metrics fields.
...
// SyncInfo summarises mutex contention from runtime/metrics. Mutex profiling must be
// enabled via runtime.SetMutexProfileFraction for these values to be non-zero; otherwise
// the runtime reports them as zero.
type SyncInfo struct {
	// MutexWaitTotalSeconds is a monotonic counter of cumulative seconds goroutines have
	// spent blocked acquiring mutexes. Derive the contention rate by sampling twice and
	// dividing the delta by elapsed wall time.
	MutexWaitTotalSeconds float64
}

// MemoryInfo holds memory usage data for the running process.
//
// Legacy fields are populated from runtime.MemStats. Newer fields prefixed with the
// runtime/metrics canonical name segment (HeapFreeBytes etc.) come from runtime/metrics
// directly. Both sets are populated by the system collector each tick.
type MemoryInfo struct {
	// Alloc is the current heap allocation in bytes.
	Alloc uint64

	// TotalAlloc is the cumulative bytes allocated over the lifetime of the process.
	TotalAlloc uint64
	...
	// /memory/classes/heap/objects:bytes (canonically equivalent to HeapAlloc but exposed
	...
	// HeapReleasedBytes is heap memory returned to the OS via madvise. Sourced from
	// /memory/classes/heap/released:bytes (parallel of the legacy HeapReleased field).
	...
	// HeapUnusedBytes is heap memory reserved for future allocation but not currently
	// containing objects, sourced from /memory/classes/heap/unused:bytes.
...
type ProcessInfo struct {
	...
	// CgroupMemoryLimit is the container memory limit in bytes from the cgroup filesystem.
	// Zero indicates the limit is unknown or unlimited.
	CgroupMemoryLimit uint64

	// CgroupMemoryCurrent is the container's current memory usage in bytes from the cgroup
	// filesystem. Zero indicates the value is unavailable.
