...
const (
	// metricNameHeapObjects is the runtime/metrics path for live heap object bytes (the
	// canonical "heap alloc" replacement).
	metricNameHeapObjects = "/memory/classes/heap/objects:bytes"

	// metricNameHeapFree is the runtime/metrics path for heap memory free for reuse but not
	// returned to the OS.
	metricNameHeapFree = "/memory/classes/heap/free:bytes"

	// metricNameHeapReleased is the runtime/metrics path for heap memory returned to the OS.
	metricNameHeapReleased = "/memory/classes/heap/released:bytes"

	// metricNameHeapStacks is the runtime/metrics path for memory backing goroutine stacks.
	metricNameHeapStacks = "/memory/classes/heap/stacks:bytes"

	// metricNameHeapUnused is the runtime/metrics path for heap memory reserved but neither
	// in use nor free for reuse.
	metricNameHeapUnused = "/memory/classes/heap/unused:bytes"

	// metricNameMemoryTotal is the runtime/metrics path for the total number of bytes
	// obtained from the OS by the runtime.
	metricNameMemoryTotal = "/memory/classes/total:bytes"

	// metricNameAllocsBytes is the runtime/metrics path for cumulative bytes allocated to
	// the heap.
	metricNameAllocsBytes = "/gc/heap/allocs:bytes"

	// metricNameFreesBytes is the runtime/metrics path for cumulative bytes freed from the
	// heap.
	metricNameFreesBytes = "/gc/heap/frees:bytes"

	// metricNameAllocsObjects is the runtime/metrics path for cumulative objects allocated.
	metricNameAllocsObjects = "/gc/heap/allocs:objects"

	// metricNameFreesObjects is the runtime/metrics path for cumulative objects freed.
	metricNameFreesObjects = "/gc/heap/frees:objects"

	// metricNameLiveObjects is the runtime/metrics path for the current live object count on
	// the heap.
	metricNameLiveObjects = "/gc/heap/objects:objects"

	// metricNameGCCycles is the runtime/metrics path for the count of completed automatic GC
	// cycles.
	metricNameGCCycles = "/gc/cycles/automatic:gc-cycles"

	// metricNameGCCyclesForced is the runtime/metrics path for the count of GC cycles forced
	// by the application.
	metricNameGCCyclesForced = "/gc/cycles/forced:gc-cycles"

	// metricNameGCPauses is the runtime/metrics histogram of stop-the-world GC pause
	// durations.
	metricNameGCPauses = "/gc/pauses:seconds"

	// metricNameGCGoGC is the runtime/metrics path for the current GOGC percent setting.
	metricNameGCGoGC = "/gc/gogc:percent"

	// metricNameGCGoMemLimit is the runtime/metrics path for the effective GOMEMLIMIT in
	// bytes.
	metricNameGCGoMemLimit = "/gc/gomemlimit:bytes"

	// metricNameSchedLatencies is the runtime/metrics histogram of goroutine scheduling
	// latencies in seconds.
	metricNameSchedLatencies = "/sched/latencies:seconds"

	// metricNameGoroutines is the runtime/metrics path for the current goroutine count.
	metricNameGoroutines = "/sched/goroutines:goroutines"

	// metricNameGoMaxProcs is the runtime/metrics path for the configured GOMAXPROCS thread
	// count.
	metricNameGoMaxProcs = "/sched/gomaxprocs:threads"

	// metricNameMutexWait is the runtime/metrics path for the cumulative mutex contention
	// wait time.
	metricNameMutexWait = "/sync/mutex/wait/total:seconds"

	// metricNameCPUClassesGC is the runtime/metrics path for cumulative CPU time spent in
	// GC.
	metricNameCPUClassesGC = "/cpu/classes/gc/total:cpu-seconds"

	// metricNameCPUClassesUser is the runtime/metrics path for cumulative CPU time spent in
	// user code.
	metricNameCPUClassesUser = "/cpu/classes/user:cpu-seconds"

	// metricNameCPUClassesTotal is the runtime/metrics path for the cumulative CPU time the
	// runtime has used in total.
	metricNameCPUClassesTotal = "/cpu/classes/total:cpu-seconds"
)

const (
	// quantileP50 is the median quantile extracted from runtime histograms (GC pauses,
	// scheduler latencies) for sidecar metadata.
	quantileP50 = 0.50

	// quantileP95 is the 95th-percentile quantile extracted from runtime histograms.
	quantileP95 = 0.95

	// quantileP99 is the 99th-percentile quantile extracted from runtime histograms.
	quantileP99 = 0.99
)
const (

	// nanosPerSecond converts seconds to nanoseconds when computing histogram quantile
	// durations.
	nanosPerSecond = 1e9
)

var (
	// requestedRuntimeMetrics is the canonical list of metrics this collector samples. The
	// order is preserved across reads so callers can rely on indices rather than name
	// lookups inside hot paths.
	requestedRuntimeMetrics = []string{
...
// runtimeMetricsSnapshot is the curated view of /proc-equivalent runtime telemetry
// produced by sampling runtime/metrics once per system collector tick. Fields default to
// zero when the underlying metric is unavailable on the running Go version.
type runtimeMetricsSnapshot struct {
	// SampledAt is the monotonic timestamp of the read.
	SampledAt time.Time

	// HeapObjectsBytes is the live heap object footprint in bytes (the canonical "heap
	// alloc" replacement under runtime/metrics).
	HeapObjectsBytes uint64

	// HeapFreeBytes is heap memory free for reuse but not returned to the OS.
	...
	// HeapUnusedBytes is heap memory reserved but neither in use nor free for reuse.
	HeapUnusedBytes uint64

	// TotalMemoryBytes is the total bytes obtained from the OS.
	TotalMemoryBytes uint64

	// AllocsBytes is the cumulative bytes allocated to the heap since process start.
	AllocsBytes uint64

	// FreesBytes is the cumulative bytes freed from the heap since process start.
	FreesBytes uint64
	...
	// NumGC is the count of completed automatic GC cycles since process start.
	NumGC uint64

	// NumForcedGC is the count of GC cycles forced by the application.
	NumForcedGC uint64

	// GCPauseP50 is the median stop-the-world GC pause derived from the /gc/pauses:seconds
	// histogram. Zero when no samples are present.
	GCPauseP50 time.Duration

	// GCPauseP95 is the 95th-percentile stop-the-world GC pause derived from the
	// /gc/pauses:seconds histogram.
	GCPauseP95 time.Duration

	// GCPauseP99 is the 99th-percentile stop-the-world GC pause derived from the
	// /gc/pauses:seconds histogram.
	...
	// GoMemLimit is the effective GOMEMLIMIT in bytes (math.MaxInt64 when unlimited).
	GoMemLimit int64

	// SchedulerLatencyP50 is the median goroutine scheduling latency derived from
	// /sched/latencies:seconds.
	SchedulerLatencyP50 time.Duration

	// SchedulerLatencyP99 is the 99th-percentile goroutine scheduling latency derived from
	// /sched/latencies:seconds.
	...
	// MutexWaitTotalSeconds is the cumulative time spent waiting for mutexes across all
	// goroutines. Sample twice and divide the delta by elapsed wall-clock time to derive a
	// contention rate.
	...
	// CPUTotalSeconds is the cumulative CPU time the runtime has used in total.
...
type runtimeMetricsCollector struct {
	// indices maps metric name to position in samples for the metrics that were registered
	// at construction. Metrics not advertised by the running Go version are absent from this
	// map.
	indices map[string]int

	// available reports whether each metric in requestedRuntimeMetrics was found at
	// construction. Used by extract to decide whether to read or leave the snapshot field at
	// zero.
	available map[string]bool

	// samples is reused across reads. metrics.Read writes into Sample.Value in place;
	// nothing else mutates the slice.
...
// sample reads the runtime/metrics view and returns a populated snapshot. metrics.Read is
// lock-free in the runtime and significantly cheaper than runtime.ReadMemStats on large
// heaps because it does not stop the world.
//
// Takes now (time.Time) which is recorded in SampledAt for downstream observers.
//
...
// uint64 returns the named metric's value as uint64, or 0 when the metric is unavailable
// or has a different kind.
//
...
// int64 returns the named metric's value as int64, accepting either Uint64 or Float64
// sources (the runtime declares /sched/gomaxprocs:threads as Uint64 but /gc/gogc:percent
// as Uint64; defensive against future kind changes).
//
// Takes name (string) which is the canonical runtime/metrics path.
//
// Returns int64 value, or 0 when the metric is missing or wrong-kind. The uint64 path
// saturates at math.MaxInt64 to avoid wrap.
...
// float64 returns the named metric's value as float64, or 0 when the metric is
...
// histogram returns the named metric's value as a Float64Histogram, or nil when the
// metric is unavailable or has a different kind.
//
// Takes name (string) which is the canonical runtime/metrics path.
//
// Returns *metrics.Float64Histogram or nil when the metric is missing or wrong-kind.
...
// histogramQuantile computes a quantile from a Float64Histogram by walking the cumulative
// distribution.
//
// The histogram's Buckets slice describes open-ended bins via boundary values; Counts
// gives the count per bin. The quantile q must be in [0, 1]. For samples falling in the
// last bin (no upper bound), histogramQuantile returns the value of the last finite
// boundary; this is conservative and under-reports tail values.
//
// Takes hist (*metrics.Float64Histogram) which is the runtime histogram being summarised;
// nil is treated as empty.
// Takes q (float64) which is the quantile in [0, 1]; out-of-range values are clamped.
//
// Returns time.Duration which is the bucket boundary at the quantile, or zero when the
// histogram is missing or has no samples.
...
// secondsToDuration converts a fractional-seconds float into a time.Duration, clamping at
...
// isInfOrNaN reports whether the supplied float is +Inf, -Inf, or NaN. Uses math.IsNaN /
// math.IsInf so the inputs are checked via well-known helpers rather than the f != f
// idiom, which can confuse linters.
//
...
// lastSnapshotMap returns a curated map of the most recent runtime metrics suitable for
// embedding in capture sidecar metadata. Histograms are summarised as p50/p95/p99 to keep
// the JSON small; counters are reported verbatim.
//
// Takes snap (runtimeMetricsSnapshot) which is the snapshot to project into a
// JSON-friendly map.
//
// Returns map[string]any keyed by canonical runtime/metrics names plus the derived
// percentile keys. Empty when the collector has not been sampled.
