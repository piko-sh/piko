...
// MonitoringService manages the monitoring subsystem lifecycle. It encapsulates telemetry
// collection, OTEL SDK integration, and an optional transport layer for remote access to
// monitoring data.
type MonitoringService interface {
	// Start begins background collection and, when a transport is configured, the transport
	// server. Blocks until the context is cancelled or an error occurs.
	//
	...
	//
	// The concrete type satisfies sdktrace.SpanProcessor when the SDK is loaded and should
	// be registered with the OpenTelemetry trace provider.
	//
	...
	//
	// The concrete type satisfies sdkmetric.Reader when the SDK is loaded and should be
	...
	// Address returns the transport server listen address, or the configured address when no
	...
	//
	// This must be called before Start() for the inspectors to be available via the
	// transport. It is safe to call multiple times; the last values are used.
	//
	...
	// SetProviderInfoInspector sets the provider info inspector for resource discovery. Must
	// be called before Start() for the inspector to be available via the transport.
	//
	// Takes inspector (ProviderInfoInspector) which may be nil.
	SetProviderInfoInspector(inspector ProviderInfoInspector)

	// SetRenderCacheStatsProvider sets the render cache stats provider. Must be called
	// before Start() for cache statistics to be included in the system stats response.
	//
...
// SystemStatsProvider provides system statistics. Implementations collect runtime and
// system metrics for the monitoring service.
...
// ResourceProvider provides resource information. Implementations collect resource stats
...
// RenderCacheStatsProvider provides render cache statistics for the monitoring service.
// The render registry adapter implements RenderCacheStatsProvider so that component and
// SVG cache sizes can be reported via the system stats endpoint.
type RenderCacheStatsProvider interface {
	// GetComponentCacheSize returns the number of entries in the component metadata cache.
	//
...
// MetricsExporter provides metrics export capabilities by integrating with the OTEL
// MeterProvider and exposing an HTTP handler. Implementations include the Prometheus
// exporter.
type MetricsExporter interface {
	// Reader returns the metric reader for OTEL MeterProvider registration. Metrics recorded
	// through OTEL will be available via Handler().
	//
	// Returns MetricReader for MeterProvider registration.
	Reader() MetricReader

	// Handler returns an HTTP handler that serves metrics. This handler should be mounted at
	// the configured metrics path (typically /metrics).
	//
	// Returns http.Handler which serves metrics in the exporter's format.
	Handler() http.Handler
}

// MonitoringDeps holds all dependencies required by the monitoring transport services.
type MonitoringDeps struct {
	// OrchestratorInspector provides read-only access to orchestrator state. May be nil if
	// orchestrator is not available.
	OrchestratorInspector orchestrator_domain.OrchestratorInspector

	// RegistryInspector provides read-only access to registry state. May be nil if registry
	// is not available.
	RegistryInspector registry_domain.RegistryInspector

	// DispatcherInspector provides read-only access to dispatcher state and DLQs. May be nil
	// if no dispatchers are configured.
	DispatcherInspector dispatcher_domain.DispatcherInspector

	// RateLimiterInspector provides read-only access to rate limiter state. May be nil if
	// rate limiter is not configured.
	RateLimiterInspector ratelimiter_domain.RateLimiterInspector

	// TelemetryProvider provides access to OTEL metrics and traces. May be nil if telemetry
	// is not configured.
	TelemetryProvider TelemetryProvider

	// SystemStatsProvider provides system statistics. May be nil if system stats collection
	// is not enabled.
	SystemStatsProvider SystemStatsProvider

	// ResourceProvider provides resource information. May be nil if FD tracking is not
	// enabled.
	ResourceProvider ResourceProvider

	// HealthProbeService provides health check functions. May be nil if health probing is
	// not set up.
	HealthProbeService HealthProbeService

	// ProviderInfoInspector provides read-only access to provider information across hexagon
	// services. May be nil if no services implement ResourceDescriptor.
	ProviderInfoInspector ProviderInfoInspector

	// RenderCacheStatsProvider provides render cache statistics. May be nil if the render
	// registry is not available.
	RenderCacheStatsProvider RenderCacheStatsProvider

	// ProfilingController manages on-demand pprof profiling. May be nil if remote profiling
	// is not enabled.
	ProfilingController ProfilingController

	// WatchdogInspector provides read-only access to watchdog state and stored profiles. May
	// be nil if the watchdog is not enabled.
	WatchdogInspector WatchdogInspector
}

// HealthProbeService provides health check methods for liveness and readiness. It
// implements monitoring_domain.HealthProbeService.
...
// MetricsCollectorAdapter defines the interface for metrics collection adapters.
// Implementations collect metrics from OTEL and store them in the TelemetryStore.
...
// ProviderInfoInspector provides read-only access to provider information across all
// hexagon services. It aggregates data from ResourceDescriptors registered at bootstrap
// time.
type ProviderInfoInspector interface {
	// ListResourceTypes returns the names of all registered resource types (e.g. "email",
	// "storage", "cache").
	//
	// Returns []string which contains the sorted resource type names.
	ListResourceTypes(ctx context.Context) []string

	// ListProviders returns column definitions and provider rows for a specific resource
	// type.
	//
	...
	// DescribeProvider returns detailed information for a single provider within a resource
	// type.
	//
	...
	// ListSubResources returns sub-resources for a named provider within a resource type.
	// Only works when the service implements SubResourceDescriptor.
	//
	// Takes resourceType (string) which identifies the resource.
	...
	// DescribeResourceType returns a service-level overview for the given resource type.
	// Only works when the service implements ResourceTypeDescriptor.
	//
	// Takes resourceType (string) which identifies the resource.
	//
...
type TransportServer interface {
	// Start begins serving on the configured address. Blocks until the context is cancelled
	// or an error occurs.
	//
	// Takes ctx (context.Context) for cancellation.
	//
...
// TransportConfig holds transport-agnostic server settings passed from the domain to the
// transport factory.
type TransportConfig struct {
	// Address is the TCP address to listen on (e.g. ":9091" or "localhost:9091").
	Address string

	// TLS holds the resolved TLS settings for the transport. When enabled, the transport
...
// calling methods directly. Concrete implementations (e.g. from otel/sdk/trace) satisfy
// SpanProcessor via structural typing.
...
//
// The domain layer passes these opaquely between adapters and the OTEL setup without
// calling methods directly. Concrete implementations (e.g. from otel/sdk/metric) satisfy
// MetricReader via structural typing.
...
// SpanProcessorFactory creates a span processor that stores spans in the given store,
// letting adapters provide their span processor implementation.
...
type ProfilingController interface {
	...
	// Close cancels any pending auto-disable timer and disables profiling. It is safe for
	// use as a shutdown hook.
	//
	// Returns error when the shutdown fails.
	Close(ctx context.Context) error

	// Status returns the current profiling state including whether it is enabled, the port,
	// time remaining, and available profile types.
	//
	...
	// provided writer. Returns an error when the flight recorder is not enabled or the write
	// fails.
	//
	...
	// CaptureProfile captures a Go runtime profile and writes the raw pprof data to the
	// provided writer. For duration-based profiles (cpu, trace), this blocks for the
	// requested duration.
	//
	...
	//
	// Returns string which contains any warning message (e.g. when runtime rates are not
	// configured for the requested profile type).
...
type ProfilingEnableOpts struct {
	...
	// BlockProfileRate sets the Go runtime block profile rate. Zero uses the default
	// production-safe rate.
	BlockProfileRate int

	// MutexProfileFraction sets the Go runtime mutex profile fraction. Zero uses the default
	// production-safe fraction.
...
type ProfilingStatus struct {
	...
	// AlreadyEnabled is true when profiling was already active before the current Enable
	// call. This allows the transport layer to inform the caller that the session was
	// extended rather than freshly started.
	AlreadyEnabled bool
}
