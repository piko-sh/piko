...
// HeaderFiles returns all HeaderFile definitions for the interp_domain
// dispatch header files.
//
// Returns []asmgen.HeaderFile which contains the offset header and the
// per-architecture dispatch macro headers.
func HeaderFiles() []asmgen.HeaderFile {
...
// dispatchMacrosForArchitecture finds the BytecodeArchitecturePort matching the
// given architecture and returns its dispatch macros. If no matching
// architecture is found, it returns an empty string.
//
// Takes archs ([]asmgen.ArchitecturePort) which is the set of registered
...
// emitDispatchOffsetsHeader returns the complete dispatch offsets header file
// content by concatenating the licence preamble, exit reason constants, call
// frame offsets, extended dispatch context offsets, ASM call info offsets, and
// variable location offsets.
//
// Returns string which is the full header file content.
func emitDispatchOffsetsHeader() string {
	return emitOffsetsLicenceAndPreamble() +
		emitOffsetsExitReasons() +
		emitOffsetsCallFrame() +
		emitOffsetsDispatchContextExtended() +
		emitOffsetsASMCallInfo() +
		emitOffsetsVarLocation()
}

// emitOffsetsLicenceAndPreamble returns the Apache 2.0 licence header, the
// DispatchContext field offset documentation, and the instruction encoding
// comment that appear at the top of the generated offsets header file.
//
...
// emitOffsetsExitReasons returns the #define block for exit reason constants
// used by the assembly dispatch loop to signal why control returned to Go.
//
...
// emitOffsetsCallFrame returns the #define block for call frame size and
// field offsets used by the inline call and return handlers.
//
// Returns string which is the call frame offset #define block.
func emitOffsetsCallFrame() string {
	return `// callFrame size in bytes (verified by TestCallFrameOffsets).
...
// emitOffsetsDispatchContextExtended returns the #define block for extended
// DispatchContext field offsets beyond the core dispatch fields, covering
// inline call state, arena indices, and register bank base pointers.
//
// Returns string which is the extended dispatch context offset #define block.
func emitOffsetsDispatchContextExtended() string {
	return `// DispatchContext extended field offsets (inline call/return):
...
// emitOffsetsASMCallInfo returns the #define block for asmCallInfo struct
// field offsets and the size shift constant used by the inline call handler
// to index into the call info array.
//
// Returns string which is the ASM call info offset #define block.
func emitOffsetsASMCallInfo() string {
...
// emitOffsetsVarLocation returns the #define block for varLocation struct
// field offsets used by the return handler to read return destination metadata.
//
// Returns string which is the varLocation offset #define block.
func emitOffsetsVarLocation() string {
