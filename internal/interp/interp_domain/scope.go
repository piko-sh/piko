...
const (
	...
	// instead of receiving a real register. Registers 0 through spillThreshold-1 are
	// available for normal allocation; spillThreshold through maxRegisterIndex are reserved
...
//
// IMPORTANT: The field layout is accessed at runtime by ASM dispatch code via the VL_*
// offsets in asm_dispatch_offsets.h. That header is auto-generated from the layout via
// `go generate` (see gen.go, which invokes cmd/asmgen). TestVarLocationOffsets verifies
// the post-generation offsets; reordering is safe as long as `go generate
// ./internal/interp/interp_domain/...` is re-run.
type varLocation struct {
	// sourceType is the exact source-level reflect.Type of the value this location holds, as
	// recorded by compileExpression from go/types. nil when the type is unknown or not
	// applicable.
	//
	// Used by emitBoxToGeneral to emit opPackTyped (preserving the precise
	// int/int8/int32/float32/... identity) instead of opPackInterface (which collapses
	// everything to int64/float64). Without this, an interface holding `int(5)` and one
	// holding `int64(5)` would be indistinguishable and type assertions would misfire.
	// Placed after spillSlot so the ASM-pinned field offsets are unchanged.
	...
	// 256) rather than a directly-addressable register. Must be materialised via opReload
	// before use in instructions and stored back via opSpill after writes.
	isSpilled bool
}

// registerAllocator manages register allocation within a function, allocating
// sequentially per bank and freeing when scopes exit. Each function has its own
// allocator.
//
// nextRegister and peakRegister use uint32 to avoid silent overflow when a function needs
// exactly 256 registers in a bank (indices 0-255). With uint8, incrementing past 255
// wraps to 0 and peakRegister under-reports, causing runtime panics.
//
// recycledRegisters holds registers returned by dead-variable analysis. declareVar uses
// allocOrRecycle to prefer these over fresh sequential allocation, keeping peak register
// counts low for functions with many short-lived variables.
type registerAllocator struct {
	...
	// recycledRegisters holds register indices returned to the allocator by dead-variable
	// analysis.
	//
	// Entries are consumed LIFO by allocOrRecycle. Only entries at or above the current
	// scope's base index may be consumed, preventing inner scopes from stealing outer-scope
	// recycled registers.
...
//
// Takes kind (registerKind) which specifies the register bank to allocate from.
//
...
// order (LIFO).
//
// Takes kind (registerKind) which specifies the register bank to free from.
...
// otherwise allocates a fresh register sequentially. Used by declareVar for named
// variable allocation so that dead-variable registers are reused.
//
// Takes kind (registerKind) which specifies the register bank to allocate from.
// Takes baseFreeIndex (int) which is the free-list length when the current scope was
// entered. Only entries at or above this index may be consumed, preventing inner scopes
// from stealing outer scope recycled registers.
//
...
func (r *registerAllocator) allocOrRecycle(kind registerKind, baseFreeIndex int) uint8 {
	if length := len(r.recycledRegisters[kind]); length > baseFreeIndex {
		...
		if uint32(register) >= r.nextRegister[kind] {
			r.nextRegister[kind] = uint32(register) + 1
			if r.nextRegister[kind] > r.peakRegister[kind] {
				r.peakRegister[kind] = r.nextRegister[kind]
			}
		}
...
//
// Takes kind (registerKind) which specifies the register bank to check.
...
// allocSpill allocates a spill slot in the given bank.
//
// The returned value is a 0-based index; the actual register file index at runtime is 256
// plus the returned value. The compile-side recover in service_compile.go surfaces a
// spill-area-exhausted panic as a compilation failure rather than allowing silent state
// corruption from a uint16 wrap.
//
// Takes kind (registerKind) which specifies the register bank to allocate a spill slot
// in.
//
// Returns uint16 which is the spill slot index.
//
// Panics with errSpillAreaExhausted when the per-bank spill area is exhausted.
func (r *registerAllocator) allocSpill(kind registerKind) uint16 {
	if r.nextSpillSlot[kind] >= maxSpillSlotsPerBank {
		panic(fmt.Errorf("%w: bank %s in function %q exhausted at %d slots",
			errSpillAreaExhausted, kind, r.functionName, maxSpillSlotsPerBank))
	}
	...
	return uint16(slot) //nolint:gosec // bounded above by maxSpillSlotsPerBank
}
...
type lexicalScope struct {
	...
	// scope was entered. On scope exit, the free lists are truncated back to these lengths,
	// discarding any entries added by the inner scope (those registers are freed by the
