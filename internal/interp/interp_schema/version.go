...
var (
	...
	// SchemaHash is the SHA-256 hash of bytecode.fbs, computed at init time. This hash
...
// Pack wraps a serialised bytecode FlatBuffer with the schema version hash. The returned
// slice has format: [32-byte hash][payload].
func Pack(payload []byte) []byte {
	return fbs.PackAlloc(SchemaHash, payload)
}

// PackInto writes the schema hash and payload into destination. destination must have
// length >= fbs.PackedSize(len(payload)).
