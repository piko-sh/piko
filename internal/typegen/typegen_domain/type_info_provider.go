...
// TypeInfoProvider provides TypeScript type information for LSP intellisense. This is the
// interface that the LSP depends on to get completion data for piko.* and action.*
// namespaces.
type TypeInfoProvider interface {
	// GetPikoCompletions returns completions for the piko namespace, returning top-level
	// completions (refs, partial, etc.) when namespace is empty, or sub-namespace
	// completions when namespace is "nav", "form", etc.
	//
	// Takes namespace (string) which specifies the sub-namespace, or empty for top-level.
	//
	// Returns []CompletionItem which contains the available completions.
	GetPikoCompletions(namespace string) []CompletionItem

	// GetPikoSubNamespaces returns the available piko sub-namespaces. Used when the user
	// types "piko." to show nav, form, ui, etc.
	//
	// Returns []string which contains the sub-namespace names.
	GetPikoSubNamespaces() []string

	// GetActionCompletions returns completions for registered actions, filtered by the given
	// prefix.
	//
	// Takes prefix (string) which filters action names (e.g., "customer" matches customer.*
	// actions).
	//
...
type CompletionItem struct {
	...
	// InsertText is the text to insert when the item is selected. If empty, Label is used.
	InsertText string
