...
var (
	//go:embed action_manifest.fbs
	schemaContent []byte

	// SchemaHash is the SHA-256 hash of action_manifest.fbs, computed at init time. This
	// hash changes whenever the schema file is modified, so the cache invalidates
	// automatically when the schema evolves.
	SchemaHash = fbs.ComputeSchemaHash(schemaContent)
)

// Pack wraps a serialised Action Manifest FlatBuffer with the schema version hash. The
