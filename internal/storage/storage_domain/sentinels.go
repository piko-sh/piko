...
var (
	...
	// errInvalidRepository is returned when a storage operation references an invalid
	// repository.
	errInvalidRepository = errors.New("validation failed: invalid repository")

	// errContentTypeEmpty is returned when a storage upload is attempted without specifying
	// a content type.
	errContentTypeEmpty = errors.New("validation failed: content type cannot be empty")

	// errNoObjectsToUpload is returned when a bulk upload is called with an empty object
	// list.
	errNoObjectsToUpload = errors.New("validation failed: no objects to upload")

	// errNoKeysToRemove is returned when a bulk removal is called with an empty key list.
	errNoKeysToRemove = errors.New("validation failed: no keys to remove")

	// errNegativeConcurrency is returned when a concurrency setting is negative.
	errNegativeConcurrency = errors.New("validation failed: concurrency cannot be negative")

	// errInvalidSourceRepo is returned when a copy or move operation references an invalid
	// source repository.
	errInvalidSourceRepo = errors.New("validation failed: invalid source repository")

	// errInvalidDestRepo is returned when a copy or move operation references an invalid
	// destination repository.
	errInvalidDestRepo = errors.New("validation failed: invalid destination repository")

	// errKeyEmpty is returned when a storage operation is attempted with an empty key.
	errKeyEmpty = errors.New("key cannot be empty")

	// errKeyWithCAS is returned when a key is provided but content-addressable storage is
	// enabled, as the key is derived from the content hash.
	errKeyWithCAS = errors.New("key must be empty when UseContentAddressing is true (key is generated from content hash)")

	// ErrSingleflightObjectTooLarge is returned by the singleflight buffering path when the
	// underlying provider yields more bytes than the configured SingleflightMemoryThreshold
	// permits, defending against a provider that reports a small Stat but streams a large
	// payload.
	ErrSingleflightObjectTooLarge = errors.New("storage: singleflight object exceeds memory threshold")

	// ErrDiskObjectTooLarge is returned by the disk provider when an object being read fully
	// into memory exceeds the configured cap.
