...
// Service is the primary port for the storage hexagon, implementing io.Closer,
// storage_domain.Service, and wdk/storage.Service.
//
// It defines the complete set of storage capabilities available to the rest of the
// application. Its method signatures use Data Transfer Objects (DTOs) to encapsulate
// parameters, which allows for future extension without breaking changes.
type Service interface {
	...
	// NewRequest returns a builder for operating on existing objects (Get, Stat, Remove,
	// Hash).
	//
	...
	// SetDefaultProvider sets the provider to be used when no specific provider is named in
	// a call.
	//
	...
	// RegisterTransformer adds a stream transformer for compression or encryption.
	//
	// Takes ctx (context.Context) for cancellation and logging propagation.
	...
	// RegisterDispatcher registers and starts a storage dispatcher for asynchronous
	// operations.
	//
	// Takes ctx (context.Context) for cancellation and logging propagation.
	...
	// FlushDispatcher forces an immediate flush of all queued operations in the dispatcher.
	//
	...
	// Takes params (*storage_dto.PutParams) which contains the object data and metadata. A
	// pointer is used to avoid copying a large struct.
	//
	...
	// StatObject retrieves metadata about an object without downloading its content.
	//
	...
	//
	// Takes providerName (string) which identifies the storage provider to use.
	// Takes params (storage_dto.RemoveManyParams) which specifies the objects to delete.
	//
	...
	// GeneratePresignedUploadURL creates a temporary, signed URL for direct client-side
	// uploads.
	//
	// Takes providerName (string) which identifies the storage provider to use.
	...
	// GeneratePresignedDownloadURL creates a temporary, signed URL for direct client-side
	// downloads.
	//
	// Takes providerName (string) which identifies the storage provider to use.
	...
	//
	// Returns *storage_dto.BatchResult which contains the success or failure status of each
	// object migration.
	...
	// Close gracefully shuts down the storage service and releases all provider resources.
	// It iterates through all registered providers and calls their Close methods.
	//
	// Returns error when any provider fails to close. Errors from individual providers are
	...
	//
	// If PublicFallbackBaseURL is set, returns an absolute URL. Otherwise, returns a
	// relative URL.
	//
...
//
// Each concrete storage implementation (such as S3, GCS, or local disk) must satisfy the
// contract. Implements io.Closer and provides operations for object storage including
// upload, download, copy, and batch operations.
type StorageProviderPort interface {
	...
	// Copy performs a server-side copy within the same repository (e.g., same S3 bucket).
	//
	...
	// CopyToAnotherRepository performs a server-side copy between different repositories.
	//
	...
	// Remove deletes an object. Should be idempotent (not return an error if the object does
	// not exist).
	//
	...
	// Rename atomically moves an object from oldKey to newKey within the same repository.
	// This is used for atomic writes: write to a temp key, then rename to the final key.
	//
	// Takes repo (string) which identifies the repository.
	...
	//   - S3/GCS/R2: Uses Copy + Delete (not truly atomic, but acceptable for temp file
	//     patterns).
	Rename(ctx context.Context, repo string, oldKey, newKey string) error

	// Exists checks if an object exists at the given key.
	//
	// This is more efficient than Stat for existence checks. On disk it uses os.Stat which
	// is already efficient. On S3 it uses HeadObject which is cheaper than GetObject.
	//
	...
	// GetHash returns the hash of an object, preferring efficient metadata lookups over
	// downloading.
	//
	// Takes params (storage_dto.GetParams) which specifies the object to hash.
	//
	...
	// PresignURL generates a signed URL for direct client uploads (e.g., to S3 or GCS).
	//
	// Takes params (storage_dto.PresignParams) which specifies the upload details.
	//
	...
	// PresignDownloadURL generates a signed URL for direct client downloads (e.g., from S3
	// or GCS).
	//
	// Takes params (storage_dto.PresignDownloadParams) which specifies the download details.
	//
	...
	// Close gracefully releases any resources held by the provider (e.g., client
	// connections).
	//
	...
	// backoff logic. If false, the service wraps the provider with RetryableOperation.
	//
	// Returns bool which indicates whether the provider manages its own retries.
	SupportsRetry() bool

	// SupportsCircuitBreaking returns true if the provider has built-in circuit breaker
	// functionality. If false, the service will wrap the provider with
	// CircuitBreakerWrapper.
	//
	// Returns bool which indicates whether the provider supports circuit breaking.
	SupportsCircuitBreaking() bool

	// SupportsRateLimiting returns true if the provider handles rate limiting internally. If
	// false, the service may apply rate limiting at the service layer.
	//
	...
	//
	// Providers can implement native batch APIs or fall back to sequential or concurrent
	// uploads.
	//
	// Takes params (*storage_dto.PutManyParams) which specifies the objects to upload.
	//
	// Returns *storage_dto.BatchResult which contains per-object success or failure details.
	...
	//
	// Providers should use native batch delete APIs where available (for example, S3
	// DeleteObjects).
	//
	// Takes params (storage_dto.RemoveManyParams) which specifies the objects to delete.
	//
	...
	// SupportsBatchOperations returns true if the provider has native, efficient batch APIs.
	// The service layer uses this to decide whether to use batch methods or fall back to
	// loops.
	//
	...
	// (e.g. S3, GCS).
	//
	// Returns bool which is true for native presigned URL support, or false when the service
	// layer must generate HMAC-signed tokens and provide an HTTP upload endpoint (e.g. disk
	// providers).
	SupportsPresignedURLs() bool
}

// StorageDispatcherPort defines the interface for batched, asynchronous storage
// operations, typically for high-throughput, non-critical writes.
