...
var (
	// errCASDeduplicated is a sentinel error used internally to signal that a CAS upload was
	// successfully deduplicated and skipped, which is not a failure condition.
	errCASDeduplicated = errors.New("CAS object already exists, upload skipped")
)

// CASResult holds the results of a successful content-addressing operation. The caller is
// responsible for managing the returned resources.
...
//
// Takes sandbox (safedisk.Sandbox) which provides filesystem access for removal.
...
//
// Takes algorithm (string) which specifies the hashing method to use. Supported values
// are "sha256" and "md5".
//
...
// streamAndComputeHash writes data to a writer while computing its hash at the same time.
//
...
// checkCASDeduplication checks if an object with the given hash already exists in
// storage. Returns errCASDeduplicated if found, nil if not found, or an error if the stat
// operation fails.
//
// Takes provider (StorageProviderPort) which provides access to storage operations.
...
//
// Returns error when the object already exists (errCASDeduplicated) or when the stat
// operation fails.
...
// handleContentAddressing processes content-addressable storage by streaming input to a
// temporary file while computing its hash.
//
// Produces a CASResult holding the new reader, the generated key, and a cleanup function.
// The caller takes ownership of these resources and must call the cleanup function when
// done.
//
// Takes provider (StorageProviderPort) which handles storage operations.
// Takes params (*storage_dto.PutParams) which specifies the input and hash algorithm.
// Takes sandbox (safedisk.Sandbox) which provides sandboxed filesystem access for
// temporary files.
//
// Returns *CASResult which contains the reader, generated key, and cleanup function for
// the caller to manage.
// Returns error when temporary file creation fails, hash computation fails, hash
// verification fails, or the file cannot be rewound.
