...
// Provides zero-allocation rendering for dynamic content by composing writer parts that
// map to quicktemplate's direct write methods. Supports lazy evaluation, type-specific
// rendering paths for strings, numbers, and booleans, and FNV hashing for key generation.
...
// WriterPartType discriminates between part types in a DirectWriter. Each type maps to a
// zero-allocation render path using quicktemplate's direct write methods.
...
const (
	...
	// WriterPartInt represents a signed integer (int, int8, int16, int32, int64). Renders
	// via qw.N().DL() using strconv.AppendInt - zero allocation.
	WriterPartInt

	// WriterPartUint represents an unsigned integer (uint, uint8, uint16, uint32, uint64,
	// uintptr). Renders via qw.N().DUL() using strconv.AppendUint with zero allocation.
	WriterPartUint

	// WriterPartFloat represents a floating-point number (float32, float64). Renders via
	// qw.N().F() using strconv.AppendFloat - zero allocation.
	WriterPartFloat

	// WriterPartBool represents a boolean value. Renders via qw.N().S("true") or
	// qw.N().S("false") - zero allocation.
	WriterPartBool

	// WriterPartAny marks a part containing an arbitrary value that is formatted via
	// Stringer or fmt.Sprint fallback. This is the slow path used when the type is unknown
	// at generator time, and may allocate during formatting.
	WriterPartAny

	// WriterPartEscapeString represents a string that needs HTML escaping at render time.
	//
	// Renders via qw.E().S() using quicktemplate's htmlEscapeWriter with zero allocation.
	// Use for user-provided strings that may contain HTML special characters.
	WriterPartEscapeString

	// WriterPartFNVString represents a dynamic string that should be FNV-32 hashed,
	// rendering as an 8-character hex string for p-key values with problematic characters or
	// unpredictable length.
	WriterPartFNVString

	// WriterPartFNVFloat represents a float value that should be FNV-32 hashed and rendered
	// as an 8-character hex string. This avoids float precision issues in p-key values
	// (e.g., 0.1 + 0.2 = 0.30000000000000004).
	WriterPartFNVFloat

	// WriterPartFNVAny represents an arbitrary value that is FNV-32 hashed and rendered as
	// an 8-character hex string. It is used for p-key values with unknown types at
	// generation time.
	WriterPartFNVAny

	// WriterPartBytes represents a raw byte slice that renders directly via qw.N().SZ()
	// without string conversion. Used for pre-encoded content like base64-encoded action
	// payloads where the bytes are already computed.
	WriterPartBytes

	// WriterPartEscapeBytes represents a byte slice that needs HTML escaping at render time.
	// Used for style values from *Bytes helpers where the content may contain user input
	// with HTML special characters.
	WriterPartEscapeBytes
)

const (
	// directWriterPartsCapacity is the size of the fixed array for common cases. This avoids
	// heap memory use for most calls.
	directWriterPartsCapacity = 8

	// decimalBase is the base for decimal number conversion.
	decimalBase = 10

	// float64BitSize is the bit size for float64 values passed to strconv.AppendFloat.
	float64BitSize = 64

	// defaultByteBufCapacity is the default capacity for pooled byte buffers. 512 bytes
	// accommodates larger CSS class strings and style values without reallocation.
	defaultByteBufCapacity = 512

	// defaultStringBufCapacity is the default capacity for pooled string buffers (64 bytes),
	// sufficient for most String() computations whilst keeping memory overhead low.
	defaultStringBufCapacity = 64

	// estimatedFloatLen is the approximate rendered length for floats and unknown types.
	// Floats are variable length; this is a reasonable upper-bound estimate.
...
// WriterPart represents one segment of a DirectWriter. Only one value field is used based
// on Type.
...
// DirectWriter holds components for lazy, zero-allocation rendering directly to output.
// It implements fmt.Stringer and uses a fixed array to avoid slice allocation for common
// cases (up to 8 parts).
type DirectWriter struct {
	...
	// borrowedBufs tracks byte buffers borrowed from a pool that must be returned when Reset
	// is called. Used by AppendPooledBytes to handle byte slices without extra memory
	// allocation.
	borrowedBufs []*[]byte

	// parts holds the fixed-size buffer for the first 8 parts, avoiding heap allocation.
...
var (
	// directWriterPool is a pool for DirectWriter objects. Wrapped in atomic.Pointer so
	// ResetDirectWriterPool can swap the underlying pool without racing concurrent Get/Put
	// callers.
	...
	// byteBufPool reuses byte-slice buffers for general-purpose byte output. Wrapped in
	// atomic.Pointer so ResetByteBufPool can swap the underlying pool without racing
...
// newDirectWriterPool builds a fresh sync.Pool whose New func returns a zero-valued
// DirectWriter. Used by init and ResetDirectWriterPool.
//
...
// newByteBufPool builds a fresh sync.Pool whose New func returns a pointer to an empty
// []byte with capacity defaultByteBufCapacity. Used by init and ResetByteBufPool.
//
// Returns *sync.Pool which is the freshly constructed pool.
...
//
// The parts array is not zeroed because append() in AppendString, AppendInt, and similar
// methods fully sets all WriterPart fields. This saves about 5% CPU time per profile as
// WriterPart has 8 fields.
//
// The switch statement is unrolled for common buffer counts (0-2) to avoid loop overhead.
// String fields are only cleared when non-empty to avoid unnecessary writes. These
// changes improve performance by 5-18%.
...
//
// At render time, this checks for fmt.Stringer, then falls back to fmt.Sprint. Use this
// for non-primitive types that the generator cannot determine at compile time.
//
...
// AppendPooledBytes adds a pooled byte slice to the output and tracks the buffer pointer
// for later release back to the pool when Reset is called.
//
// Takes bufferPointer (*[]byte) which is the pooled buffer pointer from GetByteBuf.
...
// AppendEscapePooledBytes appends a pooled byte slice that will be HTML-escaped at render
// time to prevent XSS.
//
// Takes bufferPointer (*[]byte) which is the pooled buffer pointer from GetByteBuf.
//
// Use this for style values from *Bytes helpers that may contain user input.
...
// AppendEscapeString adds a string part that will be HTML-escaped at render time.
//
// Takes s (string) which is the user-provided content to escape.
//
// Use this for user-provided strings that may contain characters like <, >, &, ", '. The
// escaping uses zero allocation via quicktemplate's qw.E().S().
...
//
// Use this for p-key values where the string could contain problematic characters or be
// unpredictably long. The output is always an 8-character hex string.
...
//
// Use this for p-key values where float precision could cause issues. The output is
// always an 8-character hex string.
...
// AppendFNVAny adds an arbitrary value that will be FNV-32 hashed at render time.
//
// Takes v (any) which is the value to hash.
//
// Use this for p-key values with unknown types at generation time. The output is always
...
//
// The result is cached after the first call. Use only when a string is needed in Go code.
// For rendering, use WriteTo or write directly via the quicktemplate writer.
...
// and CSS matching where the raw CSS value is needed rather than the HTML-escaped
// version.
//
...
// SingleStringValue returns the string value if the DirectWriter contains exactly one
// WriterPartString part. This is a zero-allocation fast path for common cases like
// `:src="props.ImageUrl"` where the value is a simple string.
//
...
// SingleBytesValue returns the byte slice if the DirectWriter contains exactly one
// WriterPartBytes or WriterPartEscapeBytes part. This is a zero-allocation fast path for
// style/class attributes using *Bytes helpers.
//
...
// RenderedLen returns the total byte length of the rendered output without allocating
// memory. Use this for size estimation instead of len(dw.String()).
//
...
//
// Returns *WriterPart which is the part at the specified index, or nil if the index is
// out of bounds.
...
// WriteTo writes all parts directly to a byte slice, returning the extended slice.
//
// This is the zero-allocation render path for String, Int, Uint, Float, and Bool types.
// WriterPartAny may allocate via fmt.Stringer or fmt.Sprint. WriterPartEscapeString
// performs HTML escaping inline. WriterPartFNV* types produce 8-character hex hashes for
// safe p-key values.
//
// Takes buffer ([]byte) which is the destination slice to append rendered parts to.
//
...
// WriteToRaw writes all parts to a byte slice WITHOUT HTML escaping. Used by StringRaw()
// for premailer and CSS matching where the raw value is needed rather than the
...
//
// The cloned writer is obtained from the pool and populated with copies of all parts. The
// clone is independent of the original - modifying one does not affect the other.
//
// For WriterPartBytes and WriterPartEscapeBytes, a new buffer is allocated from the pool
// and the data is copied. This prevents use-after-free when the original is reset and its
// buffers are returned to the pool.
//
// Returns *DirectWriter which is a new writer with copied parts, or nil if the receiver
// is nil.
...
// resetForArena clears the DirectWriter without returning byte buffers to the global
// pool. This is used when the arena manages buffer lifecycle, so we just clear our
// references without calling PutByteBuf.
//
// The parts array is not zeroed because append in AppendString, AppendInt, and similar
// methods fully sets all WriterPart fields.
...
// GetByteBuf retrieves a byte buffer from the pool for encoding operations. The caller
// must return the buffer using PutByteBuf when finished.
//
...
// ResetByteBufPool atomically swaps in a fresh byte buffer pool for test isolation. Safe
...
//
// It calls Reset to release any borrowed byte buffers before pooling. This stops
// BytesValue slice headers from pointing to memory that may be reused.
//
...
// ResetDirectWriterPool atomically swaps in a fresh DirectWriter pool for test isolation.
...
// intLen returns the number of characters needed to show an int64 as a string without
// allocating memory.
//
...
// appendHTMLEscape appends the string s to buffer with HTML special characters escaped.
...
// appendHTMLEscapeBytes appends b to buffer with HTML special characters escaped,
// handling byte slice input for < > & " '.
//
// Takes buffer ([]byte) which is the buffer to append to.
...
//
// Uses fmt.Stringer if the value has it, otherwise uses fmt.Sprint. This path allocates
// memory but handles cases the generator cannot improve.
//
...
//
// Returns []*DirectWriter which is a new slice with copied writers, or nil if the input
// is nil or empty.
