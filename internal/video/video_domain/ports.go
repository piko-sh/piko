...
// TranscoderPort defines the interface for video transcoding implementations. This port
// follows the hexagonal architecture pattern, allowing different transcoding backends
// (FFmpeg, cloud services, etc.) to be swapped without affecting the domain logic.
type TranscoderPort interface {
	// Transcode converts video from one format or codec to another using the provided
	// specification.
	//
	// Takes input (io.Reader) which provides the source video as a stream.
	// Takes spec (video_dto.TranscodeSpec) which defines the target format and codec
	// settings.
	//
	// Returns io.ReadCloser which streams the transcoded output. The caller must close this
	// when finished.
	...
	// ExtractCapabilities analyses an input video stream and returns metadata including
	// detected codecs, resolution, aspect ratio, duration, bitrate, and supported output
	// formats.
	//
	...
	//
	// Takes codec (string) which is the codec name to check, such as "h264", "h265", "vp9",
	// or "av1".
	//
	// Returns bool which is true if the codec can be used as an output format.
	SupportsCodec(codec string) bool

	// ExtractThumbnail extracts a single frame from a video at the specified position and
	// encodes it as an image.
	//
	// Takes input (io.Reader) which provides the source video data.
	// Takes spec (video_dto.ThumbnailSpec) which defines the extraction parameters including
	// timestamp, dimensions, format, and quality.
	//
	// Returns io.ReadCloser which streams the encoded image data. The caller must close this
	// when finished.
...
// StreamingTranscoderPort extends TranscoderPort with chunked streaming support. This
// enables adaptive bitrate streaming formats like HLS and DASH.
type StreamingTranscoderPort interface {
	TranscoderPort

	// TranscodeHLS generates HLS (HTTP Live Streaming) output with manifest and segments.
	// The implementation creates a master playlist (m3u8), variant playlists for each
	// bitrate, and video segments (.ts files).
	//
	// Takes input (io.Reader) which provides the source video data.
	// Takes spec (video_dto.HLSSpec) which defines the HLS encoding parameters.
	//
	// Returns video_dto.HLSResult which contains all components as streaming readers.
	// Returns error when transcoding fails.
	TranscodeHLS(ctx context.Context, input io.Reader, spec video_dto.HLSSpec) (video_dto.HLSResult, error)
}

// Service provides the main entry point for video operations. It handles transcoding,
// thumbnail extraction, and HLS streaming.
type Service interface {
	// Transcode transcodes a video according to the provided parameters. The parameters are
	// parsed into a TranscodeSpec and validated against service limits.
	//
