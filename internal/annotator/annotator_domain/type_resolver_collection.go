...
// Resolves types for collection operations including indexing, slicing, and iteration
// over arrays, slices, and maps. Validates index expressions, determines element types,
// and enforces type safety for collection access patterns in templates.
...
// tryResolveGetCollectionCall checks if a call expression is r.GetCollection() and
// processes it.
//
...
//
// Returns *ast_domain.GoGeneratorAnnotation which is the annotation if this is a
// GetCollection call, or nil otherwise.
// Returns bool which is true if this was a GetCollection call (whether successful or
...
// extractGetCollectionMemberExpr gets the member expression from a GetCollection call so
// it can be checked further.
//
// Takes n (*ast_domain.CallExpression) which is the call expression to check.
//
// Returns *ast_domain.MemberExpression which is the member expression, or nil if the call
// is not a valid GetCollection call.
...
//
// Returns *ast_domain.MemberExpression which is the member expression with the generic
// type, or nil if the pattern does not match.
...
//
// Takes memberExpr (*ast_domain.MemberExpression) which is the expression to check.
//
// Returns bool which is true if the expression is r.GetCollection, false otherwise.
...
// handleMissingCollectionService creates a diagnostic for missing CollectionService.
//
...
// diagnostic.
//
// Returns *ast_domain.GoGeneratorAnnotation which is always nil after recording the
// diagnostic.
...
// handleCollectionSemanticsError creates a diagnostic for semantics extraction failure.
//
...
// handleCollectionProcessError creates a diagnostic when a collection call cannot be
// processed.
//
// Takes ctx (*AnalysisContext) which provides the analysis state.
// Takes n (*ast_domain.CallExpression) which is the call expression that failed.
...
type GetCollectionSemantics struct {
	// TargetTypeExpression is the AST expression for the target collection element type.
...
// Takes memberExpr (*ast_domain.MemberExpression) which contains the generic type
// parameters.
//
// Returns *GetCollectionSemantics which holds the collection name, target type, and
// options.
// Returns error when the call has no arguments, the collection name is not a string
// literal, or type parameter extraction fails.
...
//
// Takes memberExpr (*ast_domain.MemberExpression) which is the member expression to
// extract from.
//
...
// extractStringLiteralFromPikoAST extracts a string value from a Piko AST expression.
//
...
// parseGetCollectionOptions parses option function calls from GetCollection arguments.
//
// Takes optionArgs ([]ast_domain.Expression) which contains the option expressions to
// parse (everything after the collection name).
//
// Returns any which is the parsed FetchOptions struct containing provider, locale, and
// filter settings.
...
// Takes options (*collection_dto.FetchOptions) which receives the parsed option values.
// Takes scope (map[string]any) which provides variable bindings for evaluation.
//
// Returns error when the expression is not a valid function call or member expression.
...
// applyWithProvider sets the provider name option from a single string argument.
//
// Takes arguments ([]ast_domain.Expression) which must contain exactly one expression
// that evaluates to a string.
// Takes options (*collection_dto.FetchOptions) which receives the provider name.
// Takes scope (map[string]any) which provides variables for expression evaluation.
//
// Returns error when arguments does not contain exactly one element or when the argument
...
//
// Takes arguments ([]ast_domain.Expression) which provides the filter key and value as
// two expressions.
// Takes options (*collection_dto.FetchOptions) which receives the filter.
// Takes scope (map[string]any) which provides variable values for expression evaluation.
//
// Returns error when arguments does not contain exactly two elements or when the first
// argument does not result in a string.
