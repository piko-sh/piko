...
// Analyses template attributes during semantic analysis, validating attribute expressions
// and resolving their types. Handles special attributes like p-if, p-for, p-bind, and
...
const (
	...
	// helpersPrefix is the prefix for client-side helper function calls in templates.
...
// AttributeAnalyser handles the analysis of directives and dynamic attributes on a node.
// It resolves expressions, validates types, and handles context switching for attributes
// from different components (e.g., slotted content).
type AttributeAnalyser struct {
	...
	// pkValidators maps component hashes to their PK validators. Each partial that has its
	// own client script uses a separate validator.
	pkValidators map[string]*PKValidator

	// mainComponentHash is the key for the main component validator. It is used when not
	// inside a partial.
	mainComponentHash string
}

// MarkPartialRendered records that a partial with the given alias has been used in the
// template. This helps avoid false warnings about unused imports.
//
...
// AnalyseNodeAttributes checks and validates all directives and dynamic attributes on a
// template node.
//
// It skips p-for on purpose, as that must be handled separately to set up the scope for
// other attributes.
//
...
// rendering inlining.
// Takes partialInvocationMap (map[string]*ast_domain.PartialInvocationInfo) which tracks
...
type attributeContextResolver struct {
	...
	// partialSelfContext is the context for self-referential analysis; nil means the main
	// ctx is used instead.
	partialSelfContext *AnalysisContext

	// activePInfo holds the partial invocation info for the current parameter being
	// analysed.
	activePInfo *ast_domain.PartialInvocationInfo

	// partialInvocationMap stores partial invocation details, keyed by hashed name.
	partialInvocationMap map[string]*ast_domain.PartialInvocationInfo
}

// forAnnotation selects the right context based on where the annotation came from.
//
// Takes ann (*ast_domain.GoGeneratorAnnotation) which specifies the annotation to find a
// context for.
//
// Returns *AnalysisContext which is the matching context for the annotation. Falls back
// to the default context when the annotation is nil or cannot be matched.
...
//
// Takes d (*ast_domain.Directive) which is the directive to resolve context for.
//
// Returns *AnalysisContext which is the resolved context, or the default context if d is
// nil.
...
//
// Returns *AnalysisContext which is the partial self context if set, otherwise the main
// context.
...
//
// Takes originVC (*annotator_dto.VirtualComponent) which is the component to analyse.
...
// getValidatorForContext returns the correct PK validator for the given partial context.
//
// If the context is inside a partial with its own client script, it returns or creates
// and caches a validator for that partial. If the context is in the main component, it
// returns the main component's validator. If a partial has no client script, it returns
// nil and handlers are treated as Go functions.
//
// Takes activePInfo (*ast_domain.PartialInvocationInfo) which identifies the current
// partial context, or nil if in the main component.
//
// Returns *PKValidator which is the correct validator for the context, or nil if no
// validator applies.
...
// analyseConditionalDirectives checks the conditional directive attributes on a template
// node.
//
...
// analyseOtherDirectives checks the key and context directives on a template node. p-ref
// is validated at the AST layer (not here) since it uses RawExpression rather than
// Expression, and validation must work for both PK and PKC files.
//
// Takes node (*ast_domain.TemplateNode) which is the node to check.
...
// in PK files, since they are only supported in PKC files.
//
// Takes node (*ast_domain.TemplateNode) which is the template node to check for timeline
// directives.
// Takes r (*attributeContextResolver) which resolves the attribute context.
...
//
// Returns ast_domain.Location which is the key directive location if present, the for
// directive location if present, or the node's own location.
...
// analyseEventDirectives processes event directives (p-on:*) for a template node.
//
// It sends each directive to the correct handler based on its expression prefix:
...
// validateEventModifiers checks that all user-facing event modifiers on a directive are
// supported. Returns false if an unknown modifier is found (a diagnostic is emitted and
...
// resolveDefaultEventDirective handles event directives without special modifiers.
//
...
// syntax `action.namespace.Name($form)`, allowing a natural JavaScript-like syntax while
// invoking server-side actions.
//
...
//  1. Setting the modifier to "action" so the emitter treats it as a server action.
//  2. Transforming the callee from a MemberExpr to an Identifier with the action name.
//
...
// addCSRFAndMethodAttributes adds CSRF and HTTP method attributes to the node using the
// resolved action.
//
...
// Takes action (ActionInfoProvider) which is the resolved action, or nil if not found.
// Takes ctx (*AnalysisContext) which receives diagnostics if the action is not found.
...
// transformActionCallExpr transforms the call expression for action calls. It replaces
// the MemberExpr callee with a simple Identifier, resolves arguments, and validates the
// arguments against the action's expected parameter types.
//
// Takes callExpr (*ast_domain.CallExpression) which is the call expression to transform.
...
// validateActionArgs checks that the arguments passed to an action call match the
// expected parameter types. Emits diagnostics for argument count mismatches and, when a
// single object literal is passed to a single struct parameter, validates field names
// against the struct definition.
//
// Takes callExpr (*ast_domain.CallExpression) which contains the arguments to validate.
// Takes actionName (string) which identifies the action for diagnostic messages.
// Takes expectedParams ([]annotator_dto.ActionTypeInfo) which describes the expected
// parameters.
// Takes ctx (*AnalysisContext) which receives any validation diagnostics.
// Takes location (ast_domain.Location) which provides source location for diagnostics.
...
// validateObjectLiteralFields checks that the keys of an object literal match the
// expected struct fields. Emits warnings for unknown fields, missing required fields, and
// type mismatches between provided values and expected field types.
//
// Takes objLit (*ast_domain.ObjectLiteral) which is the object literal to validate.
// Takes actionName (string) which identifies the action for diagnostic messages.
// Takes param (*annotator_dto.ActionTypeInfo) which describes the expected struct fields.
...
// validateFieldType checks whether a value expression's resolved type is compatible with
// the expected field type. Emits a warning if the types are incompatible.
//
// Takes valExpr (ast_domain.Expression) which is the value expression to check.
// Takes fieldInfo (*annotator_dto.ActionFieldInfo) which describes the expected field
// type.
...
// lookupActionCaseInsensitive looks up an action by name, ignoring case. Templates can
// therefore use either camelCase (action.email.contact) or PascalCase
// (action.email.Contact) when referencing actions.
//
...
// Takes actions (map[string]ActionInfoProvider) which provides information about
// available actions.
// Takes cm (*ContextManager) which manages context during analysis.
// Takes mainComponentHash (string) which identifies the main component.
// Takes pkValidator (*PKValidator) which validates PK client-side event handlers for the
// main component, or nil if no client script exists.
//
// Returns *AttributeAnalyser which is the configured analyser ready for use.
...
// hasEventModifier checks whether the given modifier name appears in the slice.
//
// Takes modifiers ([]string) which is the list of event modifiers to search.
...
// extractActionNameFromDirective extracts the action name from an action directive. For
// an expression like "action.namespace.Name($form)", returns "namespace.Name".
//
...
// isActionFieldTypeCompatible checks whether actualType (from a resolved expression) is
// compatible with expectedType (from an action field's GoType). Handles numeric type
// families so that integer literals (resolved as int64) are compatible with any integer
// field type, and float literals (resolved as float64) are compatible with any float
// field type.
//
...
//
// Returns bool which is true if t is an integer type such as int, int8, int16, int32,
// int64, uint, or related types.
...
// isActionFloatType returns true if the type string is a Go floating-point type.
//
// Takes t (string) which is the Go type name to check.
//
...
// createEventContextWithSymbols creates a child context with $event and $form symbols
// defined.
//
// Takes ctx (*AnalysisContext) which provides the parent scope for the new context.
//
// Returns *AnalysisContext which is the child context containing the synthetic $event and
// $form symbols.
...
// newSyntheticTypeInfo creates a synthetic ResolvedTypeInfo for a given type name.
//
