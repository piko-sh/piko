...
// Orchestrates the linking phase where partial invocations are resolved and validated
// against component contracts. Validates props, applies type coercion, handles defaults,
// and assembles the final component structure for code generation.
...
// ComponentLinker is the entry point for the linking stage. It validates the contracts
// between components in a fully expanded AST by walking the tree.
type ComponentLinker struct {
	// typeResolver resolves types when linking documentation references.
	typeResolver *TypeResolver
}

// validPropInfo holds details about a valid prop from a component's Props struct. This
// includes its type, default value, and binding options.
type validPropInfo struct {
	...
	// FactoryFuncName is the name of a function that creates a default value for this
	// property. Empty means no factory function is used.
	FactoryFuncName string

	// QueryParamName is the URL query parameter name used as a fallback when the prop is not
	// set by a parent component. Empty means no query parameter binding.
...
// Link performs component linking on the expanded AST, resolving partial invocations and
// connecting components within the virtual module.
//
...
// for resolving components.
// Takes entryPointPath (string) which specifies the entry point for linking.
//
// Returns *annotator_dto.LinkingResult which contains the linked AST and sorted unique
// invocations.
// Returns []*ast_domain.Diagnostic which contains any warnings or issues found during
// linking.
// Returns error when setting up the root context fails or when AST traversal encounters a
// fatal error.
...
// setupRootContextForLinking creates and fills the root context for the linking stage.
//
...
// Returns error when the entry point path is not found in the graph or the component
// cannot be found.
...
//
// Takes flattenedAST (*ast_domain.TemplateAST) which provides the template structure to
// include in the result.
// Takes combinedCSS (string) which contains the combined CSS styles.
// Takes virtualModule (*annotator_dto.VirtualModule) which holds the virtual module data.
// Takes diagnostics ([]*ast_domain.Diagnostic) which contains any diagnostics to pass
// through.
//
// Returns *annotator_dto.LinkingResult which contains the linking result with empty
// unique invocations.
...
//
// Takes state (*linkingSharedState) which holds the invocation map and the keys that set
// the order.
//
// Returns []*annotator_dto.PartialInvocation which contains the invocations in their
// original order.
...
// populateContextForLinking sets up the analysis context with the symbols needed for
// linking. It defines global symbols, component symbols (state, props, request), and
// local functions, constants, and variables.
//
// Takes ctx (*AnalysisContext) which receives the symbol definitions.
// Takes tr (*TypeResolver) which resolves type information for symbols.
// Takes vc (*annotator_dto.VirtualComponent) which provides the component data.
...
//
// Takes validProps (map[string]validPropInfo) which holds the valid property definitions,
// keyed by name.
//
...
//
// The key combines the partial alias, the invoker's invocation key, and sorted property
// expressions. This means nested partial invocations with the same expressions but
// different parent instances produce distinct keys.
//
// Takes partialAlias (string) which is the base name for the partial.
// Takes props (map[string]ast_domain.PropValue) which contains the property bindings for
// this invocation.
...
// coercePropType changes a string literal to a typed literal when the target type is a
// basic number or boolean type.
//
// Takes sourceExpression (ast_domain.Expression) which is the expression to change.
// Takes destType (goast.Expr) which is the target type.
//
// Returns ast_domain.Expression which is the new typed literal if the string can be
// parsed, or the original expression if it cannot.
...
//
// The function tries to parse the value in this order: boolean literals (true, false),
// nil, integer, float. If none of these match, it falls back to a string literal.
//
...
// beyond the generic "any" type.
//
// Takes ann (*ast_domain.GoGeneratorAnnotation) which is the annotation to check.
//
// Returns bool which is true if the annotation has a specific type that is not "any".
