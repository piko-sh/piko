...
// Analyses p-for loop key expressions to determine an effective key for each loop
// iteration. Handles root and nested loops, building keys that include parent context and
// index variables.
...
// KeyAnalyser determines the key expression for p-for loops. It handles both root and
// nested loops, building keys that include parent context and use index variables for
...
// AnalyseAndSetEffectiveKey determines and sets the EffectiveKeyExpression for a p-for
// loop when the user has not provided an explicit p-key. This is the core method for
// progressive enrichment of keys in nested loops.
//
// Takes node (*ast_domain.TemplateNode) which contains the p-for loop.
// Takes parentEffectiveKey (ast_domain.Expression) which provides the effective key from
// the parent loop.
...
// getForExprAndAnnotation gets the for-in expression and its collection annotation from a
// template node.
//
// Takes node (*ast_domain.TemplateNode) which is the template node to check.
//
// Returns *ast_domain.ForInExpression which is the for-in expression, or nil if the node
// does not have a valid for expression.
// Returns *ast_domain.GoGeneratorAnnotation which is the collection annotation, or nil if
// not found.
...
// ensureIndexVariable creates an index variable if the user did not provide one, or if
// the user explicitly used the blank identifier "_". This is needed because key
// expressions require a real variable name to reference.
//
// Takes forExpr (*ast_domain.ForInExpression) which is the for-in expression that may
// need an index variable.
// Takes collectionAnn (*ast_domain.GoGeneratorAnnotation) which provides the collection
// type for determining the index type.
// Takes ctx (*AnalysisContext) which provides the analysis context.
// Takes depth (int) which indicates the nesting depth for generating unique variable
// names.
...
// fixBlankIdentifierInChildKeys walks all child nodes and replaces any blank identifier
// "_" in their Key expressions with a new unique index variable. This is needed because
// the keyAssigner runs before semantic analysis and uses the original loop variable
// names, including "_" if the user wrote a blank identifier.
//
// Takes node (*ast_domain.TemplateNode) which is the parent node whose children will be
// processed.
// Takes newIndexVar (*ast_domain.Identifier) which is the replacement identifier for
// blank identifiers.
...
// rebuildKeyExpression updates the key expression to include the path from the parent
// node.
//
...
// buildPathParts builds the path parts based on whether this is a nested or root loop.
//
// Takes nodeKey (ast_domain.Expression) which is the key expression for the current node.
// Takes parentEffectiveKey (ast_domain.Expression) which is the effective key from the
// parent loop, or nil for root loops.
// Takes ctx (*AnalysisContext) which provides the analysis context.
//
// Returns []ast_domain.TemplateLiteralPart which contains the constructed path parts.
...
// Takes parentEffectiveKey (ast_domain.Expression) which is the key of the parent node.
//
// Returns []ast_domain.TemplateLiteralPart which contains the parent path parts joined
...
// Takes ctx (*AnalysisContext) which holds the analysis state.
//
// Returns []ast_domain.TemplateLiteralPart which holds the path parts with item variables
// replaced by index variables.
...
// appendIndexToKeyParts appends a dot separator (if needed) and the index variable to key
// parts.
//
// Takes pathParts ([]ast_domain.TemplateLiteralPart) which contains the existing key
// parts to extend.
// Takes indexVariable (*ast_domain.Identifier) which is the index expression to append.
//
// Returns []ast_domain.TemplateLiteralPart which contains the extended key parts with the
// index variable appended.
...
// annotateEffectiveKey adds string type annotations to the given key expression.
//
// Takes effectiveKey (ast_domain.Expression) which is the expression to annotate.
...
// extractPathPartsForNestedLoop extracts all parts from a key expression without
// stripping variables. This is used for nested loops where we want to preserve the
// parent's IndexVariable.
//
// Takes keyExpr (ast_domain.Expression) which is the key expression to extract parts
// from.
//
// Returns []ast_domain.TemplateLiteralPart which contains all template literal parts from
// the expression, or nil when keyExpr is nil.
...
// extractChildPositionFromKey extracts the position part from a child node's key, such as
// ":0." from a template literal expression.
//
// Takes keyExpr (ast_domain.Expression) which is the key expression to check for position
// data.
//
// Returns []ast_domain.TemplateLiteralPart which contains the position parts found, or
// nil if keyExpr is nil or not a template literal.
...
// extractPathPartsFromKey extracts the hierarchical path parts from an existing key
// expression.
//
// This strips out the loop variable that keyAssigner added, keeping only the static
// hierarchical path. For a node with p-for, node.Key is "path + loopVar", we want just
// "path".
//
// Takes keyExpr (ast_domain.Expression) which is the key expression to extract path parts
// from.
//
// Returns []ast_domain.TemplateLiteralPart which contains the static path parts without
// the loop variable suffix.
...
// replaceItemVariablesWithIndexVariables filters out ItemVariable references from path
// parts, keeping only IndexVariables and literal parts. This is critical for nested loops
// where the child's node.Key (set by keyAssigner) contains references to parent
// ItemVariables (like "row"), but we need to use only the IndexVariables (like
// "__pikoLoopIdx") that the semantic analyser created.
//
// Takes parts ([]ast_domain.TemplateLiteralPart) which contains the path parts to filter.
// Takes ctx (*AnalysisContext) which provides access to the symbol table.
//
// Returns []ast_domain.TemplateLiteralPart which contains only literal parts and index
// variable references.
...
// buildExpressionFromParts builds a key expression from template literal parts. This
// mirrors the logic from transformations.go's buildExpressionFromParts.
//
// Takes parts ([]ast_domain.TemplateLiteralPart) which contains the template literal
// segments to combine.
// Takes baseLocation (ast_domain.Location) which specifies the source location for the
// resulting expression.
//
// Returns ast_domain.Expression which is the combined expression: either a StringLiteral,
// the single dynamic expression, or a TemplateLiteral.
...
// fixBlankIdentifierInNodeKey replaces blank identifiers in a node's key and processes
// child nodes recursively.
//
// When the node is nil, returns immediately. When the node has a DirFor directive, does
// not process children as they have their own scope.
//
...
// replaceBlankIdentifierInExpr replaces blank identifiers "_" in an expression with a new
// index variable.
//
// Takes expression (ast_domain.Expression) which is the expression to check and update.
// Takes newIndexVar (*ast_domain.Identifier) which is the variable to use instead of the
// blank identifier.
//
// Returns ast_domain.Expression which is the updated expression, or nil if expression is
// nil.
...
// replaceBlankIdentifierInIdentifier checks if an identifier is blank and returns a
// replacement variable if so.
//
...
// replacement.
//
// Returns ast_domain.Expression which is a clone of newIndexVar if identifier is blank
// ("_"), or the original identifier if not.
...
// replaceBlankIdentifierInTemplateLiteral replaces blank identifiers in a template
// literal node.
//
// Takes literal (*ast_domain.TemplateLiteral) which is the template to check.
// Takes newIndexVar (*ast_domain.Identifier) which is the variable to use in place of
// blank identifiers.
//
// Returns ast_domain.Expression which is the original literal if no change was needed, or
// a new literal with blank identifiers replaced.
...
// replaceBlankIdentifierInParts goes through all parts of a template literal and replaces
// any blank identifiers with a new variable.
//
// Takes parts ([]ast_domain.TemplateLiteralPart) which contains the template literal
// parts to check.
// Takes newIndexVar (*ast_domain.Identifier) which is the new variable to use in place of
// blank identifiers.
//
...
//
// Returns *ast_domain.Identifier which is a copy of the identifier, or nil if identifier
// is nil.
...
// Takes typeInfo (*ast_domain.ResolvedTypeInfo) which provides type details for the
// variable.
//
// Returns *ast_domain.Identifier which is the identifier with annotations set for code
// generation.
...
// needsDotSeparator checks whether a dot is needed before the index variable in a
// template path.
//
// Takes pathParts ([]ast_domain.TemplateLiteralPart) which contains the template path
// parts to check.
//
...
// literal parts.
//
// Takes parts ([]ast_domain.TemplateLiteralPart) which contains the parts to search
// through.
//
// Returns []ast_domain.TemplateLiteralPart which is the position part if found, or nil if
// no position part exists.
...
// tryExtractPositionFromPart checks if the part at the given index has a position marker
// and extracts it.
//
// Takes parts ([]ast_domain.TemplateLiteralPart) which contains the template literal
// parts to check.
...
//
// Returns []ast_domain.TemplateLiteralPart which contains any position parts found, or
// nil if no position can be extracted.
...
// extractPositionFromLiteral gets the position suffix from a literal string. The position
// suffix starts from the last colon in the string.
//
// Takes lit (string) which is the literal to extract the position from.
//
// Returns []ast_domain.TemplateLiteralPart which holds the position suffix as a single
// literal part, or nil if no colon is found.
