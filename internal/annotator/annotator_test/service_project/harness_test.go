...
type ProjectTestSpec struct {
	...
	PageAssertions      map[string]PageAssertion   `json:"pageAssertions"`
...
func runProjectTestCase(t *testing.T, tc testCase) {
	...
	entryPoints := make([]annotator_dto.EntryPoint, len(spec.EntryPoints))
	for i, ep := range spec.EntryPoints {

		pikoPath := filepath.ToSlash(filepath.Join(moduleName, ep.Path))
		entryPoints[i] = annotator_dto.EntryPoint{
			Path:   pikoPath,
			IsPage: ep.IsPage,
		}
	}

	projectResult, compilationLogs, err := service.AnnotateProject(context.Background(), entryPoints, nil)
	...
	require.NotNil(t, projectResult, "ProjectAnnotationResult should not be nil, even on error")

	if spec.ExpectedError != nil {
		require.Error(t, err)
		assert.Contains(t, err.Error(), *spec.ExpectedError, "The error message did not contain the expected text")
	} else {
		allSource := getAllSource(t, absSrcDir)
		require.NoError(t, err, "AnnotateProject failed unexpectedly:\n%s", annotator_domain.FormatAllDiagnostics(projectResult.AllDiagnostics, allSource))
	}

	allDiagnostics := projectResult.AllDiagnostics
	assert.Len(t, allDiagnostics, spec.ExpectedDiagnostics, "Mismatch in total number of expected project diagnostics")
	for i, diagCheck := range spec.Diagnostics {
		if i < len(allDiagnostics) {
			actualDiag := allDiagnostics[i]
			assert.Equal(t, diagCheck.Severity, actualDiag.Severity.String(), "Project diagnostic #%d severity mismatch", i+1)
			assert.Contains(t, actualDiag.Message, diagCheck.MessageContains, "Project diagnostic #%d message mismatch", i+1)
			if diagCheck.OnLine != nil {
				assert.Equal(t, *diagCheck.OnLine, actualDiag.Location.Line, "Project diagnostic #%d line number mismatch", i+1)
			}
		} else {
			t.Errorf("Expected project diagnostic #%d ('%s') but only found %d diagnostics", i+1, diagCheck.MessageContains, len(allDiagnostics))
		}
	}
	...
	require.NotNil(t, projectResult.VirtualModule, "Project result must include the VirtualModule")
	require.NotNil(t, projectResult.VirtualModule.Graph, "VirtualModule must include the ComponentGraph")

	for entryPath, pageAssertion := range spec.PageAssertions {
		t.Run(fmt.Sprintf("PageAssertions for %s", entryPath), func(t *testing.T) {

			absEntryPath := filepath.Join(absSrcDir, entryPath)

			hashedName, ok := projectResult.VirtualModule.Graph.PathToHashedName[absEntryPath]
			require.True(t, ok, "Could not find hashed name for entry point '%s'", entryPath)

			pageResult, ok := projectResult.ComponentResults[hashedName]
			require.True(t, ok, "No annotation result found for entry point '%s' (hash: %s)", entryPath, hashedName)
			require.NotNil(t, pageResult.AnnotatedAST, "AnnotatedAST for page '%s' should not be nil", entryPath)

			for _, substring := range pageAssertion.ExpectedCombinedCSSContains {
				assert.Contains(t, pageResult.StyleBlock, substring, "Final CSS for page '%s' is missing expected content", entryPath)
			}

			for _, nodeAssert := range pageAssertion.AssertNodes {
				t.Run(nodeAssert.Description, func(t *testing.T) {
					assertNode(t, pageResult.AnnotatedAST, nodeAssert, absSrcDir)
				})
			}

			generateAndCheckGoldenFiles(t, tc, pageResult, entryPath)
		})
	}
}
...
func assertNode(t *testing.T, annotatedAST *ast_domain.TemplateAST, assertion NodeAssertion, baseDir string) {
	...
	if details.TextContent != "" {
		assert.Equal(t, details.TextContent, targetNode.RawText(context.Background()), "TextContent mismatch")
	}
...
func assertOnSubExpression(t *testing.T, node *ast_domain.TemplateNode, assertions []SubExpressionAssertion) {
	...
	for _, assertion := range assertions {
		...
		for _, rootExpr := range allExpressionsInSubtree {
			...
			foundSubExpr = findExpressionNode(rootExpr, assertion.RawExpression)
			if foundSubExpr != nil {
				break
			}
		}
...
func generateAndCheckGoldenFiles(t *testing.T, tc testCase, annotationResult *annotator_dto.AnnotationResult, entryPointPath string) {
	...
	baseName := strings.TrimSuffix(filepath.Base(entryPointPath), filepath.Ext(entryPointPath))
	goldenDir := filepath.Join(tc.Path, "golden")
	goldenASTPath := filepath.Join(goldenDir, baseName+".ast")
	goldenGoPath := filepath.Join(goldenDir, baseName+".go")
	goldenCSSPath := filepath.Join(goldenDir, baseName+".css")
	...
	require.NoError(t, err, "Failed to read golden file '%s'. Run with -update.", goldenASTPath)
	assert.Equal(t, string(expectedASTDump), actualASTDump, "AST dump mismatch for %s", entryPointPath)

	expectedGo, err := os.ReadFile(goldenGoPath)
	require.NoError(t, err, "Failed to read golden file '%s'. Run with -update.", goldenGoPath)
	assert.Equal(t, string(expectedGo), actualASTCompile, "Compiled Go AST mismatch for %s", entryPointPath)

	if fileExists(goldenCSSPath) {
		expectedCSS, err := os.ReadFile(goldenCSSPath)
		require.NoError(t, err)
		assert.Equal(t, string(expectedCSS), annotationResult.StyleBlock, "CSS output mismatch for %s", entryPointPath)
	}
}
...
func findExpressionNode(root ast_domain.Expression, rawTarget string) ast_domain.Expression {
	var found ast_domain.Expression
	normalisedTarget := normaliseExprString(rawTarget)
	walkExpression(root, func(current ast_domain.Expression) bool {
		if normaliseExprString(current.String()) == normalisedTarget {
			found = current
			return false
		}
		return true
...
func walkExpression(expression ast_domain.Expression, visitor func(ast_domain.Expression) bool) {
	if expression == nil || !visitor(expression) {
		return
	}
	switch n := expression.(type) {
	case *ast_domain.MemberExpression:
		walkExpression(n.Base, visitor)
		walkExpression(n.Property, visitor)
	case *ast_domain.IndexExpression:
		walkExpression(n.Base, visitor)
		walkExpression(n.Index, visitor)
	case *ast_domain.UnaryExpression:
		walkExpression(n.Right, visitor)
	case *ast_domain.BinaryExpression:
		walkExpression(n.Left, visitor)
		walkExpression(n.Right, visitor)
	case *ast_domain.CallExpression:
		walkExpression(n.Callee, visitor)
		for _, argument := range n.Args {
			walkExpression(argument, visitor)
		}
	case *ast_domain.TernaryExpression:
		walkExpression(n.Condition, visitor)
		walkExpression(n.Consequent, visitor)
		walkExpression(n.Alternate, visitor)
	...
	case *ast_domain.ArrayLiteral:
		for _, element := range n.Elements {
			walkExpression(element, visitor)
		}
	}
}
