...
// It simulates an invocation of a component that does not have a Props struct, yet
// is passed several props. This is the ideal input to test the ComponentLinker's
// handling of unknown props and its ability to generate the correct warnings.
func CreateExpansionResult(t *testing.T, vm *annotator_dto.VirtualModule) *annotator_dto.ExpansionResult {
	...
	iconHash := findHash("partials/icon.pk")

	iconSourcePath := vm.ComponentsByHash[iconHash].Source.SourcePath
		...
		RootNodes: []*ast_domain.TemplateNode{
			{
				...
				GoAnnotations: &ast_domain.GoGeneratorAnnotation{
					OriginalPackageAlias: &iconHash,
					OriginalSourcePath:   &iconSourcePath,
					PartialInfo: &ast_domain.PartialInvocationInfo{
						InvocationKey:       "potential-key-1",
						PartialAlias:        "icon",
						PartialPackageName:  iconHash,
						InvokerPackageAlias: mainHash,
						PassedProps: map[string]ast_domain.PropValue{
							"name": {Expression: &ast_domain.StringLiteral{Value: "arrow-down"}},
							"size": {Expression: &ast_domain.IntegerLiteral{Value: 24}},
						},
					},
				},
			},
