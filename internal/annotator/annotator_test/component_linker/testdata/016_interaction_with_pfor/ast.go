...
func CreateExpansionResult(t *testing.T, vm *annotator_dto.VirtualModule) *annotator_dto.ExpansionResult {
	...
	rowHash := findHash("partials/user-row.pk")
	mainSourcePath := vm.ComponentsByHash[mainHash].Source.SourcePath
	rowSourcePath := vm.ComponentsByHash[rowHash].Source.SourcePath
		...
		RootNodes: []*ast_domain.TemplateNode{
			{
				...
				GoAnnotations: &ast_domain.GoGeneratorAnnotation{
					OriginalPackageAlias: &mainHash,
					OriginalSourcePath:   &mainSourcePath,
				},
				DirFor: &ast_domain.Directive{
					...
					Expression: &ast_domain.ForInExpression{
						...
						Collection:    &ast_domain.MemberExpression{Base: &ast_domain.Identifier{Name: "state"}, Property: &ast_domain.Identifier{Name: "Users"}},
					},
				},
				Children: []*ast_domain.TemplateNode{
					{
						NodeType: ast_domain.NodeElement, TagName: "tr",
						GoAnnotations: &ast_domain.GoGeneratorAnnotation{
							OriginalPackageAlias: &rowHash,
							OriginalSourcePath:   &rowSourcePath,
							PartialInfo: &ast_domain.PartialInvocationInfo{
								InvocationKey:       "potential-key-1",
								PartialAlias:        "user-row",
								PartialPackageName:  rowHash,
								InvokerPackageAlias: mainHash,
								PassedProps: map[string]ast_domain.PropValue{
									"user":  {Expression: &ast_domain.Identifier{Name: "user"}},
									"index": {Expression: &ast_domain.Identifier{Name: "i"}},
								},
							},
						},
					},
