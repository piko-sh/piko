...
// TokenBucketState holds the persistent state of a token bucket rate limiter. This state
// can be stored externally (such as in Redis) and shared across instances for distributed
// rate limiting.
...
// TokenBucketStorePort is the driven port for token bucket state storage. Implementations
// must be safe for concurrent access and provide atomic read-modify-write operations for
// token bucket state.
type TokenBucketStorePort interface {
	// TryTake atomically attempts to take n tokens from the bucket. It first refills based
	// on elapsed time, then attempts to deduct tokens.
	//
	...
	// WaitDuration returns the estimated time until n tokens become available. This can be
	// used for backoff calculations.
	//
	// Takes key (string) which identifies the rate limit bucket.
...
// CounterStorePort is the driven port for fixed-window counter storage. Implementations
// must provide atomic increment operations with TTL support.
type CounterStorePort interface {
	// IncrementAndGet atomically increments the counter for key by delta. The TTL is set
	// only when the counter is first created, implementing a fixed-window strategy where the
...
// RateLimiterInspector provides read-only access to rate limiter state for monitoring. It
// is implemented by the Limiter and consumed by the monitoring gRPC service.
type RateLimiterInspector interface {
	// GetStatus returns the current status of the rate limiter, including store types, fail
	// policy, and aggregate counters.
	//
