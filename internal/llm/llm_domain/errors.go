...
const (
	// MaxRetryAfterDuration caps the Retry-After value honoured by the retry executor so a
	// hostile or misbehaving server cannot push the client into an excessively long sleep.
	MaxRetryAfterDuration = 5 * time.Minute
)

// RetryableError is an optional interface that errors can implement to indicate whether
// they should be retried. When present, this takes priority over string-based error
// classification in the retry executor.
type RetryableError interface {
	error

	// IsRetryable reports whether this error represents a transient failure that should be
	// retried.
	//
...
var (
	// ErrProviderNotFound is returned when the requested LLM provider is not registered.
	...
	// ErrProviderAlreadyExists indicates a provider with that name is already registered.
	ErrProviderAlreadyExists = errors.New("llm provider already exists")

	// ErrStreamingNotSupported is returned when the LLM provider does not support streaming
	// responses.
	...
	// ErrStructuredOutputNotSupported indicates the provider does not support structured
	// output.
	ErrStructuredOutputNotSupported = errors.New("llm provider does not support structured output")

	// ErrPenaltiesNotSupported indicates the provider does not support frequency/presence
	// penalties.
	ErrPenaltiesNotSupported = errors.New("llm provider does not support frequency/presence penalties")

	// ErrSeedNotSupported indicates the provider does not support the seed parameter.
	ErrSeedNotSupported = errors.New("llm provider does not support seed")

	// ErrParallelToolCallsNotSupported indicates the provider does not support parallel tool
	// calls.
	ErrParallelToolCallsNotSupported = errors.New("llm provider does not support parallel tool calls")

	// ErrMessageNameNotSupported indicates the provider does not support the Name field on
	// messages.
	...
	// ErrInvalidTemperature is returned when the temperature value is not between 0 and 2.
	...
	// ErrMaxCostExceeded is returned when the estimated cost is higher than the limit
	// allowed for a single request.
	ErrMaxCostExceeded = errors.New("estimated cost exceeds per-request limit")

	// ErrUnknownModelPrice indicates no pricing information is available for the model.
	ErrUnknownModelPrice = errors.New("no pricing information for model")

	// ErrProviderOverloaded indicates the provider is temporarily overloaded.
	...
	// ErrVectorStoreNotConfigured is returned when vector operations are attempted without a
	// configured vector store.
	ErrVectorStoreNotConfigured = errors.New("vector store is not configured")

	// retryableStatusCodes contains HTTP status codes that indicate transient failures worth
	// retrying.
...
// ProviderError represents an error returned by an LLM provider, carrying the provider
// name, HTTP status code, and a descriptive message. It implements both error and
// RetryableError.
type ProviderError struct {
	...
	// RetryAfter is the duration the server has hinted the client should wait before
	// retrying, parsed from the Retry-After HTTP response header. A zero value indicates no
	// hint was provided.
...
// IsRetryable reports whether this provider error represents a transient failure that
// should be retried.
//
