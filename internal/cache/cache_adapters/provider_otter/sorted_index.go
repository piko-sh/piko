...
const (
	// btreeDegree is the branching factor for B-tree nodes. Degree 32 fits optimally in CPU
	// cache lines (L1/L2).
	btreeDegree = 32

	// smallFilterThreshold is the size below which we convert slice->map for filtering. For
	// small sets, map creation overhead is acceptable for O(1) lookup benefit.
...
// first, then by key for stable sorting.
//
...
// SortedIndex maintains keys in sorted order by a field value using a B-tree. Supports
// numeric, string, and any comparable types.
//
// Thread-safe for concurrent read/write access. Uses Google's B-tree implementation for
// O(log n) operations.
...
// Add inserts or updates a key with its sortable value. If the key exists, it is removed
// and re-inserted at the correct position.
//
...
// addUnsafe inserts or updates without acquiring the lock. Caller must hold the write
// lock.
//
...
// KeysFiltered returns keys from a subset, in sorted order. Only keys present in the
...
// KeysFilteredSlice returns keys from a subset in sorted order, including only keys
// present in the filter slice. More efficient than KeysFiltered when you have a slice of
// keys.
//
...
// KeysLessThanOrEqual returns keys with values less than or equal to the threshold.
//
// Takes threshold (any) which specifies the upper bound for comparison.
// Takes ascending (bool) which determines the order of returned keys.
//
...
// appendIfInRange checks if an item falls within a range and adds its key to the result.
//
...
// NewSortedIndex creates a new empty sorted index backed by a B-tree. The B-tree uses
// degree 32 for optimal cache performance.
//
...
// compareValues compares two values for ordering. Supports int, int64, float64, string,
...
//
// Returns int which is negative if a < b, zero if a == b, or positive if a > b.
