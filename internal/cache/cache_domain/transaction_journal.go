...
var (
	_ TransactionCache[string, any] = (*transactionJournal[string, any])(nil)
)

// journalEntry records the pre-mutation state of a single cache key so the mutation can
// be undone on Rollback.
//
// Tags are not captured because the cache interface does not expose per-entry tag
// retrieval. This is acceptable because DAL consumers manage their own tag indexes
...
type journalEntry[K comparable, V any] struct {
	...
	// existed is true when the key was present before the mutation. When false, rollback
	// removes the key rather than restoring a value.
	existed bool

	// expiresAtNano is the Unix timestamp in nanoseconds when the entry was due to expire.
	// Zero means no TTL was set, and on rollback the remaining TTL is computed from this
	// value.
	expiresAtNano int64
}

// transactionJournal wraps any ProviderPort and adds undo-journal-based rollback.
// Mutations are applied immediately to the inner cache but recorded so they can be
// reversed.
//
// Synchronous loader methods (Get, BulkGet) snapshot keys before delegating, so
// loader-triggered population is rolled back correctly. Asynchronous methods (Refresh,
// BulkRefresh) cannot be journalled because the cache write happens after return; callers
// should avoid these within transactions if rollback correctness is required.
//
// NOT safe for concurrent use. Designed to be used within a single goroutine that holds
// an external lock (e.g. the DAL mutex).
type transactionJournal[K comparable, V any] struct {
	...
	// snapshotted tracks which keys already have a journal entry so that only the first
	// mutation per key is recorded.
	snapshotted map[K]struct{}

	// journal records pre-mutation state for each key, in mutation order.
	journal []journalEntry[K, V]

	// finalised is true after Commit or Rollback has been called. Subsequent mutations or
	// double-finalise attempts return an error.
	finalised bool
}

// newTransactionJournal creates a transaction journal wrapping the given cache.
//
// Takes inner (ProviderPort[K, V]) which is the underlying cache that receives all
// operations.
//
// Returns *transactionJournal[K, V] which wraps inner with undo journalling.
...
// snapshotKey records the current state of key before a mutation. Only the first mutation
// per key is recorded; subsequent mutations to the same key are skipped because rollback
// only needs the original value.
//
...
// Rollback replays the undo journal, restoring every mutated key to its pre-transaction
// state. Journal entries are replayed in reverse order so that the first mutation's
// original value is restored last (though with the snapshotted-once design each key
// appears at most once).
//
// When the original entry had a TTL, the remaining duration is computed from the
// snapshotted expiry time and restored via SetWithTTL. If the TTL has already elapsed the
// entry is not restored (it would have expired anyway).
//
// All entries are attempted even when individual restores fail; errors are collected and
// returned via errors.Join so that a single failure does not leave remaining entries
// unreplayed.
//
// Returns error when the transaction was already finalised or any restore operation
// fails.
func (t *transactionJournal[K, V]) Rollback(ctx context.Context) error {
	...
	var errs []error
	for _, entry := range slices.Backward(t.journal) {
		if entry.existed {
...
// restoreEntry puts back a single journal entry, computing the remaining TTL if one was
// set and invalidating the key when the TTL has already elapsed.
//
// Takes entry (journalEntry[K, V]) which holds the pre-mutation state to restore.
// Takes nowNano (int64) which is the current Unix time in nanoseconds, used to compute
// remaining TTL.
//
...
// Set stores a value, snapshotting the key's prior state for rollback.
//
...
// SetWithTTL stores a value with TTL, snapshotting the key's prior state.
//
// Takes key (K) which identifies the cache entry.
// Takes value (V) which is the data to store.
...
// Invalidate removes a key, snapshotting the key's prior state for rollback.
//
...
// Compute computes a new value atomically for a key, snapshotting the key's prior state.
//
// Takes key (K) which identifies the cache entry to update.
// Takes fn (func) which receives the current value and presence flag, returning the new
// value and action.
//
...
// ComputeIfAbsent computes a value if absent, snapshotting the key's prior state.
//
// Takes key (K) which identifies the entry to look up or create.
// Takes fn (func() V) which generates the value if the key is absent.
//
...
// ComputeIfPresent updates a key if present, snapshotting the key's prior state.
//
// Takes key (K) which identifies the entry to update.
// Takes fn (func) which computes the new value from the existing value.
//
...
// ComputeWithTTL atomically computes a value with TTL, snapshotting the key's prior
// state.
//
// Takes key (K) which identifies the cache entry to update.
// Takes fn (func) which receives the current value and presence flag, returning a
// ComputeResult with value, action, and TTL.
//
// Returns V which is the resulting value after the operation.
// Returns bool which indicates whether a value is now present.
// Returns error when snapshotting or computation fails.
...
// BulkSet stores multiple values, snapshotting each key's prior state.
//
...
// InvalidateByTags is not supported within a transaction because the Cache interface does
// not expose tag-to-key resolution, making it impossible to journal individual key
...
// InvalidateAll is not supported within a transaction because bulk invalidation cannot be
// efficiently journalled at the key level.
//
...
// GetIfPresent returns the value for a key if it is present and not expired, delegating
// directly to the inner cache.
//
...
// Get returns the value for key, using the loader to populate the cache on a miss. The
// key is snapshotted before delegating so that loader-triggered population can be undone
// on rollback.
//
...
// BulkGet returns values for all keys, using the bulk loader to populate missing entries.
// All keys are snapshotted before delegating so that loader-triggered population can be
// undone on rollback.
//
...
// BulkRefresh triggers asynchronous refresh for the given keys, bypassing the undo
// journal because the actual cache writes happen after return.
//
// Takes keys ([]K) which specifies the cache keys to refresh.
// Takes bulkLoader (BulkLoader[K, V]) which fetches new values in bulk.
...
// Refresh triggers an asynchronous refresh for a single key. Because the actual cache
// write happens asynchronously, it cannot be reliably journalled; callers should avoid
// Refresh within transactions if rollback correctness is required.
//
// Takes key (K) which identifies the cache entry to refresh.
// Takes loader (Loader[K, V]) which fetches the new value.
//
// Returns <-chan LoadResult[V] which receives the result upon completion.
...
// All returns an iterator over all key-value pairs in the cache, delegating to the inner
// cache.
//
...
// Keys returns an iterator over all keys in the cache, delegating to the inner cache.
//
...
// Values returns an iterator over all values in the cache, delegating to the inner cache.
//
...
// GetEntry returns a snapshot of the entry including metadata, delegating to the inner
// cache.
//
...
// ProbeEntry returns a snapshot of the entry without resetting its access timer,
...
// EstimatedSize returns the approximate number of entries in the cache, delegating to the
// inner cache.
//
...
// Stats returns a snapshot of the cache's performance statistics, delegating to the inner
// cache.
//
// Returns Stats which contains hit/miss counts and other performance metrics.
func (t *transactionJournal[K, V]) Stats() cache_dto.Stats {
	return t.inner.Stats()
}

// GetMaximum returns the current maximum capacity of the cache, delegating to the inner
// cache.
//
...
// SetMaximum changes the maximum capacity of the cache, delegating to the inner cache.
//
...
// WeightedSize returns the total weight of all items in the cache, delegating to the
// inner cache.
//
...
// SetExpiresAfter sets or changes the expiry time for a key, delegating to the inner
// cache.
//
...
// SetRefreshableAfter sets or overrides the refresh time for a key, delegating to the
// inner cache.
//
// Takes key (K) which identifies the cache entry to update.
...
// Search performs full-text search across indexed fields, delegating to the inner cache.
//
...
//
// Returns SearchResult[K, V] which contains the matched entries.
...
// Query performs structured filtering and pagination without full-text search, delegating
// to the inner cache.
//
...
// SupportsSearch returns true if the inner cache supports search and query operations.
//
...
// GetSchema returns the search schema for the inner cache, or nil if search is not
// configured.
//
// Returns *SearchSchema which describes the indexed fields, or nil when search is not
// configured.
func (t *transactionJournal[K, V]) GetSchema() *cache_dto.SearchSchema {
	return t.inner.GetSchema()
}

// Close is a no-op on the transaction journal since the transaction wrapper does not own
