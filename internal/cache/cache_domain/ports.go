...
type Cache[K comparable, V any] interface {
	// GetIfPresent returns the value for a key if it is present and not expired.
	//
	...
	// Returns error when the operation fails (e.g. network error for distributed caches).
	GetIfPresent(ctx context.Context, key K) (V, bool, error)

	// Get returns the value for a key, calling the loader if the key is missing. Provides
	// built-in protection against cache stampede (thundering herd).
	//
	...
	// Set stores a value with the given key, replacing any existing entry. It accepts
	...
	//
	// This overrides the cache's default expiration policy. The operation is atomic and
	// performs better than calling Set followed by SetExpiresAfter for remote caches.
	//
	...
	// Computes a new value atomically for the given key based on the current value. The
	// compute function executes while holding a lock on the key.
	//
	// When the context is already cancelled or has exceeded its deadline, returns the
	// context's error without performing any work.
	//
	...
	// ComputeIfAbsent atomically computes and stores a value if the key is absent. The
	// compute function is only called if the key is not present.
	//
	// When the context is already cancelled or has exceeded its deadline, returns the
	// context's error without performing any work.
	//
	...
	// ComputeIfPresent atomically computes a new value for the key if it is present. The
	// compute function is only called if the key exists.
	//
	// When the context is already cancelled or has exceeded its deadline, returns the
	// context's error without performing any work.
	//
	...
	// computes the new value from the existing value.
	//
	...
	// ComputeWithTTL atomically computes a new value for the given key with per-call TTL
	// control. The compute function executes while holding a lock on the key and can specify
	// a custom TTL for the entry.
	//
	// When the context is already cancelled or has exceeded its deadline, returns the
	// context's error without performing any work.
	//
	...
	//
	// For remote caches (e.g., Redis), this uses a pipeline or MSET for optimal performance.
	// For in-memory caches, this typically loops over items. All items share the same tags.
	//
	...
	//
	// Returns int which is the number of keys that were invalidated. The operation's
	// atomicity depends on the underlying provider.
	// Returns error when the operation fails.
	...
	//
	// The old values are served until the refresh completes. This is a fire-and-forget
	// operation.
	//
	...
	//
	// Returns <-chan cache_dto.LoadResult[V] which receives the result upon completion.
	Refresh(ctx context.Context, key K, loader cache_dto.Loader[K, V]) <-chan cache_dto.LoadResult[V]

	// All returns an iterator over all key-value pairs in the cache. The snapshot may not
	// include changes made while iterating.
	//
	...
	// GetEntry returns a snapshot of the entry, including metadata like weight and
	// expiration. This operation resets the entry's access timer for TTL calculations.
	//
	// When the context is already cancelled or has exceeded its deadline, returns the
	// context's error without performing any work.
	//
	...
	// ProbeEntry returns a snapshot of the entry without changing its access patterns. This
	// does not reset the entry's access timer, making it useful for monitoring.
	//
	// When the context is already cancelled or has exceeded its deadline, returns the
	// context's error without performing any work.
	//
	...
	// this may cause items to be removed at once.
	//
	// Takes size (uint64) which is the new maximum number of items.
	SetMaximum(size uint64)

	// WeightedSize returns the total weight of all items in the cache. If the cache does not
	// use weights, this may return the same value as EstimatedSize.
	//
	// Returns uint64 which is the current total weight.
	WeightedSize() uint64

	// SetExpiresAfter sets or changes the expiry time for a key that already exists.
	//
	// When the context is already cancelled or has exceeded its deadline, returns the
	// context's error without performing any work.
	//
	...
	// SetRefreshableAfter manually sets or overrides the refresh time for an existing key.
	//
	// When the context is already cancelled or has exceeded its deadline, returns the
	// context's error without performing any work.
	//
	...
	// Search performs full-text search across indexed TEXT fields. This enables finding
	// cached entries by content rather than just by key.
	//
	// The query string is tokenised and matched against TEXT fields defined in the
	// SearchSchema. Results are ranked by relevance score.
	//
	// Takes ctx (context.Context) for cancellation and timeout.
	...
	// This is for finding entries by exact field values or ranges.
	//
	// Takes ctx (context.Context) for cancellation and timeout.
	...
	// SupportsSearch returns true if this cache supports search and query operations. Use
	// this to check capability before calling Search or Query.
	//
	// Returns bool which is true when search operations are available.
	SupportsSearch() bool

	// GetSchema returns the search schema for this cache, or nil if search is not
	// configured. The schema describes which fields are searchable.
	//
	// Returns *SearchSchema which describes searchable fields, or nil.
	GetSchema() *cache_dto.SearchSchema
}

// ProviderPort defines the driven port for cache adapters in a hexagonal architecture.
// Each concrete implementation (Otter, Redis, etc.) must satisfy the contract defined
// here.
type ProviderPort[K comparable, V any] interface {
	Cache[K, V]
}

// Transactional is an optional interface for cache providers that support multi-operation
// rollback. Providers that implement this can participate in DAL-level transactions with
// proper rollback semantics.
//
// Providers that do not implement Transactional can still participate in transactions via
// the generic journal-based fallback provided by BeginTransaction.
type Transactional[K comparable, V any] interface {
	// BeginTransaction returns a transactional view of this cache.
	//
	// All mutations through the returned TransactionCache are journalled. The caller MUST
	// call either Commit or Rollback on the returned cache.
	//
	// Takes ctx (context.Context) for cancellation and timeout.
	//
	// Returns TransactionCache[K, V] which wraps the cache with undo journalling.
	BeginTransaction(ctx context.Context) TransactionCache[K, V]
}

// TransactionCache wraps a cache with undo journalling. Reads pass through to the
// underlying cache; writes are applied immediately but journalled so they can be undone
// on Rollback.
type TransactionCache[K comparable, V any] interface {
	...
	// Rollback replays the undo journal in reverse, restoring every mutated key to its
...
// Provider is the non-generic interface for cache providers that manage resources. It
// implements io.Closer and manages connections, pools, and other shared resources.
//
// Architecture:
...
// Service manages cache providers and creates configured cache instances. It implements
// io.Closer and cache.Service.
//
// CreateNamespace is not part of Service because Go interfaces cannot have generic
// methods. Use the standalone NewCache function or CacheBuilder instead.
