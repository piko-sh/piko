...
// Package querier_postgres is the PostgreSQL driver for the worker DAL.
...
import (
	...
	"piko.sh/piko/internal/worker/worker_dal/querier_postgres/db"
...
// New creates a worker Store backed by the given PostgreSQL database connection and
// clock.
//
// Takes database (*sql.DB) which is the open PostgreSQL connection.
...
//
// Returns []dalcore.PromoteCandidateRow which are the due scheduled and retryable jobs.
...
// GetJobIDByUniqueKey looks up the id of an existing job by its unique key.
//
// Takes uniqueKey (string) which is the idempotency key matched against
// worker_jobs.unique_key.
//
...
// InsertJobRootWithUniqueKey inserts a job root that carries a unique key.
//
// The insert does nothing on a conflict with an existing unique_key, so no id is
// returned and the underlying row scan yields sql.ErrNoRows.
//
// Takes params (dalcore.InsertJobRootParams) which describe the job root and its
// unique key.
//
// Returns string which is the inserted job id.
// Returns error when the insert fails or the unique key already exists.
