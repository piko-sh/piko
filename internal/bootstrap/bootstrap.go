...
// daemonBuilderFunc defines a function that builds a daemon service for a given run mode.
// It is used as a strategy in the dispatch table pattern.
type daemonBuilderFunc func(ctx context.Context, c *Container, deps *Dependencies) (daemon_domain.DaemonService, error)

var (
	// builders maps run mode strings to their builder functions using the Strategy pattern.
	// New run modes are added by inserting a single entry.
	builders = map[string]daemonBuilderFunc{
...
// ConfigAndContainer creates the DI container and resolves all application configuration
// during bootstrap setup.
//
// Takes ctx (context.Context) which carries the request-scoped logger.
// Takes deps (*Dependencies) which provides core dependencies (currently just AppRouter;
// nil is rejected).
// Takes opts (...Option) which allows changes to container behaviour.
//
// Returns *Container which is the fully configured container ready to build services.
// Returns error when deps.AppRouter is nil, or when the server configuration cannot be
// resolved.
...
// Daemon performs the second phase of initialisation. It takes a fully configured
// container and assembles the final, runnable daemon service.
//
// Takes ctx (context.Context) which carries the request-scoped logger.
// Takes runMode (string) which specifies the operating mode for builder selection.
...
//
// Returns daemon_domain.DaemonService which is the assembled daemon ready to run.
...
// resolveServerConfig populates the supplied target ServerConfig from programmatic
// overrides and resolver placeholders.
//
...
// validateBaseDir checks that the configured base directory exists and is a directory.
// Defaulted to "." when unset, so users without WithBaseDir get the current working
// directory.
//
// Takes sc (*ServerConfig) which is the resolved server configuration.
//
// Returns error when the base directory is missing, inaccessible, or not a directory.
...
// buildOtelSetupOptions creates OtelSetupOptions from the container's OTEL components.
// This includes the monitoring service's span processor and metrics reader, and the
// metrics exporter's reader if configured.
//
// Takes ctx (context.Context) which carries the request-scoped logger.
// Takes container (*Container) which provides access to OTEL components.
//
// Returns *driver_handlers.OtelSetupOptions with span processors and metric readers, or
// nil if no components are configured.
