...
// UnmarshalYAML implements the yaml.Unmarshaler interface to allow case-insensitive
// parsing of the validation level from a YAML configuration file.
//
// Takes unmarshal (func(...)) which decodes the YAML value into a target.
//
// Returns error when the YAML value cannot be decoded or is not one of 'strict', 'soft',
// or 'skip'.
...
// Config holds the project-wide configuration for the PikoML engine. Populated
// programmatically by the framework's bootstrap pipeline.
type Config struct {
	// OverrideAttributes sets custom default attributes for PikoML components.
	//
	// These override the component's built-in defaults. Explicit attributes on a component
	// tag override both built-in and config defaults.
	OverrideAttributes map[string]map[string]string `yaml:"overrideAttributes"`

	// CustomComponents maps custom tag names (e.g., "pml-custom-card") to Go import paths
	// for user-defined PikoML components. The core engine reserves the entry as a
	// configuration extension point.
	CustomComponents map[string]string `yaml:"customComponents"`

	// ValidationLevel sets how strict the PikoML validator is. Valid values are 'strict',
	// 'soft', or 'skip'; default is 'soft'.
	ValidationLevel ValidationLevel `yaml:"validationLevel"`

	// Breakpoint defines the screen width at which responsive layouts should apply, such as
	// stacked columns. Default is "480px".
	...
	// ClearDefaultAttributes disables the built-in default attributes that each component
	// defines, causing only OverrideAttributes from config (along with inline attributes) to
	// be used, giving full control over defaults. Defaults to false.
	ClearDefaultAttributes bool `yaml:"clearDefaultAttributes"`

	// Beautify enables pretty-printing of HTML output; mutually exclusive with Minify.
	Beautify bool `yaml:"beautify"`

	// Minify enables minification of the final HTML output. Mutually exclusive with
	// Beautify.
	Minify bool `yaml:"minify"`

	// PreviewMode indicates that the output is for browser preview, not for sending as an
	// email. When true, local image paths are resolved to served asset URLs instead of CID
	// references.
