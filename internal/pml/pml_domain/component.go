...
// AttributeType defines the type of value an attribute can accept. Used for validation.
...
const (
	...
	// TypeEnum is an attribute type for values that must match one of a set of allowed
	// values.
...
// AttributeDefinition defines the contract for a PikoML component attribute. It is used
// by the validator to check for correctness.
...
const (
	// SourceDefault represents attributes from the component's built-in defaults, optionally
...
// StyleTarget defines the mapping of a CSS property to an internal part of a component's
// rendered HTML structure, enabling declarative style projection.
type StyleTarget struct {
	// Property is the CSS property name (e.g. "background-colour").
	Property string

	// Target is a logical name for the sub-element where the style should be applied (e.g.,
	// "container", "text", "link").
	Target string
}

// Component defines the interface that all PikoML components (e.g., <pml-row>,
// <pml-button>) must implement. The contract allows the transformation engine to be
...
type Component interface {
	...
	// content. If true, the transformer will not recurse into its children.
	//
	...
	// their types. This is used by the validator.
	//
	// Returns map[string]AttributeDefinition which maps attribute names to their type
	// definitions.
	...
	// component.
	//
	// These defaults are applied first with lowest priority and can be overridden by config
	// or inline attributes, so each component can define built-in defaults internally.
	//
	// Returns map[string]string which contains attribute names mapped to their default
	// values.
	DefaultAttributes() map[string]string

	// GetAttributePrecedence returns the order in which style sources should be merged.
	//
	// The standard order is [Default, Inline], where later sources override earlier ones. A
	// component could override this for special cases.
	//
	// Returns []AttributeSource which lists the sources in merge order.
	GetAttributePrecedence() []AttributeSource

	// GetStyleTargets returns the mapping of CSS properties to the component's internal
	// targets. This is the core of the style projection logic.
	//
	...
	//
	// It takes the original <pml-*> node and the current transformation context, and returns
	// a new AST subtree composed of standard, email-safe HTML, along with any diagnostics
