...
// UnorderedList renders bulleted lists using the <ul> HTML element. It implements the
// Component interface and reuses the OrderedList logic, but uses the <pml-ul> tag and a
// different default list style.
type UnorderedList struct {
	OrderedList
}

var (
	_ pml_domain.Component = (*UnorderedList)(nil)
)

// NewUnorderedList creates a new UnorderedList component instance. An UnorderedList is an
// alias of OrderedList that renders unordered lists by default.
//
...
//
// Returns map[string]string which is an empty map as unordered lists have no default
// attributes.
func (*UnorderedList) DefaultAttributes() map[string]string {
	return map[string]string{}
}

// Transform overrides OrderedList's Transform to inject the list-style attribute set to
// "unordered". This means pml-ul always renders as an unordered list by default.
//
...
// including the style manager.
//
// Returns *ast_domain.TemplateNode which is the transformed node.
// Returns []*pml_domain.Error which contains any errors from the parent OrderedList's
// Transform method.
