...
// ListItem implements pml_domain.Component for the <pml-li> tag. It renders list items
// with separate styling for bullet points and content.
type ListItem struct {
	BaseComponent
}

var (
	_ pml_domain.Component = (*ListItem)(nil)
)

// NewListItem creates a new ListItem component instance. A ListItem renders individual
// list items with separate bullet and text styling.
//
...
var (
	...
	listItemAllowedAttributes = map[string]pml_domain.AttributeDefinition{
...
//
// Returns map[string]string which is an empty map as list items have no default
// attributes.
...
//
// Returns []pml_domain.StyleTarget which links style properties to their render targets,
// keeping bullet styles separate from text styles.
...
//
// This generates a <li> element with bullet styling applied to it, and a <span> wrapper
// for the text content with separate styling.
//
...
//
// The transformation implements the Litmus guide pattern for separate bullet/text
// styling:
//
//  1. Build <li> Styles (Bullet Styling): Extract all bullet-* attributes and apply them
//     to the <li> element. These styles affect both the bullet and the text initially.
//
//  2. Build <span> Styles (Paragraph Styling): Extract all text styling attributes
//     (colour, font-size, etc.) and apply them to a <span> that wraps the text content.
//     This resets the text back to desired styles, leaving only the bullet with the
//     bullet-* styles.
//
//  3. Separate Nested Lists from Paragraph Content: Walk through children. Paragraph and
//     inline content go inside the <span>. Any <pml-ol> children are placed directly
//     inside the <li> after the <span>. This structure prevents rogue bullets in Gmail
//     IMAP (GANGA).
//
//  4. Piko Directive Preservation: All p-* directives are transferred to the <li>
//     element, allowing for dynamic list items: <pml-li p-for="item in items">.
...
// buildBulletStyles extracts bullet-* attributes and maps them to standard CSS
// properties.
//
...
// separateListChildren splits the children of a node into text content and nested lists.
//
// Takes node (*ast_domain.TemplateNode) which contains the children to split.
//
// Returns textContent ([]*ast_domain.TemplateNode) which holds all child nodes that are
// not lists.
// Returns nestedLists ([]*ast_domain.TemplateNode) which holds all child nodes that are
// nested lists.
...
// wrapListItemContent wraps text content in a span element when styles are given, then
// adds any nested lists after the text.
//
// Takes textContent ([]*ast_domain.TemplateNode) which holds the text nodes to wrap.
// Takes nestedLists ([]*ast_domain.TemplateNode) which holds any nested list nodes to add
// after the text content.
// Takes spanStyles (map[string]string) which sets the styles to apply to the wrapping
// span element.
//
// Returns []*ast_domain.TemplateNode which holds the combined list item children ready to
// use.
...
// getListItemPositionClasses returns CSS classes based on the position of a list item
// within its parent list.
//
// Takes node (*ast_domain.TemplateNode) which is the list item to check.
// Takes ctx (*pml_domain.TransformationContext) which provides the parent node and
// sibling context.
//
...
// isNestedListNode checks if a child node is a nested list. This includes pml-ol elements
// or ul/ol elements wrapped in a div.
//
