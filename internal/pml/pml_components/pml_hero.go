...
// Hero implements the pml_domain.Component interface for the <pml-hero> tag. It renders a
// full-width banner, often with a background image, that can contain other content
...
var (
	_ pml_domain.Component = (*Hero)(nil)
)

// NewHero creates a new Hero component instance that renders a full-width banner with
...
//
// Returns map[string]string which contains the default attribute key-value pairs.
// Returns an empty map as heroes have no default attributes.
...
//
// Returns []pml_domain.StyleTarget which maps CSS properties to their target elements
// within the hero's complex structure.
...
//
// This component is one of the most complex due to its need to overlay content on a
// responsive background image that works even in Outlook.
//
// The transformation uses several email hacks to ensure cross-client compatibility:
//
//  1. VML (Vector Markup Language) Background for Outlook: Outlook on Windows does not
//     support CSS background-image. To solve this, a VML shape is generated inside
//     Outlook-only conditional comments. - <v:rect>: A full-width rectangle is created. -
//     <v:fill>: Uses background-url as its source, handles background-position and
//     background-size (via aspect="atleast" for cover). - <v:textbox>: All hero content
//     is rendered inside this textbox, creating the illusion of HTML content overlaid on
//     a background image.
//
//  2. CSS Background for Modern Clients: For Gmail, Apple Mail, and others, a div with
//     CSS background-image, background-position, background-size, and background-repeat
//     is used.
//
//  3. Dual-Mode Height Calculation: - mode=ValueFixedHeight: The height attribute is
//     strictly enforced. In Outlook, height is set on the VML <v:rect>. In modern
//     clients, it is set as height and line-height on a <td> wrapper. -
//     mode=ValueFluidHeight (Default): Uses the "padding hack". The aspect ratio of the
//     background image sets a percentage-based padding-bottom on a spacer <td>, making
//     height scale proportionally with width.
//
//  4. Multi-Layered Table Structure: Nested tables orchestrate these behaviours: an
//     outermost Outlook container table, the VML structure (Outlook only), a modern div
//     container (hidden from Outlook), and an inner content table.
//
//  5. Piko Directive Preservation: All p-* directives are transferred to the outermost
//     element, ensuring entire hero sections can be rendered dynamically.
//
// Takes node (*ast_domain.TemplateNode) which is the <pml-hero> node to transform.
...
// validateMandatoryAttributes checks if all required background attributes are present.
//
// Takes styles (*pml_domain.StyleManager) which provides access to the element's style
// attributes.
// Takes ctx (*pml_domain.TransformationContext) which receives diagnostics when
// validation fails.
// Takes node (*ast_domain.TemplateNode) which provides location information for error
// reporting.
//
// Returns bool which is true when background-url, background-width, and background-height
// are all present.
...
// renderVML creates the VML (Vector Markup Language) structure for Outlook, generating a
// v:rect with v:fill for the background image and rendering content inside a v:textbox
// where height handling differs based on the mode attribute.
//
...
// renderContentTable creates the table structure that wraps the hero's children.
//
// This table is used inside both the VML v:textbox (for Outlook) and the modern div. The
// structure differs based on the mode attribute.
//
...
// renderInnerContent wraps the children with inner padding and background colour.
//
// Takes styles (*pml_domain.StyleManager) which provides style values.
// Takes children ([]*ast_domain.TemplateNode) which are the nodes to wrap.
// Takes containerWidth (float64) which specifies the container width.
//
// Returns *ast_domain.TemplateNode which contains the wrapped content with Outlook
...
// renderModern creates the AST for modern, CSS-background-supporting clients. This builds
// a div with CSS background styles and the content table inside.
//
// Takes styles (*pml_domain.StyleManager) which provides style values.
// Takes children ([]*ast_domain.TemplateNode) which are the nodes to wrap.
// Takes mode (string) which determines the output structure.
// Takes containerWidth (float64) which specifies the container width.
//
// Returns *ast_domain.TemplateNode which is the modern div element with CSS background
// styles.
...
// calculateContainerWidth returns the container width, using the mobile breakpoint as the
// default if the width is not set.
//
// Takes ctx (*pml_domain.TransformationContext) which provides the current transformation
// state including container dimensions.
//
// Returns float64 which is the container width, or WidthMobileBreakpoint if the width is
// zero.
...
// calculateChildContainerWidth works out the width available for child elements by
// removing padding from the total width.
//
...
//
// Returns float64 which is the width available for child elements. If the padding is
// larger than the container width, returns the original width.
...
// buildHeroVMLRectStyle creates the style string for the VML v:rect element in hero
// components.
//
...
// buildVMLStructure creates the opening VML structure markup for Microsoft Outlook email
// client support.
//
...
// buildVMLFragment builds the full VML fragment with an optional outer table wrapper.
//
...
// Takes contentTable (*ast_domain.TemplateNode) which provides the inner content to wrap.
//
// Returns []*ast_domain.TemplateNode which contains the built fragment nodes ready for
// rendering.
...
// buildOuterTableStart creates the opening outer table wrapper for container background
// colour in Microsoft Outlook.
//
// Takes attrs (vmlAttributes) which provides the alignment, width, and background colour
// settings for the table.
//
// Returns string which contains the VML conditional comment markup for Outlook support.
...
// calculateAdjustedHeight subtracts padding from height for proper sizing in fixed-height
// mode.
//
...
// calculateFluidHeightPadding works out the padding-bottom percentage for fluid-height
// mode.
//
// Takes styles (*pml_domain.StyleManager) which provides access to background width and
// height values.
// Takes mode (string) which specifies the height mode to check.
//
// Returns string which is the percentage padding (e.g. "56.25%") or empty if the mode is
// not fluid-height or background sizes are missing.
...
// buildContentTableStructure creates a table with tbody, tr, and td elements to wrap
// content.
//
// Takes tdStyles (map[string]string) which provides CSS styles for the td element.
// Takes wrappedChildren (*ast_domain.TemplateNode) which contains the content to place
// inside the table cell.
//
// Returns *ast_domain.TemplateNode which is the complete table structure with the content
// wrapped inside.
...
// addSpacerRowIfNeeded adds a spacer row to the table when in fluid-height mode.
//
// Takes contentTable (*ast_domain.TemplateNode) which is the table to modify.
// Takes mode (string) which sets the layout mode.
// Takes paddingBottomPercent (string) which sets the spacer height as a percentage.
...
// buildInnerChildrenTable creates the table structure that holds the actual children.
//
// Takes children ([]*ast_domain.TemplateNode) which are the child nodes to wrap in a
// table.
//
// Returns *ast_domain.TemplateNode which is the table element containing the children.
...
// Takes childrenTable (*ast_domain.TemplateNode) which contains the nested content to
// wrap.
//
...
//
// Returns outlookWrapperHTML which contains the start and end conditional comment blocks
// for Outlook rendering.
...
// parseVMLBackgroundPosition converts a CSS background-position value to VML origin and
// position values.
//
...
// wrapWithContainerBgIfNeeded wraps the hero div with an outer div if a container
// background colour is set.
//
// Takes styles (*pml_domain.StyleManager) which provides access to style values.
// Takes heroDiv (*ast_domain.TemplateNode) which is the element to wrap.
//
// Returns *ast_domain.TemplateNode which is the original heroDiv if no background colour
// is set, or a new wrapper div containing it.
