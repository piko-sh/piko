...
// DaemonConfig holds the resolved configuration values needed by the daemon service for
// server and health probe startup. All fields are value types; pointer-to-value
...
type DaemonConfig struct {
	...
	// TLSRedirectHTTPPort, when non-empty, starts a plain HTTP listener on this port that
	// 301-redirects all requests to the HTTPS server.
	...
	// ShutdownDrainDelay is the duration to wait after signalling drain (readiness returning
	// 503) before shutting down the main HTTP server. This gives load balancers time to
	// deregister the instance.
	ShutdownDrainDelay time.Duration

	// MaxConcurrentSEOJobs caps the number of SEO artefact regenerations that may run at
	// once.
	//
	// Rapid build notifications (such as dev-mode hot reloads) otherwise stack goroutines
	// without bound. Zero or negative values resolve to a sensible default in NewService.
	MaxConcurrentSEOJobs int

	// NetworkAutoNextPort enables automatic port selection when the default port is already
	// in use.
