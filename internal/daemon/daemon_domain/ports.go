...
// DaemonService defines the interface for the main daemon that manages the HTTP server
// lifecycle, file watching, and asset processing.
type DaemonService interface {
	// RunDev runs the application in development mode.
	//
	// Returns error when the development server fails to start or stops with an error.
	RunDev(parentCtx context.Context) error

	// RunProd runs the application in production mode.
	//
	// Returns error when the application fails to start or encounters a fatal error.
...
// ServerAdapter defines an interface for HTTP server operations. It allows different
// server types to be used for testing.
...
// RouteProvider defines the contract for services that can mount routes to a Chi router.
...
// RouterDependencies groups all dependencies needed to build the HTTP router. This
// reduces parameter count and makes the BuildRouter signature more maintainable.
type RouterDependencies struct {
	...
	// PresignUploadHandler handles presigned URL uploads; may be nil if not enabled.
	PresignUploadHandler http.Handler

	// PresignDownloadHandler handles presigned URL downloads. May be nil if not enabled.
	PresignDownloadHandler http.Handler

	// PublicDownloadHandler handles public file downloads without authentication; may be nil
	// if not enabled.
	...
	// AuthProvider resolves authentication state from requests. Nil means no auth middleware
	// is installed.
	...
	// AnalyticsService distributes analytics events to registered collectors. Nil means no
	// analytics middleware is installed.
	AnalyticsService *analytics_domain.Service

	// CSPConfig provides the computed CSP settings for security headers middleware.
	CSPConfig security_dto.CSPRuntimeConfig
}

// RouterConfig holds the exact fields the HTTP router needs from the server
// configuration. This replaces passing the full ServerConfig to BuildRouter, making the
// router's requirements explicit and decoupled.
type RouterConfig struct {
	// DevAPIHandler serves the /_piko/dev/api/* REST endpoints for the dev tools overlay
	// widget. Nil in production mode.
	DevAPIHandler DevAPIHandlerPort

	// DevEventsBroadcaster serves the /_piko/dev/events SSE endpoint for dev-mode build
	// notifications. Nil in production mode.
	DevEventsBroadcaster http.Handler

	// DevPreviewHandler serves the /_piko/dev/preview/* endpoints for dev-mode template
	...
	// MaxConcurrentRequests is the maximum concurrent in-flight requests (default: 10000).
	// Set to 0 to disable.
	...
	// DisableHTTPCache forces all static asset responses to use no-cache, requiring ETag
...
// RouterBuilder defines how to build the final HTTP router. It is implemented by
// daemon_adapters.HTTPRouterBuilder.
type RouterBuilder interface {
	// BuildRouter constructs an HTTP router with the given configuration and dependencies.
	//
	...
	// Close stops background goroutines owned by the builder. Call when the router is no
	// longer needed.
	Close()
}
...
// can be invalidated, typically in response to file system changes.
...
// artefact generation is skipped during hot-reload.
...
// RouterManager handles route loading and reloading for hot-reload support. It implements
// http.Handler and is only used in development modes.
...
// AssetPipelinePort processes build results to create transformation profiles. It allows
...
// DrainSignaller marks a service as draining so that readiness probes return unhealthy
// before the HTTP servers shut down. This allows load balancers to deregister the
// instance during the drain period.
type DrainSignaller interface {
	// SignalDrain marks the service as draining. After this call, readiness checks should
	// return unhealthy.
	SignalDrain()
}

// SignalNotifier provides an interface for receiving OS shutdown signals. In production,
// it uses signal.NotifyContext to listen for SIGINT/SIGTERM; in tests, it can be mocked
// to control shutdown timing.
type SignalNotifier interface {
	// NotifyContext returns a context that is cancelled when a shutdown signal is received.
