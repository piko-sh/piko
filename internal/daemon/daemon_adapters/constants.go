...
const (
	...
	// defaultIdleTimeout is the maximum time to wait for the next request on a keep-alive
	// connection.
	...
	// headerContentEncoding is the HTTP header key for specifying content encoding.
	...
	// headerETag is the HTTP header name for entity tags used for cache control. Canonical
	// MIME form is "Etag" (CanonicalMIMEHeaderKey lowercases after the first character of
	// each hyphen-separated segment).
	headerETag = "Etag"
	...
	// jitCacheControl is the Cache-Control value for JIT-generated responses. These
	// responses are served immediately but must be revalidated.
	...
	// cacheControlMutableAsset is the prod Cache-Control for assets that change when a
	// project is updated (theme, sitemaps, robots.txt, HLS playlists, embedded frontend).
	...
	// errMessageInternalServer is the error message returned to clients for internal server
	// errors.
	...
	// hlsVariantPrefix is the prefix added to quality names to form HLS variant IDs.
	hlsVariantPrefix = "hls_"

	// hlsDefaultSegmentDuration is the default duration in seconds for HLS segments.
	...
	// msgVariantNotFound is the log message used when a video variant cannot be found.
	...
	// logFieldChiPattern is the log field key for the chi-translated route pattern (e.g.
	// "/docs/*" derived from "/docs/{slug:.+}").
	...
	// logFieldIsE2EOnly is the log field key indicating whether a route is only registered
	...
	// logFieldETagMatch is the tracing attribute key that records whether the client's ETag
	// matched the cached resource.
	...
	// logFieldHTTPFile is the file name used for log field identification in HTTP handlers.
	logFieldHTTPFile = "driven_http.go"

	// logFieldHTTPRouterFile is the filename used in structured log entries for HTTP router
	// operations.
	...
	// headerXPPResponseSupport is the canonical MIME form of the response support
	// negotiation header. CanonicalMIMEHeaderKey("X-PP-Response-Support") lowercases "PP" to
	// "Pp".
	headerXPPResponseSupport = "X-Pp-Response-Support"
)

var (
	// pageDefPool is a sync.Pool for reusing PageDefinition structs to reduce allocations in
	// high-throughput page request handling.
	...
	// headerValContentTypeHTML is a pre-allocated header value slice for zero-alloc direct
	// map assignment.
	//
	// Assigned to the response header map via h[key] = slice, bypassing Header.Set() and its
	// per-call []string + CanonicalMIMEHeaderKey allocations. The stdlib only reads these
	// slices during response writing, never mutating them, so sharing across concurrent
	// requests is safe.
	headerValContentTypeHTML = []string{contentTypeHTML}

	// headerValContentTypeMPEGURL is a pre-allocated header value slice for HLS playlist
	// content type.
	headerValContentTypeMPEGURL = []string{contentTypeMPEGURL}

	// headerValContentTypeText is a pre-allocated header value slice for plain text content
	// type.
	headerValContentTypeText = []string{contentTypeTextPlain}

	// headerValEncodingBrotli is a pre-allocated header value slice for Brotli content
	// encoding.
	...
	// headerValCacheStatusMiss is a pre-allocated header value slice for a cache miss
	// status.
	...
	// headerValCacheStatusStale is a pre-allocated header value slice for a stale cache
	// status.
	headerValCacheStatusStale = []string{"STALE"}

	// headerValVaryCachePage is the Vary header for cached page responses without i18n.
	// Accept-Encoding because different compressed variants are served.
	headerValVaryCachePage = []string{"Accept-Encoding"}

	// headerValVaryCachePageI18n is the Vary header for cached page responses with i18n
	// enabled. Includes Accept-Language because the cache key varies by locale.
