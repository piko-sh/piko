...
// FrontendModule represents a built-in Piko frontend module that implements fmt.Stringer.
// These are optional JavaScript bundles that add extra features such as analytics,
// modals, or toasts to the core framework.
type FrontendModule uint8

const (
	// ModuleAnalytics is a frontend module that adds Google Analytics (GA4) support.
	ModuleAnalytics FrontendModule = iota + 1

	// ModuleModals provides helper functions for modal dialogues. Exports: showModal,
	// closeModal, updateModal, and reloadPartial.
	ModuleModals

	// ModuleToasts provides toast notification helpers. Exports: showToast(message, variant,
	// duration).
	ModuleToasts

	// ModuleDev provides dev tools including auto-refresh on rebuild via SSE. Only loaded in
	// dev and dev-i modes.
	ModuleDev

	// ModuleComponents provides the PKC component runtime (PPElement, VDOM, reactivity).
	// Unlike other modules, this is not injected site-wide; it is loaded on demand when a
	// page contains PKC components.
	ModuleComponents
)

// AnalyticsConfig holds settings for the analytics module. It supports Google Analytics
// (GA4) direct integration and/or Google Tag Manager (GTM) container loading.
type AnalyticsConfig struct {
	// GTMContainerID is a Google Tag Manager container ID (e.g., "GTM-XXXXXXX"). When set,
	// the GTM container script is loaded and SPA navigation events are pushed to the
	// dataLayer for GTM triggers to consume.
	GTMContainerID string `json:"gtmContainerId,omitempty"`

	// TrackingIDs holds GA4 measurement IDs (e.g., "G-XXXXXXXXXX"). Multiple IDs allow
	// sending data to more than one property.
	TrackingIDs []string `json:"trackingIds"`

	// DebugMode enables detailed console logging for debugging.
	DebugMode bool `json:"debugMode,omitempty"`

	// AnonymiseIP hides user IP addresses before sending data to Google Analytics.
	AnonymiseIP bool `json:"anonymiseIp,omitempty"`

	// DisablePageView stops automatic page view tracking when users move between pages. Set
	// to true to track page views yourself.
	DisablePageView bool `json:"disablePageView,omitempty"`
}

// ModalsConfig provides settings for the Modals module. All fields default to false, and
// the frontend applies its own defaults when fields are not set, so only set fields you
// want to change.
type ModalsConfig struct {
	// DisableCloseOnEscape stops modals from closing when the Escape key is pressed. When
	// false (the default), pressing Escape closes the modal.
	DisableCloseOnEscape bool `json:"disableCloseOnEscape,omitempty"`

	// DisableCloseOnBackdrop stops modals from closing when clicking the backdrop. When
	// false (the default), clicking the backdrop closes the modal.
...
type ToastsConfig struct {
	// Position sets where toasts appear on the screen. Valid values are "top-right",
	// "top-left", "bottom-right", "bottom-left", "top-centre", and "bottom-centre".
	Position string `json:"position,omitempty"`

	// DefaultDuration is the display time in milliseconds; default is 5000.
	DefaultDuration int `json:"defaultDuration,omitempty"`

	// MaxVisible sets the maximum number of toasts shown at once; default is 5.
	MaxVisible int `json:"maxVisible,omitempty"`
}

// ModuleEntry holds a built-in frontend module with its optional settings.
type ModuleEntry struct {
	// Config is the strongly-typed config (AnalyticsConfig, ModalsConfig, etc.) or nil.
	Config any
...
// AssetPath returns the path to the module's JS file in the embedded asset store.
//
// Returns string which is the asset path, or an empty string for unknown modules.
...
// ServeURL returns the URL at which this module is served to browsers. This is used for
// generating <script> and <link rel="modulepreload"> tags.
//
...
// CustomFrontendModule represents a user-provided frontend JavaScript module. The content
// is provided at compile time via go:embed and registered at startup.
type CustomFrontendModule struct {
	// Config holds optional settings passed to the module; nil means no config.
...
// Takes config (map[string]any) which specifies module configuration options.
//
// Returns *CustomFrontendModule which is the initialised module ready for use.
...
//
// Returns string which is the fully qualified path to the minified JavaScript file for
// this module.
...
//
// Returns string which is the full path to the minified JavaScript asset.
...
var (
	...
	// cachedModuleConfigHTML holds the pre-built HTML for module configuration script
	// elements.
	cachedModuleConfigHTML string

	// cachedDevWidgetHTML holds the HTML element for the dev tools overlay widget. Set once
	// at startup in dev mode; empty in production.
	cachedDevWidgetHTML string
)

// SetModuleHTML sets the cached module HTML for site-wide injection. Call this once
// during startup after all modules are registered.
//
...
// GetModulePreloadHTML returns the cached preload HTML for frontend modules.
//
// Returns string which contains the HTML link elements for module preloading.
func GetModulePreloadHTML() string {
	return cachedModulePreloadHTML
}

// GetModuleScriptHTML returns the cached script HTML for frontend modules.
//
...
// GetModuleConfigHTML returns the cached HTML script for frontend module settings.
//
// Returns string which contains the pre-rendered HTML script configuration.
func GetModuleConfigHTML() string {
	return cachedModuleConfigHTML
}

// SetDevWidgetHTML sets the HTML element for the dev tools overlay widget. Call once at
// startup in dev mode.
//
...
// GetDevWidgetHTML returns the cached dev widget HTML element. Empty in production mode.
//
...
// GenerateModuleHTML creates the HTML needed to load frontend modules.
//
// Takes moduleEntries ([]ModuleEntry) which contains the built-in modules to include.
// Takes customModules (map[string]*CustomFrontendModule) which provides custom modules to
// include alongside the built-in ones.
//
// Returns preloadHTML (string) which contains link tags for module preloading.
// Returns scriptHTML (string) which contains script tags for module loading.
// Returns configHTML (string) which contains a JSON script tag with module settings, or
// an empty string if no modules have settings.
...
// writeModulePreloadTag writes a modulepreload link tag, with optional SRI integrity and
...
// Takes sriHash (string) which is the SRI hash for the integrity attribute, or empty to
// omit integrity.
...
// writeModuleScriptTag writes a module script tag, with optional SRI integrity and
// crossorigin attributes.
//
// Takes buf (*strings.Builder) which receives the generated HTML.
...
//
// Returns string which is the quoted hash prefix in hexadecimal format.
