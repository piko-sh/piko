...
type WebsiteConfig struct {
	...
	// CanonicalBaseURL is the absolute site origin used to build canonical and hreflang
	// URLs.
	//
	// An example value is "https://example.com". When empty the live request host is used,
	// which is fine in development but can bake a stale host into cached pages. The
	// bootstrap populates it from the SEO sitemap hostname so heads and sitemap agree.
