...
// SEOConfig holds configuration for all SEO-related artefact generation, including
...
type SitemapConfig struct {
	// Sitemaps defines named sitemap chunks for large sites with 50,000 or more URLs. Each
	...
	// Sources lists runtime API endpoints that return dynamic URLs for the sitemap. Each
	// endpoint should return a JSON array of SitemapURLInput objects, for example
...
type SitemapEntryDefaults struct {
	// ChangeFreq is the default change frequency hint for search engines. Valid values:
...
type RobotsConfig struct {
	// CustomRules defines custom robots.txt rules for specific user agents. Each rule group
	// targets one or more user agents and can set Allow and Disallow paths.
	CustomRules []RobotsRuleGroup `json:"customRules" yaml:"customRules" usage:"Custom robots.txt rules." summary:"hide"`

	// BlockAiBots controls whether to block known AI crawler bots from accessing the site.
	// When enabled, blocks GPTBot, ChatGPT-User, Claude-Web, anthropic-ai, Google-Extended,
	// Applebot-Extended, Bytespider, CCBot, cohere-ai, Diffbot, FacebookBot, ImagesiftBot,
	...
	// BlockNonSeoBots controls whether to block known non-SEO web scrapers. When true, adds
	// rules to block bots such as Nuclei, WikiDo, Riddler, PetalBot, Zoominfobot,
...
type RobotsRuleGroup struct {
	// UserAgents is the list of user agent names this rule group applies to. Use "*" to
	// target all bots, or specific names like "Googlebot", "Bingbot", etc.
	...
	// Allow is the list of URL path patterns that these user agents may crawl, overriding
	// more general Disallow rules. Useful for whitelisting specific paths within a
	// disallowed section.
