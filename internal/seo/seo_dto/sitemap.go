...
type SitemapURL struct {
	...
	// LastMod is the date of last modification in W3C Datetime format (YYYY-MM-DD).
	LastMod string `xml:"lastmod,omitempty"`

	// ChangeFreq is a hint to crawlers about how often the page may change. Valid values:
	// always, hourly, daily, weekly, monthly, yearly, never.
	ChangeFreq string `xml:"changefreq,omitempty"`

	// Priority is how important this URL is compared to other URLs on the site. Valid range:
...
// AlternateLink represents an i18n alternate language link. It follows the hreflang
// standard for declaring language and regional variants of a page.
type AlternateLink struct {
	// Rel specifies the link relation type; must be "alternate" for hreflang links.
...
// ImageEntry represents an image within a sitemap URL entry. Including images in sitemaps
// helps search engines discover and index them more effectively.
type ImageEntry struct {
	...
	// GeoLocation is the geographic location of the image (e.g. "London, United Kingdom").
...
type VideoEntry struct {
	...
	// FamilyFriendly indicates whether the video is suitable for all audiences. Valid
	// values: "yes" (default) or "no".
	FamilyFriendly string `xml:"video:family_friendly,omitempty"`

	// Description is the text describing the video; required, maximum 2048 characters.
	...
	// Live indicates whether the video is a live stream. Valid values: "yes" or "no".
	...
	// RequiresSubscription indicates whether a subscription is required. Valid values: "yes"
	// or "no".
...
type NewsPublication struct {
	// Name is the required name of the news publication (e.g. "The Example Times").
...
// SitemapURLInput represents the JSON format for dynamic URL sources. External APIs such
// as a headless CMS should return a list of these objects.
type SitemapURLInput struct {
	...
	// Images is a list of full image URLs for this page. For richer image metadata (caption,
	// title, licence), use ImageEntries instead.
...
// VideoInputEntry is the JSON input format for video entries from dynamic sources.
//
...
// NewsInputEntry is the JSON input format for news entries from dynamic sources.
...
// ImageInputEntry is the JSON input format for rich image metadata from dynamic sources.
...
// PageSEOMetadata contains SEO-relevant metadata extracted from a component's render
// function. This avoids the need for full page rendering when only metadata is required.
type PageSEOMetadata struct {
	// LastModified is the date when the page was last changed. If nil, the file's
...
// site has multiple sitemap files, typically for sites with 50,000+ URLs.
...
//
// For small sites (URLs <= MaxURLsPerSitemap), only Sitemaps[0] is populated and Index is
// nil. For large sites, multiple Sitemaps are populated and Index contains references to
// them.
type SitemapBuildResult struct {
	// Index is the sitemap index file; nil when only one sitemap is needed.
	Index *SitemapIndex

	// Sitemaps holds one or more sitemap files for the build result. A single sitemap
	// produces one entry; split sitemaps produce multiple entries.
	Sitemaps []Sitemap
}
