...
func TestDotF32KernelBoundariesAMD64(t *testing.T) {
	t.Parallel()
	...
	for _, k := range kernels {
		t.Run(k.name, func(t *testing.T) {
			t.Parallel()
			for _, length := range productionDotBoundaryDims {
				t.Run(fmt.Sprintf("dim=%d", length), func(t *testing.T) {
					t.Parallel()
					rng := rand.New(rand.NewPCG(uint64(length)*0x9E3779B97F4A7C15+2, 0xCAFEF00DBEEFFACE))
					...
					got := k.fn(a, b)
					tolerance := productionDotTolerance(a, b)
