...
// recordOperationMetrics records both count and duration metrics for a crypto operation.
//
// Takes provider (EncryptionProvider) which identifies the provider type for metrics.
...
// Takes durationMs (int64) which specifies the operation duration in milliseconds.
...
//
// Takes provider (EncryptionProvider) which identifies the provider for metrics.
...
//
// This is a critical security measure for ephemeral encryption keys. The memory barrier
// prevents the compiler from optimising away the writes.
//
