...
func metricAttributes(ctx context.Context, keyValues ...string) metric.MeasurementOption {
	if len(keyValues)%2 != 0 {
		_, l := logger_domain.From(ctx, log)
		l.Error("metricAttributes received odd number of arguments; metric dimensions dropped",
			logger_domain.Int("arg_count", len(keyValues)),
		)
