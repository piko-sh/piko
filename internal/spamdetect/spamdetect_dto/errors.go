...
var (
	...
	// Returned by callers that wrap Analyse and reject above-threshold
	// submissions via a single sentinel rather than inspecting
	...
	// ErrDetectorUnavailable indicates a named detector could not be
	// resolved from the registry. Returned when an expected detector is
	...
	// ErrDetectorNameEmpty indicates RegisterDetector was called with an
	// empty name.
	ErrDetectorNameEmpty = errors.New("spam detection: detector name is empty")

	// ErrDetectorNil indicates RegisterDetector was called with a nil
	// detector.
...
// about which operation and detector produced the failure. Callers can
// use errors.As to extract operation and detector attribution.
