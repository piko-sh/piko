...
type Field struct {
	// Key is the form argument key.
	Key string

	// Type is the semantic type of the field.
	Type FieldType

	// Signals lists the spam detection signals that apply to the field.
	Signals []Signal

	// MaxLength is the maximum byte length for the field's value.
	MaxLength int

	// Weight is the scoring weight for the field.
	Weight float64
}
...
//
// Panics when NewSchema returns a non-nil error.
func MustNewSchema(entries ...SchemaEntry) *Schema {
	schema, err := NewSchema(entries...)
	if err != nil {
		panic(fmt.Errorf("spamdetect: MustNewSchema: %w", err))
	}
	return schema
}
...
//
// Takes key (string) which is the form argument key.
// Takes signals (...Signal) which are the detection signals to apply.
//
...
func Language(language string) SchemaEntry {
	return schemaOptionFunc(func(schema *Schema) {
		if len(schema.languages) >= maxDetectorOptions {
			schema.recordCapExceeded("Language", maxDetectorOptions)
			return
		}
...
func DetectorWeight(detectorName string, weight float64) SchemaEntry {
	return schemaOptionFunc(func(schema *Schema) {
		...
		if len(schema.detectorWeights) >= maxDetectorOptions {
			schema.recordCapExceeded("DetectorWeight", maxDetectorOptions)
			return
		}
...
func DetectorConfig(detectorName string, options map[string]any) SchemaEntry {
	return schemaOptionFunc(func(schema *Schema) {
		...
		if len(schema.detectorOptions) >= maxDetectorOptions {
			schema.recordCapExceeded("DetectorConfig", maxDetectorOptions)
			return
		}
		if len(options) > maxDetectorOptionEntries {
			schema.recordCapExceeded(fmt.Sprintf("DetectorConfig(%q) options", detectorName), maxDetectorOptionEntries)
			return
		}
...
func Meta(key, value string) SchemaEntry {
	return schemaOptionFunc(func(schema *Schema) {
		...
		if len(schema.metadata) >= maxDetectorOptions {
			schema.recordCapExceeded("Meta", maxDetectorOptions)
			return
		}
...
func CaptureHeader(headerName string) SchemaEntry {
	return schemaOptionFunc(func(schema *Schema) {
		if len(schema.capturedHeaders) >= maxDetectorOptions {
			schema.recordCapExceeded("CaptureHeader", maxDetectorOptions)
			return
		}
...
func (f *FieldEntry) MaxLength(length int) *FieldEntry {
	...
	case length < 1:
		length = 1
	case length > maxFieldMaxLength:
		length = maxFieldMaxLength
	}
...
// recordCapExceeded records a builder cap breach against the schema.
//
// Takes builder (string) which identifies the builder entry.
// Takes capacity (int) which is the maximum allowed entries.
func (s *Schema) recordCapExceeded(builder string, capacity int) {
...
func (s *Schema) validate() {
	if len(s.fields) > maxSchemaFields {
		...
		s.fields = s.fields[:maxSchemaFields]
	}

	seenKeys := make(map[string]struct{}, len(s.fields))
	for _, field := range s.fields {
		if field.Key == "" {
			continue
		}
		if _, duplicate := seenKeys[field.Key]; duplicate {
			s.buildErrors = append(s.buildErrors, fmt.Errorf("%w: %q", ErrSchemaDuplicateField, field.Key))
			continue
		}
		seenKeys[field.Key] = struct{}{}
	}

	if s.thresholdExplicit && (s.threshold < 0 || s.threshold > 1) {
		s.buildErrors = append(s.buildErrors, fmt.Errorf("%w: got %v", ErrSchemaInvalidThreshold, s.threshold))
	}
}
