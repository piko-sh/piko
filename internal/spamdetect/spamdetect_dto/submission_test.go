...
func TestSubmission_Sanitise_UTF8Safety(t *testing.T) {
	t.Parallel()
	schema := MustNewSchema(TextField("name").MaxLength(10))
	submission := &Submission{
...
func TestSubmission_Sanitise_MetadataFields(t *testing.T) {
	...
	assert.Equal(t, normalisedIPHashLength, len(submission.RemoteIP))
...
func TestSubmission_Sanitise_WithMetadataAndHeaders(t *testing.T) {
	t.Parallel()
	schema := MustNewSchema(TextField("message").MaxLength(50))
	submission := &Submission{
		Fields: map[string]FieldValue{
