...
const (
	// csrfCookieName is the name of the CSRF binding cookie. Using _piko_ prefix to
	// namespace piko cookies.
	csrfCookieName = "_piko_csrf_token"

	// csrfCookieTokenBytes is the byte length of the random cookie token. 16 bytes = 128
	// bits of entropy, matching the ephemeral token length.
	csrfCookieTokenBytes = 16
)

var (
	// cookieRandomBytesPool reuses byte slices to reduce allocation pressure during CSRF
	...
	// b64CookieBufPool reuses byte slices to reduce allocation pressure during base64
	// encoding of CSRF cookie tokens.
...
// cookieCSRFSourceAdapter implements CSRFCookieSourceAdapter using an HttpOnly cookie for
...
type cookieCSRFSourceAdapter struct {
	...
	// randReader provides cryptographic random bytes for token generation. Defaults to
	// crypto/rand.Reader; injectable for testing.
...
// GetOrCreateToken returns the CSRF binding token for the request. If no token exists in
// the request cookies, it generates a new random token and sets it as an HttpOnly cookie
// on the response.
//
...
func (a *cookieCSRFSourceAdapter) GetOrCreateToken(r *http.Request, w http.ResponseWriter) (string, error) {
	...
	cookie := acquireCSRFCookie() //nolint:gosec // secure attributes (HttpOnly, Secure) applied by SecureCookieWriter
...
// InvalidateToken removes the CSRF binding token cookie by setting it with a negative
// MaxAge. Call this on logout or explicit session invalidation.
//
...
//
// Takes cookieWriter (security_domain.SecureCookieWriter) which handles secure cookie
// setting with appropriate flags.
// Takes maxAge (time.Duration) which is how long the cookie lasts. Set maxAge to 0 for
...
func acquireCSRFCookie() *http.Cookie {
	cookie, ok := csrfCookiePool.Get().(*http.Cookie) //nolint:gosec // secure attributes (HttpOnly, Secure) applied by SecureCookieWriter
	if !ok {
		cookie = &http.Cookie{}
	}
	cookie.Name = ""
...
// releaseCSRFCookie returns a cookie to the pool. Must be called after SetCookie has
// serialised the cookie.
//
