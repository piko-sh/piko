...
// CSRFTokenService provides CSRF token generation and validation. It uses the Double
// Submit Cookie pattern where tokens are bound to a browser session cookie rather than a
...
type CSRFTokenService interface {
	...
	// Takes r (*http.Request) which provides the request context for token generation.
	// Takes buffer (*bytes.Buffer) which is used to build the action token. The returned
	// CSRFPair.ActionToken is a slice into this buffer, so the buffer must remain valid for
	// the lifetime of the returned pair.
	//
	...
	// ValidateCSRFPair checks whether the CSRF token pair is valid. Validates that the
	// cookie value embedded in the token matches the actual cookie from the request.
	//
	// Takes r (*http.Request) which provides the request context and cookie.
	// Takes rawEphemeralTokenFromRequest (string) which is the ephemeral token from the
	// request.
	// Takes actionToken ([]byte) which is the action token from the request header. Use
	// mem.Bytes() for zero-copy conversion from string.
	//
	// Returns bool which is true if the token pair is valid.
	// Returns error when validation fails.
	// Returns CSRFValidationError with specific error codes for frontend error recovery.
	ValidateCSRFPair(r *http.Request, rawEphemeralTokenFromRequest string, actionToken []byte) (bool, error)
}

// CSRFCookieSourceAdapter provides the CSRF binding token value for the Double Submit
// Cookie pattern. This abstracts cookie management to allow framework users to integrate
// with their own session systems.
//
// The default implementation uses an HttpOnly cookie. Framework users can override this
// to use their authentication session token (for example, from SCS).
type CSRFCookieSourceAdapter interface {
	// GetOrCreateToken returns the CSRF binding token for the request. If no token exists,
	// it creates one and sets it on the response.
	//
	...
	// GetToken returns the existing CSRF binding token from the request cookie. Used during
	// token validation.
	//
	...
	// InvalidateToken removes the CSRF token cookie. Call this when a user logs out or when
	// the session ends.
	//
...
type RequestContextBinderAdapter interface {
	...
	//
	// Takes r (*http.Request) which is the HTTP request to extract the identifier from.
	//
	// Returns string which is the binding identifier.
	GetBindingIdentifier(r *http.Request) string
}

// SecureCookieWriter provides secure cookie setting with enforced security defaults. It
// applies appropriate security flags to cookies (HttpOnly, Secure, SameSite) based on
// configuration, helping protect against session hijacking and CSRF attacks.
type SecureCookieWriter interface {
	// SetCookie writes a cookie to the response with security flags applied. The cookie's
	// security attributes are modified based on configuration: HttpOnly is forced if
	// configured, Secure is forced if configured and serving over HTTPS, and SameSite
	// defaults to the configured value if not set.
	//
	// Takes w (http.ResponseWriter) which is the response to write the cookie to.
	// Takes cookie (*http.Cookie) which is the cookie to set.
	SetCookie(w http.ResponseWriter, cookie *http.Cookie)

	// IsHTTPS returns whether the current request is served over HTTPS. Used to check if the
	// Secure flag should be set on cookies.
	//
	// Returns bool which is true if the request uses HTTPS, false otherwise.
	IsHTTPS() bool
}

// ClientIPExtractor extracts the real client IP address from HTTP requests. It handles
