...
// Directive represents a Content-Security-Policy directive name. Using a typed constant
// prevents typos and allows compile-time checking.
type Directive string

const (
	// DefaultSrc is the fallback directive used when other fetch directives are not set.
	...
	// ScriptSrcElem restricts the locations from which script elements may be executed.
	ScriptSrcElem Directive = "script-src-elem"

	// ScriptSrcAttr restricts the locations from which inline script event handlers may be
	// executed.
	...
	// StyleSrcElem is the directive that controls where style elements may load from.
	StyleSrcElem Directive = "style-src-elem"

	// StyleSrcAttr restricts the locations from which inline style attributes may be
	// applied.
	...
	// ConnectSrc is the directive that controls which URLs can be accessed using script
	// interfaces such as fetch, XMLHttpRequest, and WebSocket.
	...
	// FrameSrc is the directive that controls which URLs may be used as sources for frames.
	FrameSrc Directive = "frame-src"

	// ChildSrc is the directive that controls where workers and frames can load from.
	...
	// ManifestSrc restricts the locations from which application manifests may be loaded.
	ManifestSrc Directive = "manifest-src"

	// PrefetchSrc is the directive that limits where resources may be prefetched from.
	PrefetchSrc Directive = "prefetch-src"

	// BaseURI is the CSP directive that controls which URLs can be used in a document's base
	// element.
	BaseURI Directive = "base-uri"

	// Sandbox is the directive name for enabling a sandbox on the requested resource.
	Sandbox Directive = "sandbox"

	// FormAction restricts the URLs which can be used as the target of form submissions.
	...
	// UpgradeInsecureRequests instructs user agents to treat all insecure URLs as secure.
	...
	// RequireTrustedTypesFor enforces Trusted Types at the DOM XSS injection sinks.
	RequireTrustedTypesFor Directive = "require-trusted-types-for"

	// TrustedTypes is the directive that controls which Trusted Types policies may be
	// created.
...
var (
	...
	booleanDirectives = map[Directive]bool{ //nolint:exhaustive // exhaustive case-set intentionally partial; missing entries are no-ops
