...
// Source represents a Content-Security-Policy source value. Sources can be keywords (like
// 'self'), schemes (like 'data:'), hosts (like 'cdn.example.com'), or special values
// (like hashes and request tokens).
type Source string
...
const (
	...
	// UnsafeInline allows inline scripts and styles. Warning: This significantly reduces CSP
	// protection against XSS.
	UnsafeInline Source = "unsafe-inline"

	// UnsafeEval allows use of eval() and similar dynamic code execution. Warning: This
	...
	// StrictDynamic allows scripts loaded by trusted scripts to execute. When present,
	// 'self' and URL-based allowlists are ignored for script-src.
	StrictDynamic Source = "strict-dynamic"

	// ReportSample tells the browser to include a sample of the code that broke the rules in
	// violation reports.
	...
	// Data is a CSP source value that allows data: URIs for inline content such as images
	// and fonts.
	Data Source = "data:"
	...
	// HTTP allows any resource loaded over HTTP. Warning: this reduces security; prefer
	...
	// RequestTokenPlaceholder is the substitution marker that the security middleware
	// replaces with a unique token for each request, enabling strict CSP policies that
	// permit specific inline scripts and styles.
	RequestTokenPlaceholder Source = "{{REQUEST_TOKEN}}"

	// Script is the keyword for require-trusted-types-for directive. It is the only valid
	// value for that directive and will be quoted.
	Script Source = "script"

	// AllowDuplicates allows more than one Trusted Types policy with the same name. Used
	// with the trusted-types directive.
	AllowDuplicates Source = "allow-duplicates"

	// Wildcard allows Trusted Types policies to be created with any unique name. Unlike
	// keywords, it is not quoted in the output.
	Wildcard Source = "*"
)

var ( //nolint:exhaustive // exhaustive case-set intentionally partial; missing entries are no-ops
	// keywordSources is the set of sources that require single-quoting.
	keywordSources = map[Source]bool{ //nolint:exhaustive // exhaustive case-set intentionally partial; missing entries are no-ops
		Self:            true,
		None:            true,
		UnsafeInline:    true,
		UnsafeEval:      true,
...
//
// Takes h (string) which specifies the host in formats such as "cdn.example.com",
...
// Scheme creates a Source from a URL scheme. The scheme should include the trailing colon
// (e.g., "wss:").
//
...
// Hash creates a Source from a hash algorithm and base64-encoded hash value. The result
// is automatically single-quoted.
//
...
//
// Use this for allowing specific inline scripts or styles by their content hash.
//
...
//
// This produces the CSP-spec required format for inline script/style authorisation. For
// dynamic per-request tokens, use RequestTokenPlaceholder instead.
//
// Takes token (string) which is the per-request token value to embed in the source.
//
...
// alphanumeric, dash, hash, equals, underscore, slash, at-sign, period, and percent.
//
...
// formatSourceForHeader formats a source value for inclusion in a CSP header. Keywords
// are automatically single-quoted; other values are returned as-is.
//
