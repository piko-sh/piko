...
// MockCSRFTokenService is a test double for CSRFTokenService that returns zero values
// from nil function fields and tracks call counts atomically.
type MockCSRFTokenService struct {
	// GenerateCSRFPairFunc is the function called by GenerateCSRFPair.
	GenerateCSRFPairFunc func(w http.ResponseWriter, r *http.Request, buffer *bytes.Buffer) (security_dto.CSRFPair, error)

	// ValidateCSRFPairFunc is the function called by ValidateCSRFPair.
	...
	// GenerateCSRFPairCallCount tracks how many times GenerateCSRFPair was called.
	GenerateCSRFPairCallCount atomic.Int64

	// ValidateCSRFPairCallCount tracks how many times ValidateCSRFPair was called.
...
var (
	_ CSRFTokenService = (*MockCSRFTokenService)(nil)
)

// GenerateCSRFPair creates a new CSRF token pair for the given request.
//
// Takes w (http.ResponseWriter) which is the response writer for setting cookies.
...
//
// Returns (CSRFPair, error), or (CSRFPair{}, nil) if GenerateCSRFPairFunc is nil.
func (m *MockCSRFTokenService) GenerateCSRFPair(w http.ResponseWriter, r *http.Request, buffer *bytes.Buffer) (security_dto.CSRFPair, error) {
	m.GenerateCSRFPairCallCount.Add(1)
...
func (m *MockCSRFTokenService) ValidateCSRFPair(r *http.Request, rawEphemeralTokenFromRequest string, actionToken []byte) (bool, error) {
	m.ValidateCSRFPairCallCount.Add(1)
