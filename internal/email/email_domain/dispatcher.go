...
const (
	...
	// defaultInitialDelay is the starting wait time before retrying a failed email.
	defaultInitialDelay = 5 * time.Second

	// defaultMaxDelay is the maximum wait time between retry attempts.
	defaultMaxDelay = 5 * time.Minute

	// defaultBackoffFactor is the multiplier for retry delays in exponential backoff.
	...
	// logFieldDeadLetterQueue is the log field key for the dead letter queue setting.
	...
	// logFieldRetryWorkerCount is the log field key for the number of retry workers.
...
// deadLetterContext holds details about an email that has failed for good and cannot be
// sent.
...
type failureMeta struct {
	// firstAttempt is the timestamp of the initial send attempt for this email; zero value
	// means no prior attempt has been recorded.
	firstAttempt time.Time
...
// EmailDispatcher implements EmailDispatcherPort and provides batched email sending with
// retry, dead-letter queue, and circuit breaker capabilities.
type EmailDispatcher struct {
	// clock provides time operations for scheduling and timestamps.
	clock clock.Clock

	// startTime records when the dispatcher was started; zero value means not running.
	startTime time.Time
	...
	// circuitBreaker provides fault tolerance by stopping calls to the email provider when
	// repeated failures occur; nil disables circuit breaking.
	...
	// retryConfig holds settings for retry behaviour including maximum attempts, initial
	// delay between retries, and dead letter queue handling.
	...
	// retryWorkerCount is the number of parallel goroutines that process retries.
...
// NewEmailDispatcher creates a new EmailDispatcher with the given provider, dead letter
...
// Takes config (*email_dto.DispatcherConfig) which sets retry behaviour and queue sizes.
//
...
//
// Spawns one goroutine for main processing, one for retry production, and N goroutines
// for retry workers. All goroutines run until Stop is called.
...
//
// Safe for concurrent use. Waits for all pending work to finish and persists the retry
...
//
// It respects the context for cancellation or deadlines, returning an error if the
// context is done before the item can be queued.
//
...
//
// Takes interval (time.Duration) which specifies the new flush interval. Values of zero
// or less are ignored.
//
// Safe for concurrent use.
...
//
// Returns int which is the count of emails queued for retry or manual review.
...
//
// Returns int which is the count of emails queued for retry.
...
// retryProducerLoop manages the retry priority queue as a producer. It wakes up when the
// next item is due, collects all ready items, and pushes them onto a channel for the
// retry workers to consume.
...
//
// Safe for concurrent use. Items that cannot be dispatched due to shutdown are returned
// to the retry heap.
...
// retryWorker reads retry jobs from the queue and processes them. Several workers run at
// the same time to handle retries more quickly.
//
...
//
// Returns time.Duration which is the time to wait, or zero if an item is ready to process
// now.
//
...
// collectReadyRetryItems removes all items from the heap that are ready to retry.
//
...
// processRetryItem attempts to send a single retried email and handles the outcome.
//
...
// handleFailedEmail processes a failed email by deciding whether to retry or send it to
// the dead-letter queue. It acts as a high-level dispatcher for failure handling.
//
...
//
// Takes dlqCtx (*deadLetterContext) which holds the failed email and retry details.
...
// processingLoop runs the main loop that batches and sends emails from the queue. It acts
// as a state machine, passing actions to helper methods.
...
// processNewEmail adds an email to the batch and sends it when the batch is full.
//
...
//
// Returns []*email_dto.SendParams which is the updated batch, or an empty batch if the
// batch was sent.
...
//
// Returns []*email_dto.SendParams which is a new empty batch if emails were sent, or the
// same batch if it was already empty.
...
// drainQueueAndFlush empties all queued emails and sends any remaining partial batch.
// Used for manual flush operations to ensure predictable behaviour.
//
...
//
// Takes batch ([]*email_dto.SendParams) which contains any pending emails to send before
// draining the queue.
...
// sendBatch sends a batch of emails, using bulk sending if the provider supports it.
// Falls back to sending emails one at a time if bulk sending fails.
//
	...
	_ = l.RunInSpan(ctx, "EmailDispatcher.sendBatch", func(spanCtx context.Context, spanLog logger_domain.Logger) error {
		...
		if d.provider.SupportsBulkSending() {
			if err := d.provider.SendBulk(spanCtx, batch); err != nil {
				...
				d.totalSuccessful.Add(int64(len(batch)))
				d.totalProcessed.Add(int64(len(batch)))
			}
		} else {
			d.sendBatchIndividually(spanCtx, batch)
		}
...
// sendWithCircuitBreaker wraps the provider's Send method with circuit breaker logic and
// records metrics.
//
...
// sendBatchIndividually sends each email in the batch one at a time. Failed emails are
// passed to handleFailedEmail for retry handling.
//
// Takes batch ([]*email_dto.SendParams) which contains the emails to send.
...
// persistRetryQueueOnShutdown moves all pending retry items to the dead-letter queue to
...
// applyDispatcherConfigDefaults fills in missing or invalid fields in the config with
// sensible default values.
//
// Takes config (*email_dto.DispatcherConfig) which holds the dispatcher settings to fill
// in.
