...
// adapter implements email_domain.AssetResolverPort by delegating to the registry
// service. It fetches assets, applies the requested transformation profile, and returns
// them as ready-to-embed email attachments with CID set.
type adapter struct {
...
var (
	_ email_domain.AssetResolverPort = (*adapter)(nil)
)

// ResolveAsset fetches and transforms a single asset based on the request.
//
// Takes request (*email_dto.EmailAssetRequest) which specifies the asset to resolve and
// any transformation options.
//
// Returns *email_dto.Attachment which contains the resolved asset ready for email
// attachment.
// Returns error when the artefact cannot be fetched, no suitable variant is available, or
// the variant content cannot be retrieved.
...
//
// Takes requests ([]*email_dto.EmailAssetRequest) which specifies the assets to resolve.
//
...
//
// Takes variants ([]registry_dto.Variant) which contains the available variants to choose
// from.
// Takes request (*email_dto.EmailAssetRequest) which specifies the desired profile,
// width, and density.
//
...
// findBestMatchingVariant selects the most suitable variant for an email asset request
// using a step-by-step fallback strategy.
//
// Takes variants ([]registry_dto.Variant) which contains the available image variants to
// choose from.
// Takes request (*email_dto.EmailAssetRequest) which specifies the desired profile,
// width, and density.
//
// Returns *registry_dto.Variant which is the best matching variant, or nil if variants is
// empty.
// Returns string which describes the fallback used, or empty if an exact match is found.
//
// Fallback strategy: 1. Exact match: profile + width + density (if all specified). 2.
// Profile + width: ignore density mismatch. 3. Profile only: ignore width and density
// mismatches. 4. Width + density: ignore profile (use any variant with matching size). 5.
// Source variant: the original unmodified file. 6. Any available variant: last resort
...
//
// Takes registryService (registry_domain.RegistryService) which provides the underlying
// registry operations for asset resolution.
//
// Returns email_domain.AssetResolverPort which is the configured adapter ready for use.
...
//
// Takes request (*email_dto.EmailAssetRequest) which holds the asset details to convert
...
// logVariantSelection logs details about the chosen variant and any fallback that was
// used.
//
...
// Takes fallbackUsed (string) which explains why a fallback was needed, or is empty if no
// fallback was used.
// Takes requestedProfile (string) which is the profile that was first asked for.
...
// extractFilename gets the filename from a source path. For example,
// "assets/images/logo.png" returns "logo.png".
//
// Takes sourcePath (string) which is the full path to get the filename from.
//
// Returns string which is the filename after the last slash, or the full path if no slash
// is found.
...
// findExactMatch searches for a variant that matches profile, width, and density exactly.
//
// Takes variants ([]registry_dto.Variant) which is the list of variants to search
// through.
// Takes request (*email_dto.EmailAssetRequest) which specifies the profile, width, and
// density to match.
//
// Returns *registry_dto.Variant which is the first matching variant, or nil if no exact
// match is found.
...
// findProfileAndWidthMatch finds a variant that matches the profile and width, ignoring
// density. This only applies when density was requested.
//
// Takes variants ([]registry_dto.Variant) which provides the list of variants to search.
// Takes request (*email_dto.EmailAssetRequest) which specifies the profile and width to
// match.
//
// Returns *registry_dto.Variant which is the first matching variant, or nil if no match
// is found or density was not requested.
...
// findProfileOnlyMatch searches for a variant that matches only the profile, without
// checking dimensions. This is used as a fallback when width or density was requested but
// no exact match was found.
//
// Takes variants ([]registry_dto.Variant) which contains the available variants to search
// through.
// Takes request (*email_dto.EmailAssetRequest) which specifies the profile and dimension
// criteria.
//
// Returns *registry_dto.Variant which is the first matching variant, or nil if no match
// is found or no dimensions were requested.
...
// findDimensionMatch searches for a variant that matches the requested width and density,
// without checking the profile. This only runs if a profile was originally requested.
//
// Takes variants ([]registry_dto.Variant) which contains the image variants to search
// through.
// Takes request (*email_dto.EmailAssetRequest) which specifies the desired width,
// density, and profile values.
//
// Returns *registry_dto.Variant which is the first matching variant, or nil if no match
// is found or if no profile was requested.
...
//
// Returns bool which is true if width is zero or the variant's width tag matches the
// requested width in either "Npx" or "N" format.
...
// variantMatchesDensity checks if a variant's density matches the requested density.
//
// Takes variant (*registry_dto.Variant) which is the variant to check.
// Takes density (string) which is the requested density to match against.
//
// Returns bool which is true if density is empty or matches the variant's density tag.
