...
// LifecycleService is the primary driving port for the lifecycle hexagon. It handles the
// build-to-runtime bridge: file watching, build notifications, asset pipeline
// orchestration, and router hot-reload coordination.
type LifecycleService interface {
	// Start begins lifecycle management: file watching and build notification handling. It
	// runs in the background and returns straight away after starting goroutines.
	//
	...
	// RunInitialTasks runs one-time startup tasks such as asset seeding and config loading.
	// Call this after Start to perform the first asset discovery.
	//
	// Returns error when a startup task fails.
	RunInitialTasks(ctx context.Context) error

	// GetEntryPoints returns the current set of discovered entry points. The coordinator
	// uses this to know what to build.
	//
...
// implements lifecycle_domain.FileSystemWatcher and io.Closer.
type FileSystemWatcher interface {
	// Watch begins watching the specified directories for file changes.
	//
	// Takes recursiveDirs ([]string) which lists directories to watch recursively, including
	// all subdirectories.
	// Takes nonRecursiveDirs ([]string) which lists directories to watch at the top level
	// only.
	//
	// Returns <-chan lifecycle_dto.FileEvent which yields file change events as they occur.
	...
	// UpdateWatchedFiles adds or removes files from the watch list. Used to track asset
	// files found during builds.
	//
...
// RouterReloadNotifier is a driven port that the lifecycle uses to tell the daemon to
// reload routes after a build finishes. This is typically provided by the daemon's
// RouterManager.
...
// InterpretedBuildOrchestrator handles JIT compilation for interpreted mode, converting
// build artefacts into a runnable InterpretedManifestRunner. This is only used in dev-i
// mode and should be nil in compiled or production modes.
type InterpretedBuildOrchestrator interface {
	// BuildRunner creates a new manifest runner from scratch. This is called on the first
	// build or when a full rebuild is needed.
	//
	// Takes result (*annotator_dto.ProjectAnnotationResult) which contains the parsed
	// project data.
	//
	...
	// MarkComponentsDirty for targeted rebuilds where only a subset of components is present
	// in the result.
	//
	...
	// result into the existing manifest rather than replacing it. This is used by targeted
	// rebuilds where the result only contains the changed component and its dependents.
	//
	// Takes result (*annotator_dto.ProjectAnnotationResult) which contains the changed
	// components to mark.
	//
	...
	// IsInitialised reports whether the initial runner has been created. Callers use the
	// result to tell the first build apart from later updates.
	//
	// Returns bool which is true if the runner has been created, false otherwise.
	IsInitialised() bool

	// GetAffectedComponents returns all component paths that transitively depend on the
	// given component, found by BFS through the reverse dependency map. Used to determine
	// which components need rebuilding when a partial changes.
	//
	// Takes relPath (string) which is the relative path of the changed component.
	//
	// Returns []string which contains the relative paths of all affected components.
...
// when a new runner is created after a build in interpreted mode.
...
// BuildCacheInvalidator triggers a rebuild in interpreted mode. The manifest runner
// implements BuildCacheInvalidator when it supports cache invalidation.
...
// AssetPipelinePort processes build results to create transformation profiles. It turns
// asset requirements from the build manifest into instructions for the registry.
type AssetPipelinePort interface {
	// ProcessBuildResult processes a project annotation result from the build.
	//
	// Takes result (*annotator_dto.ProjectAnnotationResult) which contains the build
...
// RenderRegistryCachePort provides cache clearing for render registries. It is used by
// lifecycle to clear caches when assets such as SVG files change.
...
type BuilderAdapter interface {
	...
	// (context cancelled, flush timeout).
	RunBuild(ctx context.Context) (*BuildResult, error)
}

// BridgeWithCounter provides access to the count of processed artefact events. It is used
// to detect when a pipeline flush has finished.
...
// DevEventNotifier sends browser-visible notifications when dev builds complete. It is
// only wired in dev/dev-i modes; nil in production.
type DevEventNotifier interface {
	// NotifyRebuildComplete broadcasts a build-completion event to connected browsers.
	//
	// The affected paths are project-relative component paths (e.g. "pages/login.pk") which
	// the implementation converts to URL route patterns.
	//
	// Takes ctx (context.Context) which carries logging context.
	// Takes affectedPaths ([]string) which lists the components that were rebuilt.
