...
const (
	...
	// profileTranspiledJS is the profile name for TypeScript-to-JavaScript transpilation
	// output.
	...
	// profileSliceCapacity is the default pre-allocation capacity for profile slices during
	// build profile construction.
...
var (
	...
	extensionProfiles = map[string]profileBuilder{
...
// Takes dependsOn (string) which specifies a dependency profile name, or empty for no
// dependency.
// Takes tags (map[string]string) which provides key-value pairs for the resulting tags.
// Takes params (map[string]string) which provides key-value configuration parameters.
//
// Returns registry_dto.NamedProfile which is the constructed profile ready for use in
// component and compression chain building.
...
// buildStaticAssetProfiles creates profiles for static assets that only need compression.
//
// Takes ctx (profileContext) which provides the file extension and other context for
// building the profile.
//
// Returns []registry_dto.NamedProfile which contains the compression profiles for the
// static asset.
...
//
// Returns []registry_dto.NamedProfile which holds the minify and compress chain settings
// for SVG files.
...
//
// Returns []registry_dto.NamedProfile which contains the minify and compress chain for
// CSS processing.
...
// isPreMinified reports whether a file name indicates the asset has already been
// minified, such as "foo.min.js" or "bar.min.es.js".
//
// Takes name (string) which is the artefact ID or file path to check.
//
// Returns bool which is true if the file name contains a ".min" segment before the final
// ".js" extension.
...
//
// Takes ctx (profileContext) which provides the artefact context for profile creation.
//
// Returns []registry_dto.NamedProfile which contains the minify and compress chain
// profiles for JavaScript processing.
...
// buildTypeScriptProfiles creates profiles for TypeScript files that need transpilation
// to JavaScript, followed by minification and compression.
//
// Takes ctx (profileContext) which provides the artefact ID and file extension.
//
// Returns []registry_dto.NamedProfile which contains the transpilation profile followed
// by the minify/compress chain.
...
// NormaliseAssetArtefactID replaces transpilable source extensions in an artefact ID with
// the output extension. TypeScript ".ts" becomes ".js" so that the registry entry matches
// the path browsers will request.
//
...
// buildMinifyCompressChain creates a chain of profiles for minifying and then compressing
// an asset with gzip and brotli.
//
// Takes minifyCapability (string) which specifies the capability needed to minify the
// asset.
// Takes dependsOn (string) which specifies the input profile this chain depends on.
...
//
// Returns []registry_dto.NamedProfile which contains the minify and compress profiles for
// the chain.
...
// buildMinifyCompressChainWithPriority creates a minify -> compress chain with custom
// priority.
//
...
// Takes minExt (string) which specifies the file extension for minified files.
// Takes minifyPriority (registry_dto.ProfilePriority) which sets the priority for the
// minify profile.
// Takes compressedType (string) which specifies the type for compressed output.
//
// Returns []registry_dto.NamedProfile which contains the minify profile followed by gzip
// and brotli compression profiles.
...
// buildCompressChain creates a gzip and brotli profile chain from a source profile.
//
// Takes dependsOn (string) which specifies the source profile this chain depends on.
...
// Takes ext (string) which provides the base file extension for compressed outputs.
//
// Returns []registry_dto.NamedProfile which contains the gzip and brotli compression
// profiles.
