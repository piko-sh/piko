...
export async function toggle(event, ...args) {
  return (await __getInstance__(event)).toggle(...args);
}
getGlobalPageContext().setExports({ toggle });
{
  const __s__ = document.querySelector("[partial_name='toggle-button']") ?? document.querySelector("[data-pageid]") ?? document.body;
