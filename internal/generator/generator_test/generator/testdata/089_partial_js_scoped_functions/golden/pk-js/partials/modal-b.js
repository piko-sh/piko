...
async function __createInstance__(__scope__) {
  ...
  function open() {
    const statusEl = document.getElementById("modal-b-status");
    if (statusEl) {
      statusEl.textContent = "Modal B is OPEN!";
      statusEl.classList.add("open");
    }
    console.log("modal-b open() was called");
  }
  return { open };
}
...
{
  const __s__ = document.querySelector("[partial_name='modal-b']") ?? document.querySelector("[data-pageid]") ?? document.body;
