...
const (
	...
	hybridRuntimeImportPath = "piko.sh/piko/wdk/runtime"
)

// tryEmitCollectionCall checks if the expression is a collection call and emits it if so.
//
// Takes expression (ast_domain.Expression) which is the expression to check and
// potentially emit.
//
// Returns goast.Expr which is the emitted Go expression, or nil if not a collection call.
...
// emitDynamicCollectionFetcher creates a call to a generated collection fetcher function.
//
// Handles dynamic collection providers that fetch data at runtime. It takes the blueprint
// from the Collection Service and:
...
//
// Takes dynamicInfo (any) which is a blueprint from collection_dto.DynamicCollectionInfo
// containing the fetcher code.
//
...
//
// Hybrid mode (ISR - Incremental Static Regeneration) combines static generation with
// runtime revalidation. Static generation embeds content at build time for fast initial
// response. Runtime revalidation uses ETag-based staleness checks with background
// refresh.
//
...
//	}
//
// Takes ann (*ast_domain.GoGeneratorAnnotation) which contains the hybrid collection
// settings.
//
...
//
// This is needed because the provider's FetcherFunc AST is reused across many components,
// and each one must be renamed on its own.
//
// When functionDeclaration is nil, returns nil.
//
// Takes functionDeclaration (*goast.FuncDecl) which is the function declaration to copy.
//
...
// buildHybridGetterFunc constructs the AST for a hybrid collection getter function.
//
...
// Takes info (*collection_dto.DynamicCollectionInfo) which provides the provider name,
// collection name, and target type.
//
...
//
// Takes providerLit (goast.Expr) which is the string literal for the provider name.
// Takes collectionLit (goast.Expr) which is the string literal for the collection name.
//
...
// buildRevalidateStmt creates the if-statement that triggers background revalidation when
// needed.
//
// Takes providerLit (goast.Expr) which is the string literal for the provider name.
// Takes collectionLit (goast.Expr) which is the string literal for the collection name.
...
// buildDecodeBlobStmt creates the assignment statement that decodes the collection blob
// into the target type.
//
// Takes targetType (goast.Expr) which is the type parameter for DecodeCollectionBlob.
//
...
//
// Takes targetType (goast.Expr) which is the element type of the returned slice.
//
...
// validateHybridCollectionInfo checks that the annotation has valid DynamicCollectionInfo
// and returns it.
//
// Takes ann (*ast_domain.GoGeneratorAnnotation) which holds the annotation to check.
//
// Returns *collection_dto.DynamicCollectionInfo which contains the collection details
// when valid.
// Returns *ast_domain.Diagnostic which describes the error when the annotation is missing
// DynamicCollectionInfo or has the wrong type.
...
// handleNonHybridMode handles the case where IsHybridCollection is set but HybridMode is
// false.
//
// Takes ann (*ast_domain.GoGeneratorAnnotation) which contains the annotation with hybrid
// collection settings.
//
// Returns goast.Expr which is the static collection literal, or nil if none exists.
// Returns []goast.Stmt which is always nil in this mode.
// Returns []*ast_domain.Diagnostic which contains a warning about falling back to a
// static literal.
...
// hybridMissingLiteralDiagnostic creates an error for a hybrid collection that is missing
// a static literal.
//
// Takes info (*collection_dto.DynamicCollectionInfo) which provides the provider and
// collection names for the error message.
//
