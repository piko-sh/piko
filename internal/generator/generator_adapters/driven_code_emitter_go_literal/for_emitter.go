...
type forEmitter struct {
	...
	// expressionEmitter converts expressions to Go code, stored as interface for pooling.
	expressionEmitter ExpressionEmitter

	// astBuilder provides access to the main AST builder for creating code within loop
	// bodies.
	astBuilder AstBuilder

	// currentPartialScopeID is the hashed name of the current partial used for CSS scoping.
	// It is set during emit and used by generateLoopBody.
...
//
// Takes node (*ast_domain.TemplateNode) which contains the for directive to process.
// Takes parentSliceExpr (goast.Expr) which is the slice expression to append results to.
// Takes partialScopeID (string) which is the HashedName of the current partial for CSS
// scoping.
// Takes mainComponentScope (string) which is the HashedName of the main component being
// generated.
//
// Returns []goast.Stmt which contains the generated loop statements.
// Returns []*ast_domain.Diagnostic which contains any errors encountered during emission.
...
// emitWithExtractedIterable emits a p-for loop using a pre-extracted iterable variable.
//
// This is called when the parent node has already extracted the collection to a variable
// for accurate child slice capacity calculation. Using the pre-extracted variable avoids
// double-evaluation of collection expressions (important for method calls with side
// effects) and redundant code emission.
//
// Takes node (*ast_domain.TemplateNode) which is the template node containing the p-for
// directive.
// Takes parentSliceExpr (goast.Expr) which is the expression for the parent slice
// capacity calculation.
// Takes loopInfo (*LoopIterableInfo) which contains the extracted iterable variable name
...
//
// Takes ctx (context.Context) which carries cancellation and deadline signals for
...
// determineLoopVarNames determines the appropriate variable names for loop index and
// item.
//
// Takes node (*ast_domain.TemplateNode) which is the template subtree to check for
// variable usage.
// Takes forExpr (*ast_domain.ForInExpression) which contains the loop variable
// definitions.
//
// Returns idxVarName (string) which is the index variable name or blank identifier if
// unused.
// Returns itemVarName (string) which is the item variable name or blank identifier if
// unused.
...
// wrapBodyWithConditional wraps loop body statements with a p-if condition if present.
//
// Takes node (*ast_domain.TemplateNode) which contains the template node with optional
// p-if directive.
// Takes bodyStmts ([]goast.Stmt) which contains the loop body statements to wrap.
// Takes bodyDiags ([]*ast_domain.Diagnostic) which contains diagnostics from body
// processing.
// Takes prereqStmts ([]goast.Stmt) which contains prerequisite statements to execute
// before the body.
//
// Returns wrappedBodyStmts ([]goast.Stmt) which contains the body statements, wrapped in
// an if statement when a p-if directive exists.
// Returns updatedPrereqStmts ([]goast.Stmt) which contains the prerequisite statements
// with any condition prerequisites appended.
// Returns allDiags ([]*ast_domain.Diagnostic) which contains all diagnostics including
// any from condition processing.
...
// collectionNeedsNilCheck determines if a collection requires a nil check before
// iteration.
//
// Takes collAnn (*ast_domain.GoGeneratorAnnotation) which specifies the collection
// annotation to check.
//
...
// emitDeterministicMapLoopWithContext creates the loop statements for iterating over a
...
//
// Returns []goast.Stmt which contains the generated loop statements.
// Returns []*ast_domain.Diagnostic which contains any issues found during code
// generation.
//
...
// buildLoopBody generates the Go statements for the body of a for-loop. It detects if any
// partials are invoked within the loop body whose props depend on the loop's variables
// and generates the Render() calls for those partials inside the loop, ensuring the loop
// variables are in scope.
//
// Takes originalNode (*ast_domain.TemplateNode) which is the template node being
// processed.
// Takes forExpr (*ast_domain.ForInExpression) which contains the for-loop expression
// details.
// Takes parentSliceExpr (goast.Expr) which is the slice expression being iterated over.
//
// Returns []goast.Stmt which contains the generated loop body statements.
// Returns []*ast_domain.Diagnostic which contains any diagnostics from code generation.
...
// generateLoopDependentPartialCalls finds and generates render calls for loop-dependent
// partials.
//
// Takes originalNode (*ast_domain.TemplateNode) which is the template node containing the
// loop to search for partial invocations.
//
// Returns []goast.Stmt which contains the generated render call statements.
// Returns []*ast_domain.Diagnostic which contains any diagnostics from sorting or
// generating the partial calls.
...
//
// Takes invocation (*annotator_dto.PartialInvocation) which provides the details of the
// partial to render.
//
...
// Takes originalNode (*ast_domain.TemplateNode) which is the template node being
// processed.
// Takes forExpr (*ast_domain.ForInExpression) which contains the for-in expression to
// check.
// Takes parentSliceExpr (goast.Expr) which is the parent slice expression for adding
// results.
//
// Returns []goast.Stmt which contains the loop body statements.
// Returns []*ast_domain.Diagnostic which contains any errors found during code creation.
...
//
// Returns *ast_domain.TemplateNode which is the cloned node without loop or conditional
// directives but with the original children preserved.
...
// Takes forExpr (*ast_domain.ForInExpression) which is the for loop expression context.
//
...
//
// Takes originalNode (*ast_domain.TemplateNode) which is the template node to hoist.
// Takes parentSliceExpr (goast.Expr) which is the slice to append results to.
//
// Returns []goast.Stmt which contains the loop body statements.
// Returns []*ast_domain.Diagnostic which contains any diagnostics from static node
// registration.
...
// buildMapLoopBody creates the statements for the loop body when iterating over a map
// with sorted keys.
//
// Takes node (*ast_domain.TemplateNode) which is the template node to process.
// Takes forExpr (*ast_domain.ForInExpression) which holds the loop variable details.
...
//
// Returns []goast.Stmt which contains the key extraction and sorting statements. Sorting
// is only added when keys are strings.
...
// isInvocationLoopDependentRecursive checks if an invocation depends on loop variables.
// This can be direct (through its props) or indirect (through its dependencies, such as a
// nested partial whose parent is inside a p-for loop).
//
// Takes invocation (*annotator_dto.PartialInvocation) which is the invocation to check.
// Takes invocationsByKey (map[string]*annotator_dto.PartialInvocation) which maps
// invocation keys to their invocations for looking up dependencies.
// Takes visited (map[string]bool) which tracks invocation keys that have already been
// checked to prevent infinite recursion in circular dependencies.
//
// Returns bool which is true if any passed prop depends on a loop variable, or if any
// dependency depends on a loop variable.
...
// subtreeDependsOnLoopVars checks if a template node or any of its children use variables
// from a given for-in loop.
//
...
//
// Returns bool which is true if any part of the subtree uses the loop variables.
...
// subtreeContainsForLoops checks if a node or any of its children contain a p-for
// directive.
//
// Used to stop hoisting of nodes that contain loops inside them, such as from expanded
// partials. Hoisting such nodes would cause problems with dynamic key expressions.
//
// Takes node (*ast_domain.TemplateNode) which is the root node to check.
//
...
// visitDirectiveExpressions visits expressions from structural and display directives on
// a template node.
//
// Takes node (*ast_domain.TemplateNode) which contains the directive fields to check.
// Takes visit (func(...)) which is called for each non-nil directive expression found.
...
// visitDynamicContentExpressions visits each expression in the dynamic attributes and
// rich text parts of a template node.
//
// Takes node (*ast_domain.TemplateNode) which is the template node to visit.
// Takes visit (func(...)) which is called for each expression found.
...
// visitEventExpressions visits expressions from event handlers, including both standard
// and custom events.
//
...
// visitEventsMap iterates over a map of event handlers and calls the visit function for
// each expression found.
//
// Takes eventsMap (map[string][]ast_domain.Directive) which contains event handlers
...
//
// Takes node (*ast_domain.TemplateNode) which contains the bind directives to visit.
...
// visitIfNotNil calls the visit function with the directive's expression if both the
// directive and its expression are not nil.
//
...
// Takes loopVars (map[string]bool) which holds the loop variable names to look for.
//
// Returns bool which is true if the expression contains any of the loop variables.
...
// visitExpression walks an AST expression tree and calls the visitor function on each
// node.
//
// The visitor is called before visiting child nodes. When expr is nil or visitor returns
...
// visitIndexExpression processes the base and index parts of an index expression.
//
...
// visitCallExpression visits the callee and all arguments of a call expression.
//
...
// visitTemplateLiteral calls the visitor function for each non-literal part of a template
// literal.
//
...
//
// Takes n (*ast_domain.TernaryExpression) which is the ternary expression to visit.
// Takes visitor (func(...)) which is called for each sub-expression.
