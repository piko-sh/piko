...
// computeRelativePath converts an absolute source path to a clean relative path relative
// to the project BaseDir, with no leading "./" prefix.
//
// Takes absPath (string) which is the absolute file path to convert.
//
// Returns string which is the relative path, or the original absolute path if conversion
// fails.
...
// compiler embeds into DWARF debug info, enabling debuggers like Delve to map breakpoints
// and stack traces back to .pk files. When false (default), it emits "// line file:line"
// (with a space) which is a plain comment visible to humans but ignored by the compiler.
//
// Takes node (*ast_domain.TemplateNode) which provides the source location.
//
// Returns goast.Stmt which is an empty statement when the node has no source mapping
// data, or an expression statement with a line directive.
...
// directiveMappingStmt creates a line directive with column precision for a template
// directive (p-for, p-if, p-else-if).
//
// The column allows debuggers to distinguish between multiple directives on the same
// element line. Respects the EnableDwarfLineDirectives config; see formatLineDirective.
//
// Takes node (*ast_domain.TemplateNode) which provides the source file path.
// Takes dir (*ast_domain.Directive) which provides the directive's name location with
// line and column.
//
// Returns goast.Stmt which is an empty statement when the node or directive has no source
// mapping data, or an expression statement with a line directive.
...
//
// When EnableDwarfLineDirectives is true, it emits "//line file:line[:col]" (valid
// DWARF). When false, it emits "// line file:line[:col]" (plain comment). Column is
// included only when col > 0.
//
// Takes relPath (string) which is the relative source file path.
// Takes line (int) which is the source line number.
// Takes col (int) which is the source column number, or 0 to omit column.
//
...
// sourceMappingCommentGroup creates a comment with source location data for attaching to
// declarations.
//
// Takes node (*ast_domain.TemplateNode) which provides the source location.
//
// Returns *goast.CommentGroup which holds the source mapping comment, or nil if the node
