...
// EmitterConfig holds settings for code generation. It is created once per EmitCode call
// and never changed, with field ordering set for memory alignment.
type EmitterConfig struct {
	// SourcePathHasClientScript maps source file paths to whether they contain client-side
	// scripts. When a path is present, its boolean value is returned directly; when absent,
	// the script detection falls back to other methods.
	...
	// EnablePrerendering controls whether static nodes are prerendered to HTML bytes. When
	// false, all nodes use AST fallback rendering.
...
// enables accurate child slice capacity calculation and avoids double-evaluation of
// collection expressions (which could be method calls with side effects).
type LoopIterableInfo struct {
	...
	// CollectionAnn is the type annotation for the collection, used for nil-check decisions
	// and map type detection.
	CollectionAnn *ast_domain.GoGeneratorAnnotation

	// VarName is the generated Go variable name for this loop iterable (e.g., "loopIter_1").
	VarName string
...
// EmitterContext holds state that changes during code generation. Fields are ordered for
// better memory alignment.
type EmitterContext struct {
	// requiredImports maps package paths to their aliases for generated code. The key is the
	// full package path; the value is the alias or empty if none.
	requiredImports map[string]string

	// usedAliases tracks import aliases in use to detect name conflicts. Key: alias name,
	// Value: canonical package path using this alias.
	usedAliases map[string]string

	// customTagsVarName holds the name of the CustomTags variable at package level; set when
	// static variables are built.
	customTagsVarName string

	// fetcherDecls holds the generated functions that fetch collections.
	fetcherDecls []goast.Decl

	// userCodeLineDirectives collects //line directives for user-authored declarations that
	// need to be injected during post-processing.
	...
	// staticAttrVarCtr is an atomic counter for creating unique static attribute slice
	// variable names.
...
//
// Returns *EmitterContext which has empty import and alias maps with all counters set to
// zero.
