...
// NeedsSliceExpansion reports whether the given query requires runtime SQL rewriting for
// slice parameters. This is determined by checking both the emitter strategy and the
// actual SQL text: expansion is only needed when the SQL contains parenthesised ?-style
// placeholders like (?1) that must be expanded to (?, ?, ...) at runtime.
//
// PostgreSQL queries use ANY($1) which accepts native array parameters and never need
// expansion, even when emitted through the database/sql emitter. SQLite and MySQL queries
// use IN (?1) which requires expansion.
//
...
//
// Returns []ast.Stmt which contains the variable definitions and flattening loops.
