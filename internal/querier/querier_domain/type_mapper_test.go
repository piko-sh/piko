...
func TestDefaultMappings(t *testing.T) {
	...
	expectedCategories := []struct {
		...
		{category: querier_dto.TypeCategoryAggregateState, label: "aggregate_state"},
	}
...
func TestResolveOverrideType_CapsArrayDepth(t *testing.T) {
	t.Parallel()

	mapper := NewTypeMapper(&querier_dto.TypeCatalogue{
		Types: map[string]querier_dto.SQLType{},
	})

	override := querier_dto.TypeOverride{SQLTypeName: "text[][][][][][][][]"}

	got := mapper.resolveOverrideType(override)
	...
	assert.LessOrEqual(t, wraps, maxArrayDimensions, "array wraps must not exceed cap")
}
