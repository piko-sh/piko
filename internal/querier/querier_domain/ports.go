...
// domain and external adapters. Includes interfaces for SQL engine parsing, code
// emission, and the main querier service for dependency inversion.
...
// this to parse dialect-specific SQL into the domain's neutral IR.
type EngineParserPort interface {
	// ParseStatements parses a SQL string into a sequence of parsed statements using the
	// engine's native parser.
	//
	...
	// ApplyDDL interprets a parsed DDL statement and returns the mutation it applies to the
	// schema catalogue. Non-DDL statements (DML) are ignored and return a nil mutation
	// without error.
	//
	// Takes statement (querier_dto.ParsedStatement) which is a parsed SQL statement from
	// ParseStatements.
	//
	...
	// layer performs further type resolution and nullability propagation on this result.
	//
	...
	//
	// Returns *querier_dto.RawQueryAnalysis which contains unresolved column and parameter
...
// rules. The type resolver and function resolver depend on this narrow interface rather
// than the full EnginePort.
type EngineTypeSystemPort interface {
	// NormaliseTypeName converts an engine-specific type name and optional modifiers
	// (precision, scale, length) into a structured SQLType.
	//
	// Takes name (string) which is the engine-native type name (e.g. "varchar", "numeric",
	// "int8").
	// Takes modifiers ([]int) which are optional type parameters such as precision, scale,
	// or length.
	//
	...
	// PromoteType selects the wider type when both operands share the same category but
	// differ in precision or width (e.g. int2 vs int8 promotes to int8).
	//
	...
	// CanImplicitCast reports whether a value of the source category can be implicitly cast
	// to the target category according to the engine's casting rules.
	//
	...
	// SupportedExpressions returns the bitmask of SQL expression kinds this engine adapter
	// can emit in expression maps. The type resolver validates expressions against these
	// capabilities and emits diagnostics for unsupported patterns.
	//
	// Returns querier_dto.SQLExpressionFeature which is the bitmask of supported expression
	// features.
	SupportedExpressions() querier_dto.SQLExpressionFeature
}

// EngineDirectivePort provides comment and directive syntax configuration. The directive
// parser depends on this narrow interface for parsing SQL comment directives.
type EngineDirectivePort interface {
	// CommentStyle returns the comment syntax used by this engine's query files. SQL
	// dialects typically return "--"; non-SQL targets (GraphQL, Cypher) return "#" or "//".
	//
	// Returns querier_dto.CommentStyle which describes the line-comment prefix.
	CommentStyle() querier_dto.CommentStyle

	// SupportedDirectivePrefixes returns the set of parameter reference sigils that may
	// appear in directive comment lines for this engine.
	//
	// Returns []querier_dto.DirectiveParameterPrefix which lists the valid prefix styles.
	SupportedDirectivePrefixes() []querier_dto.DirectiveParameterPrefix
}

// EngineCataloguePort provides built-in catalogues and schema metadata. Components that
// need access to the engine's default schema, built-in functions, types, or table-valued
// functions depend on the port.
type EngineCataloguePort interface {
	// DefaultSchema returns the default schema name for this engine dialect. PostgreSQL uses
	// "public", SQLite uses "main", MySQL uses "" (empty).
	//
	// Returns string which is the default schema name.
	DefaultSchema() string

	// BuiltinFunctions returns the engine's catalogue of built-in functions with their
	// signatures, overloads, return types, and nullability behaviour.
	//
	// Returns *querier_dto.FunctionCatalogue which maps function names to their overloaded
	// signatures.
	BuiltinFunctions() *querier_dto.FunctionCatalogue

	// BuiltinTypes returns the engine's catalogue of built-in SQL types, mapping
	// engine-specific type names to structured SQLType values.
	//
...
// EngineMetadataPort provides engine identity and capability flags consumed by external
// adapters such as the code emitter.
type EngineMetadataPort interface {
	// ParameterStyle returns the parameter placeholder style for this engine. PostgreSQL
	...
	// SupportsReturning reports whether the engine supports RETURNING clauses on INSERT,
...
// embedding all engine sub-interfaces.
...
// CatalogueFunctionResolverPort is an optional extension interface for resolving
// user-defined functions returning composite or set-of types from the catalogue.
...
// FunctionResolverPort is an optional extension interface that engine adapters can
// implement to handle context-dependent or polymorphic function resolution that the
// standard overload resolution cannot match. Examples include BigQuery's ANY_VALUE,
// Oracle's DECODE, and polymorphic functions like ARRAY_AGG where the return type depends
// on the argument types.
type FunctionResolverPort interface {
	// ResolveFunctionCall resolves a function call that the standard overload resolution
...
// MultiStatementAnalyserPort is an optional extension interface that engine adapters can
// implement to handle multi-statement query blocks where earlier statements provide setup
// (temp tables, variable assignments) for the primary query. If an engine does not
// implement the extension, only the last statement in the block is analysed.
type MultiStatementAnalyserPort interface {
	// AnalyseMultiStatement analyses a sequence of statements as a single logical query
	// block, accumulating scope from setup statements into the final analysis result.
	//
...
// ExtensionLoaderPort is an optional extension interface for providing function
...
// CatalogueProviderPort defines the contract for building a schema catalogue. The default
...
// CodeEmitterPort defines the outbound adapter contract for generating source code from
// analysed queries.
type CodeEmitterPort interface {
	// EmitModels generates source code for table model structs and enum types derived from
	// the schema catalogue.
	//
	// Takes packageName (string) which is the target package name for the generated code.
	...
	// EmitQueries generates source code for query methods, parameter structs, and result
	// structs from analysed queries.
	//
	// Takes packageName (string) which is the target package name for the generated code.
	...
	// EmitQuerier generates the top-level Queries struct, constructor, WithTx helper, and
	// DBTX interface. The exact shape of the generated scaffold depends on the emitter
	...
	// Takes queries ([]*querier_dto.AnalysedQuery) which are the analysed queries used to
	// determine which SQL constants to eagerly prepare.
	//
	...
	// EmitOTel generates the otel.go file containing the QueryNameResolver function that
	// maps SQL query constant text to human-readable operation names for OpenTelemetry
	// instrumentation.
	//
	// Takes packageName (string) which is the target package name.
...
// This allows the domain to remain independent of the filesystem and enables testing with
...
// MigrationServicePort defines the driving port for database migration operations. It
...
// MigrationExecutorPort defines the database-specific operations needed by the migration
// service. Each engine adapter (SQLite, PostgreSQL) provides an implementation handling
// locking, transaction wrapping, and SQL execution against the target database.
type MigrationExecutorPort interface {
	...
	// or down migration.
	// Takes useTransaction (bool) which controls whether the migration runs inside a
...
// SeedServicePort defines the driving port for database seed operations. Seeds are
// forward-only SQL files applied after migrations, tracked in a history table for
// idempotency.
...
//
// Seeds use a dialect-specific advisory lock keyed separately from the migration lock so
// concurrent replica boots cannot duplicate seed applications. Idempotent INSERT
// semantics on the history table provide a belt-and-braces guarantee even when the lock
...
type SeedExecutorPort interface {
	...
	//
	// The lock must use a key distinct from the migration lock so the two surfaces serialise
