...
const (
	...
	// ColourSpaceCMYK is the cyan/magenta/yellow/key (black) colour space used in print.
	ColourSpaceCMYK
)

const (
	// colourParseMinRGBParts is the minimum number of comma-separated parts in an rgb()
	// value.
	colourParseMinRGBParts = 3

	// colourParseRGBAPartCount is the expected number of comma-separated parts in an rgba()
	// value.
	colourParseRGBAPartCount = 4

	// colourParseMinHSLParts is the minimum number of comma-separated parts in an hsl()
	// value.
	colourParseMinHSLParts = 3

	// colourParseHSLAPartCount is the expected number of comma-separated parts in an hsla()
	// value.
	colourParseHSLAPartCount = 4

	// colourHexShortLength is the character count of a short-form hex colour string (#RGB).
	colourHexShortLength = 3

	// colourHexFullLength is the character count of a full hex colour string (#RRGGBB).
	colourHexFullLength = 6

	// colourHexWithAlphaLength is the character count of a hex colour string with alpha
	// (#RRGGBBAA).
	colourHexWithAlphaLength = 8

	// colourMaxChannel is the maximum value of an 8-bit colour channel.
	colourMaxChannel = 255.0

	// colourPercentDivisor is the divisor for converting a percentage value to a 0-1
	// fraction.
	colourPercentDivisor = 100.0

	// hslHueDegrees is the full rotation of the HSL hue wheel in degrees.
	hslHueDegrees = 360.0

	// hslSectorSize is the number of degrees per sector in the HSL hue wheel.
	hslSectorSize = 60.0
...
//
// Takes hue (float64) which is the hue angle in degrees (0-360).
// Takes saturation (float64) which is the saturation fraction (0-1).
// Takes lightness (float64) which is the lightness fraction (0-1).
// Takes alpha (float64) which is the opacity value (0-1).
//
...
//
// Returns a formatted string such as "rgb(0.50, 0.50, 0.50)" or "cmyk(0.00, 0.00, 0.00,
// 1.00)".
...
// ParseColour parses a CSS colour string into a Colour. Supports named colours, hex
// (#RGB, #RRGGBB, #RRGGBBAA), rgb(), rgba(), hsl(), and hsla() functional notation.
//
// Takes value (string) which is the CSS colour string to parse.
//
// Returns the parsed Colour and true on success, or ColourBlack and false when the string
// is not recognised.
...
//
// Takes value (string) which is the hex string including the leading "#".
//
// Returns the parsed Colour and true on success, or ColourBlack and false for invalid hex
// lengths.
...
// parseHexByteValue converts a two-character hex string to a normalised 0-1 float64
// channel value.
//
// Takes hexPair (string) which is the two-character hex string to convert.
//
// Returns the normalised channel value, or 0 on parse failure.
...
// parseRGBColourValue parses an rgb() or rgba() functional notation string into a Colour.
//
// Takes value (string) which is the CSS rgb/rgba string to parse.
//
// Returns the parsed Colour and true on success, or ColourBlack and false when too few
// parts are present.
...
// parseHSLColourValue parses an hsl() or hsla() functional notation string into a Colour.
//
// Takes value (string) which is the CSS hsl/hsla string to parse.
//
// Returns the parsed Colour and true on success, or ColourBlack and false when parsing
// fails.
...
// parseColourChannelValue parses a colour channel value from either a percentage string
// or a 0-255 integer string.
//
// Takes value (string) which is the channel value to parse.
//
// Returns the normalised 0-1 channel value, or 0 on parse failure.
...
// parsePercentageValue parses a percentage string into a 0-1 fraction.
//
// Takes value (string) which is the percentage string to parse, with or without a
// trailing "%" sign.
//
...
var (
	// namedColours maps CSS named colour keywords to their Colour values.
	//
	//nolint:revive // CSS spec colours
	namedColours = map[string]Colour{
