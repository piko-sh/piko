...
type FontDescriptor struct {
	...
	// Weight is the CSS font-weight value (100-900). 400 is normal, 700 is bold.
	Weight int
...
// String returns a human-readable representation of the font descriptor.
//
...
// FontMetrics holds the vertical metrics for a font at a given size, all in points.
type FontMetrics struct {
	// Ascent is the distance from the baseline to the top of the tallest glyph.
	Ascent float64

	// Descent is the distance from the baseline to the bottom of the lowest glyph (typically
	// negative in some systems, but stored as a positive value here).
	Descent float64
	...
	// XHeight is the height of lowercase letters (e.g. 'x') above the baseline.
	XHeight float64
...
type GlyphPosition struct {
	...
	// XAdvance is the horizontal distance the pen advances after drawing this glyph.
	XAdvance float64

	// GlyphID is the font-internal glyph identifier.
	GlyphID uint16

	// ClusterIndex is the index of the first input rune that this glyph corresponds to. When
	// HarfBuzz produces ligatures, multiple input runes map to a single glyph; ClusterIndex
	// identifies which runes.
	ClusterIndex int

	// RuneCount is the number of input runes consumed by this glyph cluster.
	RuneCount int
}
