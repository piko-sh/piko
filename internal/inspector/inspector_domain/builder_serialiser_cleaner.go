...
// This file is dedicated to a very specific and critical workaround: "cleaning" the
// internal representation of types that the go/types library produces for methods on
// instantiated generic types.
//
// The Problem: The go/types library can represent the type `string` inside a method
// signature like `func (b Box[string]) Method() string` as a special *types.TypeParam
// object whose String() method returns "string/* type parameter */".
//
// The Solution: This code detects this annotation, extracts the clean type string (e.g.,
// "string"), and then uses the official go/parser and the full type-checking context to
// resolve that string back into its proper, canonical *types.Type. This approach handles
// primitives, named types, and complex composite types.
...
var (
	// astResolverPool reuses astResolver instances to reduce allocation pressure during
	// generic type cleaning.
	astResolverPool = sync.Pool{
		New: func() any {
			return &astResolver{}
		},
	}
...
// cleaningContext holds the data needed to resolve a type string back to a types.Type
// within a specific file's scope.
type cleaningContext struct {
	// aliasToPath maps import aliases to their full import paths for the file being
	// processed.
...
// resolve is the main dispatcher for the astResolver. It delegates to specific methods
// based on the AST node type.
//
// Takes e (ast.Expr) which is the expression node to resolve.
//
// Returns types.Type which is the resolved type, or nil if the node type is not
// supported.
...
//
// Returns types.Type which is the resolved type, or nil if the selector cannot be
// resolved.
...
//
// Returns types.Type which is the pointer type, or nil if the element type cannot be
// resolved.
...
//
// Returns types.Type which is a slice type if the node has no length, an array type with
// length zero if a length is present, or nil if the element type cannot be resolved.
...
//
// Takes ctx (*cleaningContext) which provides the cleaning context for the resolver.
//
...
// cleanAnnotatedSignature walks through a signature and cleans any annotated type
...
//
// Returns *types.Signature which is the cleaned signature, or the original if no changes
// were needed.
...
// cleanAnnotatedTuple cleans each type within a tuple of function parameters or results.
//
...
// cleanAnnotatedType cleans a type by resolving annotated type parameters or passing
// composite types to their respective cleaners.
//
// Takes typ (types.Type) which is the type to clean.
// Takes ctx (*cleaningContext) which provides the cleaning state and cache.
//
// Returns types.Type which is the cleaned type, or the original if no cleaning is needed.
...
// cleanCompositeType picks and runs the correct cleaner for a composite type. Splits the
// logic of cleanAnnotatedType into smaller parts.
//
// Takes typ (types.Type) which is the type to clean.
// Takes ctx (*cleaningContext) which tracks visited types and cleaning state.
//
// Returns types.Type which is the cleaned type, or the original if the type is not a
// handled composite type.
...
// cleanPointer cleans the element type of a pointer and returns a new pointer if the
// element was changed.
//
// Takes t (*types.Pointer) which is the pointer type to clean.
// Takes ctx (*cleaningContext) which provides the cleaning context.
//
// Returns types.Type which is a new pointer if the element changed, or the original
// pointer if unchanged.
...
//
// Returns types.Type which is the original slice if unchanged, or a new slice with a
// cleaned element type.
...
// cleanArray returns a cleaned copy of the array type if its element type needs cleaning,
// otherwise returns the original array.
//
// Takes t (*types.Array) which is the array type to clean.
// Takes ctx (*cleaningContext) which provides the cleaning settings.
//
// Returns types.Type which is either a new array with a cleaned element type or the
// original array if no cleaning was needed.
...
// Takes ctx (*cleaningContext) which holds the cleaning state.
//
// Returns types.Type which is the original map if unchanged, or a new map with cleaned
...
// Takes ctx (*cleaningContext) which tracks the cleaning state.
//
// Returns types.Type which is a new channel with a cleaned element type, or the original
...
// cleanNamed removes type annotations from the type arguments of a named generic type.
//
...
// Takes ctx (*cleaningContext) which provides the cleaning context.
//
// Returns types.Type which is the cleaned type with annotations removed from its type
// arguments.
...
// extractBaseTypeName extracts the base type name from a type parameter. It removes any
// trailing annotation comments from the type string.
//
...
// resolveCleanedTypeFromString parses a type string and returns the matching Go type. It
// uses the AST resolver to handle parsing and type lookup.
//
