...
func (m *MarkdownProvider) rewriteAssetNode(
	...
	case ast_domain.NodeRawHTML:
		...
		rewritten, changed := rewriteRelativeURLs(ctx, rewriteParams{
			Sandbox:        sandbox,
			Registrar:      m.assetRegistrar,
			Analyser:       analyser,
...
func extractSlugOverride(fm map[string]any, collectionName, defaultURL string) slugOverride {
	...
	slugVal, ok := fm["slug"].(string)
	if !ok || slugVal == "" {
		return override
	}
	cleaned := sanitiseRelativePath(slugVal)
	cleaned = capSlugBytes(cleaned)
	if cleaned == "" {
		return override
	}
	override.slug = cleaned
	override.url = "/" + collectionName + "/" + cleaned
	return override
}
