...
func TestRewriteRelativeURLs_ImgDoubleQuoted(t *testing.T) {
	...
	result, changed := rewriteRelativeURLs(context.Background(), rewriteParams{
		Sandbox:        sandbox,
		Registrar:      registrar,
		Analyser:       nil,
		MdDirectory:    "tutorials",
		CollectionName: "docs",
		MdRelativePath: "tutorials/foo.md",
	}, raw)
...
func TestRewriteRelativeURLs_ImgSkipsAbsoluteURLs(t *testing.T) {
	...
	result, changed := rewriteRelativeURLs(context.Background(), rewriteParams{
		Sandbox:        newSandboxForRewriter(t, nil),
		Registrar:      registrar,
		Analyser:       nil,
		MdDirectory:    "",
		CollectionName: "docs",
...
func TestRewriteRelativeURLs_CancelledContext(t *testing.T) {
	...
	cancel()
	raw := `<img src="one.svg"/>`
	result, changed := rewriteRelativeURLs(ctx, rewriteParams{
		Sandbox:        sandbox,
		Registrar:      registrar,
		Analyser:       nil,
		MdDirectory:    "",
...
func TestRewriteRelativeURLs_NilRegistrarAndAnalyser(t *testing.T) {
	...
	result, changed := rewriteRelativeURLs(context.Background(), rewriteParams{
		Sandbox:        newSandboxForRewriter(t, nil),
		Registrar:      collection_domain.AssetRegistrar(nil),
		Analyser:       nil,
		MdDirectory:    "",
		CollectionName: "docs",
		MdRelativePath: "foo.md",
...
func TestRewriteRelativeURLs_NilRegistrarOnlyAnchorsRewrite(t *testing.T) {
	...
	result, changed := rewriteRelativeURLs(context.Background(), rewriteParams{
		...
		Registrar:      nil,
		Analyser:       analyser,
		MdDirectory:    "get-started",
		CollectionName: "docs",
		MdRelativePath: "get-started/install.md",
	}, raw)
	require.True(t, changed)
...
func TestRewriteRelativeURLs_BlobExceedsSizeCap(t *testing.T) {
	t.Parallel()

	registrar := &stubAssetRegistrar{}
	sandbox := newSandboxForRewriter(t, nil)
	oversize := strings.Repeat("a", maxRawHTMLBytes+1)
	...
	result, changed := rewriteRelativeURLs(context.Background(), rewriteParams{
		...
		MdDirectory:    "",
		CollectionName: "docs",
		MdRelativePath: "huge.md",
	}, oversize)

	assert.False(t, changed)
	assert.Equal(t, oversize, result, "result must equal input when blob exceeds cap")
	registrar.mu.Lock()
	calls := len(registrar.calls)
	registrar.mu.Unlock()
	assert.Zero(t, calls, "registrar must not be invoked when blob exceeds cap")
}

func TestRewriteRelativeURLs_AcceptsLeadingDotsFilename(t *testing.T) {
	t.Parallel()
	...
	require.True(t, changed, "filename starting with two dots must not be rejected as path traversal")
	assert.Contains(t, result, "/_piko/assets/..hidden.svg")
}
...
func TestRewriteRelativeURLs_MaxReplacements(t *testing.T) {
	...
	_, changed := rewriteRelativeURLs(context.Background(), rewriteParams{
		Sandbox:        sandbox,
		Registrar:      registrar,
		Analyser:       nil,
		MdDirectory:    "",
