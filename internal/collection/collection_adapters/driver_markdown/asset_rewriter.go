...
func rewriteRelativeURLs(ctx context.Context, params rewriteParams, raw string) (string, bool) {
	...
	if (params.Registrar == nil && params.Analyser == nil) || params.Sandbox == nil || raw == "" {
		return raw, false
	}

	if len(raw) > maxRawHTMLBytes {
		l.Warn("Skipping raw HTML rewrite: blob exceeds size cap",
			logger_domain.String(keyPath, params.MdRelativePath),
			logger_domain.Int("blob_bytes", len(raw)),
			logger_domain.Int("max_bytes", maxRawHTMLBytes))
		return raw, false
	}
	...
	state := &rewriteState{
		...
		mdDirectory:    params.MdDirectory,
...
func buildImgSrcReplacement(
	...
	if resolved == "." || resolved == "" || resolved == ".." || strings.HasPrefix(resolved, "../") || filepath.IsAbs(resolved) {
