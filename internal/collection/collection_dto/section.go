...
//
// Unlike flat section data from content providers, SectionNode holds nested children to
// build a tree structure for navigation. It works with any content source such as
// markdown or CMS systems.
...
//
// This configuration is passed to BuildSectionTree to customise filtering and hierarchy
// generation.
//
// Design Philosophy:
...
type SectionTreeConfig struct {
	...
	//
	// Headings below this level are filtered out. Default is 2, which starts at h2 and skips
	// h1 (typically the page title).
	MinLevel int

	// MaxLevel is the highest heading level to include in the tree.
	//
	// Headings with a level greater than MaxLevel are not included. Default: 4 (includes h2,
	// h3, h4).
	MaxLevel int
}
