...
// native PDF drawing commands.
type SVGWriter struct{}

var (
	_ pdfwriter_domain.SVGWriterPort = (*SVGWriter)(nil)
)
...
// RenderSVG parses the SVG string and renders it as native PDF vector commands into the
// provided content stream at the given position and size.
//
// Takes ctx (context.Context) which controls cancellation.
// Takes svgData (string) which holds the raw SVG markup to parse.
// Takes svgRenderContext (pdfwriter_domain.SVGRenderContext) which provides the PDF
// stream and resource managers.
// Takes x (float64) which specifies the horizontal placement offset in PDF points.
// Takes y (float64) which specifies the vertical placement offset in PDF points.
// Takes renderW (float64) which specifies the target rendering width in PDF points.
// Takes renderH (float64) which specifies the target rendering height in PDF points.
//
// Returns error which indicates a parse failure if the SVG markup is malformed.
...
type renderContext struct {
	...
	// extGStateManager holds the manager for registering extended graphics state resources
	// such as opacity.
	...
	// registerFont holds the callback that registers a font and returns its PDF resource
	// name.
...
// including aspect ratio handling.
//
...
// computeDefaultViewBox constructs a fallback viewBox from the SVG intrinsic dimensions.
//
// Takes svg (*SVG) which holds the parsed SVG document.
//
...
// computeAspectRatioTransform calculates the uniform scale and translation for
// preserveAspectRatio alignment.
//
// Takes par (AspectRatio) which holds the parsed preserveAspectRatio value.
// Takes scaleX (float64) which specifies the initial horizontal scale factor.
// Takes scaleY (float64) which specifies the initial vertical scale factor.
// Takes renderW (float64) which specifies the target rendering width in PDF points.
// Takes renderH (float64) which specifies the target rendering height in PDF points.
// Takes vbox (ViewBox) which holds the SVG viewBox dimensions.
//
// Returns finalScaleX (float64) which holds the computed horizontal scale.
// Returns finalScaleY (float64) which holds the computed vertical scale.
// Returns translateX (float64) which holds the horizontal alignment offset.
// Returns translateY (float64) which holds the vertical alignment offset.
...
// renderNode dispatches rendering for a single SVG node based on its tag type.
//
// Takes ctx (context.Context) which controls cancellation.
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
...
// renderGroup renders an SVG group or nested SVG element by applying its transform and
// rendering children.
//
// Takes ctx (context.Context) which controls cancellation.
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
...
// renderShapeOrContent renders individual SVG shape or content elements such as rect,
// circle, path, and text.
//
// Takes ctx (context.Context) which controls cancellation.
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
...
// applyTransform concatenates the node transform matrix to the current graphics state if
// it is not the identity.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes node (*Node) which holds the node whose transform to apply.
...
// applyGroupOpacity registers an extended graphics state for the group opacity if it is
// less than fully opaque.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes style (*Style) which holds the resolved style properties including opacity.
...
// renderUse renders an SVG use element by resolving its href to a definition and
// rendering the referenced node.
//
// Takes ctx (context.Context) which controls cancellation.
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes node (*Node) which holds the use element to resolve.
// Takes parentStyle (*Style) which holds the inherited style properties.
...
// renderRect renders an SVG rect element, handling optional rounded corners.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes node (*Node) which holds the rect element to render.
// Takes style (*Style) which holds the resolved style properties.
...
// renderCircle renders an SVG circle element as a Bezier-approximated ellipse.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes node (*Node) which holds the circle element to render.
// Takes style (*Style) which holds the resolved style properties.
...
// renderEllipse renders an SVG ellipse element as a Bezier-approximated path.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes node (*Node) which holds the ellipse element to render.
// Takes style (*Style) which holds the resolved style properties.
...
// renderLine renders an SVG line element as a stroked path between two points.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes node (*Node) which holds the line element to render.
// Takes style (*Style) which holds the resolved style properties.
...
// renderPolyline renders an SVG polyline or polygon element from its points attribute.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes node (*Node) which holds the polyline element to render.
// Takes style (*Style) which holds the resolved style properties.
// Takes closed (bool) which controls whether the path is closed as a polygon.
...
// renderPath renders an SVG path element by parsing its d attribute into PDF path
// commands.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
...
// renderImage renders an SVG <image> element by embedding the referenced raster image as
// a PDF XObject.
//
...
// paintPath applies the appropriate fill and stroke painting operations for the current
// path.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes style (*Style) which holds the resolved style properties.
...
// paintGradientPath attempts to paint the path with a gradient fill, falling back to
// solid painting on failure.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes style (*Style) which holds the resolved style properties.
// Takes hasStroke (bool) which indicates whether a stroke should also be applied.
...
// paintSolidPath applies solid fill and stroke colours and issues the appropriate PDF
// paint operator.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes style (*Style) which holds the resolved style properties.
// Takes hasFill (bool) which indicates whether a fill should be applied.
// Takes hasStroke (bool) which indicates whether a stroke should be applied.
...
// applyFillStyle sets the PDF fill colour and opacity from the resolved SVG style.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes style (*Style) which holds the resolved style properties including fill colour
// and opacity.
...
// applyStrokeStyle sets the PDF stroke colour, line width, cap, join, dash pattern, and
// opacity from the resolved SVG style.
//
// Takes rc (*renderContext) which provides the PDF stream and resource managers.
// Takes style (*Style) which holds the resolved style properties including stroke
// attributes.
...
// emitEllipse emits four cubic Bezier curves approximating an ellipse centred at (cx, cy)
// with radii rx and ry.
//
// Takes stream (*pdfwriter_domain.ContentStream) which holds the PDF content stream to
// write into.
// Takes cx (float64) which specifies the centre x coordinate.
// Takes cy (float64) which specifies the centre y coordinate.
...
// emitRoundedRect emits a rectangle path with rounded corners using cubic Bezier curves.
//
// Takes stream (*pdfwriter_domain.ContentStream) which holds the PDF content stream to
// write into.
...
// Takes rx (float64) which specifies the horizontal corner radius.
// Takes ry (float64) which specifies the vertical corner radius.
...
// attrFloat reads a named attribute from the node and parses it as a float, returning the
// fallback on failure.
//
// Takes node (*Node) which holds the element to read from.
// Takes name (string) which specifies the attribute name.
// Takes fallback (float64) which specifies the default value if the attribute is missing
// or invalid.
//
// Returns float64 which holds the parsed attribute value or the fallback.
...
// parsePointsList parses a space-or-comma-separated list of float coordinates from an SVG
// points attribute.
//
// Takes s (string) which holds the raw points attribute value.
//
// Returns []float64 which holds the parsed coordinate values.
