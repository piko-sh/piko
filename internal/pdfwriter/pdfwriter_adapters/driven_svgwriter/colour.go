...
//
// Supports named colours, #rgb, #rrggbb, rgb(r,g,b), rgba(r,g,b,a), hsl(h,s,l),
// hsla(h,s,l,a), and "none". The value "currentColor" returns the sentinel
// CurrentColourSentinel which callers must resolve from context.
//
// Takes s (string) which specifies the SVG colour string to parse.
//
// Returns Colour which holds the parsed colour value.
// Returns bool which holds false for "none" or unparseable values.
...
var (
	...
	// CurrentColourSentinel is a marker value returned by ParseColour for "currentColor".
	// The caller must replace it with the inherited colour.
	CurrentColourSentinel = Colour{R: -1, G: -1, B: -1, A: -1}
)
...
// parseHexColour parses a hex colour string (without the leading #) into a Colour value.
//
// Takes hex (string) which specifies the 3 or 6 character hex colour string.
//
// Returns Colour which holds the parsed colour.
// Returns bool which holds false if the hex string has an invalid length or characters.
...
// parseColourComponent parses a single RGB component value, supporting both absolute
// (0-255) and percentage formats.
//
...
//
// Takes s (string) which specifies the hue string, optionally suffixed with "deg".
//
...
// parsePercentComponent parses a percentage string into a normalised value in [0,1].
//
// Takes s (string) which specifies the percentage string, optionally suffixed with "%".
//
...
//
// Takes h (float64) which specifies the hue in [0,360).
// Takes s (float64) which specifies the saturation in [0,1].
// Takes l (float64) which specifies the lightness in [0,1].
//
...
// hueToRGB computes a single RGB channel value from the HSL intermediate values using
// piecewise linear interpolation.
//
// Takes p (float64) which specifies the lower interpolation bound.
// Takes q (float64) which specifies the upper interpolation bound.
// Takes t (float64) which specifies the hue offset for the channel.
//
