...
var (
	...
	log = logger.GetLogger("piko/internal/registry/registry_dal/querier_adapter_postgres")
...
type DAL struct {
	...
	// handle, so the adapter keeps the DBTX directly to run these dynamically-built queries.
...
// GetMultipleArtefacts retrieves multiple artefacts by their IDs. The slice parameter is
// expanded to numbered placeholders at runtime by the generated query.
//
...
// Returns []int64 which contains the row IDs to delete, bound to the IN-clause slice
// parameter and expanded to numbered placeholders at runtime.
