...
// TaskExecutor handles the running of tasks sent by the orchestrator. Implementations
...
// TaskStore provides storage and retrieval for task state. It handles crash recovery and
...
type TaskStore interface {
	...
	// FetchAndMarkDueTasks retrieves tasks that are due and marks them as in progress.
	//
	...
	// PromoteScheduledTasks moves tasks that are due from the scheduled queue to the ready
	// queue.
	//
	...
	// CreateTaskWithDedup creates a task with deduplication support, checking for existing
	// active tasks with the same DeduplicationKey and returning ErrDuplicateTask if one
	// exists. If DeduplicationKey is empty, it behaves identically to CreateTask.
	//
	...
	// Takes maxRetries (int) which is the maximum retry count before a task is marked
	// FAILED.
	...
	// uses row-level locking to prevent multiple nodes from recovering the same task.
	//
	...
	// RecoverClaimedTasks recovers tasks previously claimed by a node. Tasks are set to
	// RETRYING if attempts remain, or FAILED otherwise, and the lease is cleared.
	//
	...
	// CleanupOldResolvedReceipts deletes resolved receipts older than the cutoff. Used for
...
// RecoveryClaimedTask represents a task that has been claimed for recovery. This is
// returned by ClaimStaleTasksForRecovery so the caller knows which tasks were claimed and
...
// PendingReceipt represents a workflow receipt that is waiting to be resolved. Used for
// startup recovery and workflow status checking.
...
// OrchestratorService manages task lifecycle including dispatch, scheduling, and
// execution. It coordinates workers and tracks workflow completion.
type OrchestratorService interface {
	...
	// Run starts the orchestrator's background processes and blocks until the context is
	// cancelled.
	...
	// GetTaskDispatcher returns the TaskDispatcher for direct task dispatch. The bridge uses
	// this for competing-consumer task distribution.
	//
	// Returns TaskDispatcher which handles task dispatch operations.
	GetTaskDispatcher() TaskDispatcher

	// DispatchDirect saves and dispatches a task straight away, without using the async
	// batch queue. This is mainly for testing where you need tasks to run in a set order.
	//
...
// EventBus provides pub/sub messaging with both channel-based and handler-based
// subscriptions. It is backed by WatermillEventBus which wraps Watermill's GoChannel
// provider.
//
...
type EventBus interface {
	...
	//
	// Takes topic (string) which specifies the event topic to subscribe to. Supports
	// wildcard patterns (e.g., "artefact.*").
	//
	...
	// SubscribeWithHandler subscribes to a topic and invokes the handler for each received
	// message.
	//
	...
	//
	// The handler is called concurrently for different messages. Wildcard patterns are
	// supported (e.g., "artefact.*").
...
// EventHandler defines a callback that processes subscription events, returning nil to
// acknowledge success or an error to trigger redelivery. Handlers must be idempotent as
// messages may be delivered multiple times.
type EventHandler func(ctx context.Context, event Event) error

// DelayedPublisher handles scheduling tasks for future execution. It keeps a min-heap of
// tasks ordered by their scheduled time and sends them when they become due.
type DelayedPublisher interface {
	...
	// Start begins the delayed task processing loop. Call this once before scheduling tasks.
	Start(ctx context.Context)

	// Stop shuts down the publisher in a controlled way.
	Stop()

	// PendingCount returns the number of tasks waiting to be sent. Used for idle detection;
	// the dispatcher is not idle while tasks are pending.
	//
