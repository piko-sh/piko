...
import (
	...
	orchestrator_db "piko.sh/piko/internal/orchestrator/orchestrator_dal/querier_postgres/db"
...
var (
	...
	// log is the package-level logger for the querier_adapter_postgres package.
	log = logger_domain.GetLogger("piko/internal/orchestrator/orchestrator_dal/querier_adapter_postgres")
...
// between the domain layer and the PostgreSQL storage layer.
type Adapter struct {
...
func (a *Adapter) CreateTaskWithDedup(ctx context.Context, task *orchestrator_domain.Task) error {
	...
	dedupParams := orchestrator_db.CreateTaskWithDedupParams{
		ID:               params.ID,
		...
		DeduplicationKey: new(task.DeduplicationKey),
	}

	return a.runInTransaction(ctx, func(ctx context.Context, qtx *orchestrator_db.Queries) error {
		rowsAffected, err := qtx.CreateTaskWithDedup(ctx, dedupParams)
		if err != nil {
...
// ClaimStaleTasksForRecovery atomically claims stale PROCESSING tasks for recovery. The
// method fetches candidate stale tasks within a transaction and attempts to claim each
// one individually, collecting only those where the claim succeeds.
//
...
func (a *Adapter) ListWorkflowSummary(ctx context.Context, limit int) ([]orchestrator_domain.WorkflowSummary, error) {
	...
	for i, row := range rows {
		results[i] = orchestrator_domain.WorkflowSummary{
			...
			CompleteCount: derefInt64Ptr(row.CompleteCount),
			FailedCount:   derefInt64Ptr(row.FailedCount),
			ActiveCount:   derefInt64Ptr(row.ActiveCount),
			CreatedAt:     derefInt64Ptr(row.CreatedAt),
			UpdatedAt:     derefInt64Ptr(row.UpdatedAt),
		}
	}
...
// derefInt64Ptr returns the value behind a nullable int64 pointer, defaulting to zero
// when nil.
//
...
// existing queries to avoid starting a nested transaction.
//
