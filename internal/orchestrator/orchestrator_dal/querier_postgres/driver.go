...
// driver is the PostgreSQL implementation of dalcore.Driver.
//
// It translates dalcore's dialect-neutral flat structs into the generated PostgreSQL
// query calls. Deduplication uses a single INSERT with ON CONFLICT DO NOTHING, and
// recovery omits the columns Postgres does not rewrite.
type driver struct {
...
// New creates an orchestrator DAL backed by the given PostgreSQL database connection or
// transaction.
//
...
//
// The insert is a single INSERT with ON CONFLICT DO NOTHING, so created is false when a
// row with the same key already existed.
//
// Takes params (dalcore.CreateTaskParams) which describes the task to insert.
// Takes deduplicationKey (string) which is the key that guards against duplicates.
//
// Returns bool which is true when a new task was inserted, or false when a row with the
// same deduplication key already existed.
// Returns error when the insert fails.
func (d *driver) CreateWithDedup(ctx context.Context, params dalcore.CreateTaskParams, deduplicationKey string) (bool, error) {
	rowsAffected, err := d.queries.CreateTaskWithDedup(ctx, orchestrator_db.CreateTaskWithDedupParams{
		ID:               params.ID,
...
// toCreateTaskParams maps a dalcore.CreateTaskParams to the generated PostgreSQL params.
//
