...
// ExtractBodyStyles extracts styles for the body selector from CSS and returns them as
// inline styles plus the remaining CSS with body rules removed.
//
// Body styles are applied inline even when the body element does not exist during
// premailer processing. Handles both simple "body { ... }" rules and compound selectors
// like ".foo, body { ... }". Uses esbuild's CSS parser for accurate parsing.
//
// Takes css (string) which is the CSS content to process.
//
// Returns inlineStyles (string) which contains the extracted body styles formatted for
// inline use, with !important declarations stripped.
// Returns cleanedCSS (string) which is the original CSS with body rules removed.
...
// parseStyleAttribute converts a style attribute string into a map for easy merging.
//
...
// reconstructStyleAttribute converts a style map back into a CSS string for use in an
// HTML style attribute.
//
// The !important flag is always stripped from inline styles because inline styles already
// have the highest specificity, making it unnecessary. Some email clients may not handle
// !important in inline styles correctly. If you need !important for webmail overrides,
// use the KeepBangImportant option which keeps the rule in the style block.
//
// Takes styleMap (map[string]property) which contains the CSS properties to rebuild.
//
// Returns string which is the formatted CSS string with properties sorted alphabetically,
...
// processRuleForBodyExtraction processes a single CSS rule by extracting body styles and
// returning a modified rule with body selectors removed.
//
// Takes rule (css_ast.Rule) which is the CSS rule to process.
// Takes styleMap (*map[string]property) which collects the extracted body styles.
// Takes symbols ([]ast.Symbol) which provides symbol context for extraction.
//
// Returns *css_ast.Rule which is the modified rule without body selectors, or nil if the
...
//
// Takes selectors ([]css_ast.ComplexSelector) which contains the CSS selectors to check.
//
// Returns hasBody (bool) which is true if any selector targets the body element.
// Returns nonBodySelectors ([]css_ast.ComplexSelector) which contains all selectors that
// do not target the body element.
...
// isSimpleBodySelector checks if a complex selector is a simple "body" selector.
//
// Takes selector (css_ast.ComplexSelector) which is the selector to check.
//
// Returns bool which is true if the selector is exactly "body" with no subclass selectors
// such as classes, IDs, or pseudo-classes.
...
// extractBodyDeclarations gets CSS declarations from rules and adds them to the style
// map.
//
...
// Takes styleMap (*map[string]property) which receives the extracted properties.
...
//
// Returns string which is the inline style attribute value. Adds a trailing semicolon if
...
//
// Returns string which contains the rebuilt CSS text, or an empty string if rules is
// empty.
