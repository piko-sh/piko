...
var (
	// problematicProperties defines CSS properties that have poor or inconsistent support in
	// major email clients, especially Outlook. This map is used to generate warnings.
	//
	// NOTE: MSO-specific properties (mso-*) are intentionally NOT included in this map.
	// These are Outlook-specific CSS extensions that SHOULD be preserved and used for fixing
	// Outlook rendering issues. Common MSO properties include:
	...
	problematicProperties = map[string]string{
...
// validateEmailCompatibility checks CSS rules for properties and values that may not work
// well in email clients.
//
...
//
// Returns []*ast_domain.Diagnostic which contains warnings for any problems found.
...
// validateRuleProperties checks all properties in a style rule for email client support
// problems.
//
// Takes rule (styleRule) which contains the CSS properties to check.
// Takes sourcePath (string) which identifies the source file for diagnostics.
//
// Returns []*ast_domain.Diagnostic which contains any warnings found for properties that
// may not work in email clients.
...
// Takes sourcePath (string) which identifies the source file for diagnostics.
//
// Returns []*ast_domain.Diagnostic which contains warnings for values that do not work
// well in email clients, or nil if the value needs no special checks.
...
// validateDisplayValue checks for CSS display values that do not work well in email
// clients.
//
...
//
// Returns []*ast_domain.Diagnostic which holds warnings for display values like flex or
// grid that email clients do not support, or nil when the value is safe to use.
...
// validatePositionValue checks if a position property value works in email clients.
//
// Takes value (string) which is the position property value to check.
// Takes sourcePath (string) which identifies the source file for diagnostics.
//
// Returns []*ast_domain.Diagnostic which contains warnings for position values like
// "fixed" or "sticky" that email clients do not support, or nil if the value is valid.
...
//
// Returns []*ast_domain.Diagnostic which holds warnings for background values that email
// clients do not support, such as CSS gradients.
...
// checkLeftoverRules checks CSS rules that were not inlined for problems. It processes
// rules one by one, including nested rules in @media queries.
//
...
// Takes sourcePath (string) which identifies the source file for diagnostics.
//
// Returns []*ast_domain.Diagnostic which contains any problems found, or nil if the rule
// type is not checked.
...
//
// Takes selector (*css_ast.RSelector) which contains the CSS selector and its rules to
// check.
// Takes sourcePath (string) which identifies the source file for error messages.
//
// Returns []*ast_domain.Diagnostic which contains any problems found in the declarations.
...
// checkLeftoverDeclaration checks a single CSS declaration for compatibility issues.
//
// Takes declaration (*css_ast.RDeclaration) which is the CSS declaration to check.
// Takes sourcePath (string) which identifies the source file for diagnostics.
//
// Returns []*ast_domain.Diagnostic which contains any compatibility warnings found.
...
// checkLeftoverDeclarationValues checks the token values within a CSS declaration.
//
// Takes declaration (*css_ast.RDeclaration) which is the CSS declaration to check.
...
// validateLeftoverTokenValue checks a single token value for compatibility issues in
// email clients.
//
...
// Takes sourcePath (string) which identifies the source file for diagnostics.
//
// Returns *ast_domain.Diagnostic which contains a warning when the value is not
// well-supported in email clients, or nil when no issues are found.
