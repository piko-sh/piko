...
func TestProvider_Type(t *testing.T) {
	...
	assert.Equal(t, captcha_dto.ProviderTypeTurnstile, testProvider.Type())
}
...
func TestProvider_ScriptURL(t *testing.T) {
	...
	assert.Equal(t, "https://challenges.cloudflare.com/turnstile/v0/api.js", testProvider.ScriptURL())
}
...
func TestProvider_Verify_MalformedJSON(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "parsing turnstile response")
}
...
func TestProvider_MalformedTimestamp(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		writer.Header().Set("Content-Type", "application/json")
		writer.WriteHeader(http.StatusOK)
		_, _ = writer.Write([]byte(`{"success":true,"challenge_ts":"garbage"}`))
	}))
	defer server.Close()
