...
// charHandler processes a character at a given position and returns the next position.
...
// Encoder provides phonetic encoding using Norwegian phonetic rules. It implements the
// linguistics_domain.PhoneticEncoderPort interface.
//
// The encoder handles Norwegian-specific patterns including the KJ/SJ sounds, retroflex
// RS, and various vowel patterns.
type Encoder struct {
	// maxLength is the maximum number of characters in the output code.
	maxLength int
}

// NewWithMaxLength creates a new Norwegian phonetic encoder with a custom maximum code
// length.
//
