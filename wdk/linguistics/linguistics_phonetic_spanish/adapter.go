...
// New creates a new Spanish phonetic encoder.
//
// Returns *Encoder which is ready to encode Spanish text.
...
func handleA(_ string, position int, result *strings.Builder) int {
...
func handleB(word string, position int, result *strings.Builder) int {
	return handleDoubleConsonant(word, position, 'B', PhoneticB, result)
}

// handleC processes the letter C at the given position in a word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position of the letter C.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the next position to process after handling C.
...
// handleE dispatches the phonetic encoding for the letter E.
//
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the updated position after processing.
func handleE(_ string, position int, result *strings.Builder) int {
...
func handleF(word string, position int, result *strings.Builder) int {
	return handleDoubleConsonant(word, position, 'F', PhoneticF, result)
}

// handleG processes the letter G at the given position in a word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
...
func handleG(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "GU") {
		if position+DigraphLength < len(word) && isSoftVowel(word[position+DigraphLength]) {
			_, _ = result.WriteString(PhoneticG)
			return position + DigraphLength
		}
		_, _ = result.WriteString(PhoneticG)
	...
	return handleDoubleConsonant(word, position, 'G', PhoneticG, result)
}
...
func handleL(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "LL") {
		_, _ = result.WriteString(PhoneticJ)
		return position + DigraphLength
	}

	_, _ = result.WriteString("L")
	return position + 1
}
...
//
// Returns int which is the updated position after processing.
func handleO(_ string, position int, result *strings.Builder) int {
...
// handleQ processes the letter Q in phonetic encoding.
//
// Takes word (string) which is the word being encoded.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the next position to process after handling Q.
...
func handleR(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "RR") {
		_, _ = result.WriteString(PhoneticR)
		return position + DigraphLength
	}
...
// handleT processes the letter T using double consonant handling rules.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the output.
...
func handleV(_ string, position int, result *strings.Builder) int {
	_, _ = result.WriteString(PhoneticB)
	return position + 1
}
...
//
// Returns int which is the next position to process in the word.
func handleY(word string, position int, result *strings.Builder) int {
	if isWordEnd(word, position) {
		_, _ = result.WriteString("I")
		return position + 1
	}

	if position+1 < len(word) && isSpanishConsonant(word[position+1]) {
		_, _ = result.WriteString("I")
		return position + 1
	}
