// Copyright 2026 PolitePixels Limited
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// This project stands against fascism, authoritarianism, and all forms of
// oppression. We built this to empower people, not to enable those who would
// strip others of their rights and dignity.

package linguistics_phonetic_hungarian

import (
	"strings"

	"piko.sh/piko/internal/linguistics/linguistics_domain"
)

const (
	// Language is the language code for this encoder.
	Language = "hungarian"

	// DefaultMaxLength is the default maximum length for Hungarian phonetic codes.
	DefaultMaxLength = 6

	// latinAlphabetSize is the number of letters in the Latin alphabet after Unicode
	// normalisation.
	latinAlphabetSize = 26
)

// charHandler processes a character at the given position and returns the next position.
type charHandler func(word string, position int, result *strings.Builder) int

var (
	// charHandlers is an array dispatch table for character processing. Index is calculated
	// as character - 'A' for uppercase letters (A=0, B=1, ..., Z=25).
	charHandlers = [latinAlphabetSize]charHandler{
		0:  handleA,
		1:  handleB,
		2:  handleC,
		3:  handleD,
		4:  handleE,
		5:  handleF,
		6:  handleG,
		7:  handleH,
		8:  handleI,
		9:  handleJ,
		10: handleK,
		11: handleL,
		12: handleM,
		13: handleN,
		14: handleO,
		15: handleP,
		16: handleQ,
		17: handleR,
		18: handleS,
		19: handleT,
		20: handleU,
		21: handleV,
		22: handleW,
		23: handleX,
		24: handleY,
		25: handleZ,
	}
)

// Encoder provides phonetic encoding using Hungarian phonetic rules. It implements the
// linguistics_domain.PhoneticEncoderPort interface.
type Encoder struct {
	// maxLength is the maximum number of characters in the output code.
	maxLength int
}

// NewWithMaxLength creates a new Hungarian phonetic encoder with a custom maximum code
// length.
//
// Takes maxLength (int) which controls the maximum length of phonetic codes.
//
// Returns (*Encoder, error) where the error is always nil for this encoder.
func NewWithMaxLength(maxLength int) (*Encoder, error) {
	if maxLength <= 0 {
		maxLength = DefaultMaxLength
	}

	return &Encoder{
		maxLength: maxLength,
	}, nil
}

// Encode returns the phonetic encoding of a Hungarian word.
//
// Takes word (string) which is the word to encode phonetically. The word should be
// normalised for best results.
//
// Returns string which is the phonetic code.
func (e *Encoder) Encode(word string) string {
	if len(word) == 0 {
		return ""
	}

	word = strings.ToUpper(word)

	var result strings.Builder
	position := 0

	for position < len(word) && result.Len() < e.maxLength {
		position = processCharacter(word, position, &result)
	}

	code := result.String()
	if len(code) > e.maxLength {
		return code[:e.maxLength]
	}
	return code
}

// GetLanguage returns the language this encoder is configured for.
//
// Returns string which is the language code.
func (*Encoder) GetLanguage() string {
	return Language
}

var (
	_ linguistics_domain.PhoneticEncoderPort = (*Encoder)(nil)
)

// Factory creates a new Hungarian phonetic encoder instance.
//
// Use this with linguistics_domain.RegisterPhoneticEncoderFactory for explicit
// registration.
//
// Returns linguistics_domain.PhoneticEncoderPort which is the encoder instance.
// Returns error when the encoder cannot be created.
func Factory() (linguistics_domain.PhoneticEncoderPort, error) {
	return New()
}

// New creates a Hungarian phonetic encoder with the default maximum length.
//
// Returns *Encoder which is ready to encode strings.
// Returns error which is always nil for this encoder.
func New() (*Encoder, error) {
	return NewWithMaxLength(DefaultMaxLength)
}

// processCharacter processes a single character and returns the next position. Uses array
// dispatch table for O(1) handler lookup.
//
// Takes word (string) which is the input word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the processed output.
//
// Returns int which is the next position to process in the word.
func processCharacter(word string, position int, result *strings.Builder) int {
	character := word[position]

	if character >= 'A' && character <= 'Z' {
		if handler := charHandlers[character-'A']; handler != nil {
			return handler(word, position, result)
		}
	}

	return position + 1
}

// handleA processes the letter A in the word at the given position.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the output.
//
// Returns int which is the next position to process.
func handleA(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'A', "A", result)
}

// handleB processes the letter B at the given position in a word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the output.
//
// Returns int which is the new position after processing.
func handleB(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'B', "B", result)
}

// handleC processes the letter C at the given position in a word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the next position to process.
func handleC(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "CS") {
		_, _ = result.WriteString(PhoneticC)
		if hasPrefix(word, position, "CSS") {
			return position + TrigraphLength
		}
		return position + DigraphLength
	}

	if position+1 < len(word) && word[position+1] == 'C' {
		_, _ = result.WriteString(PhoneticK)
		return position + DigraphLength
	}

	_, _ = result.WriteString(PhoneticK)
	return position + 1
}

// handleD processes the letter D and its digraph and trigraph variants.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the new position after processing.
func handleD(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "DZS") {
		_, _ = result.WriteString(PhoneticDJ)
		return position + TrigraphLength
	}

	if hasPrefix(word, position, "DZ") {
		_, _ = result.WriteString("DZ")
		return position + DigraphLength
	}

	return handleDoubleChar(word, position, 'D', "D", result)
}

// handleE processes the letter 'E' at the given position in the word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the output.
//
// Returns int which is the new position after processing.
func handleE(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'E', "E", result)
}

// handleF processes an 'F' character in the word and writes to result.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which receives the output.
//
// Returns int which is the new position after processing.
func handleF(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'F', "F", result)
}

// handleG processes the letter G in Hungarian phonetic encoding.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the new position after consuming the G sequence.
func handleG(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "GY") {
		_, _ = result.WriteString(PhoneticDJ)
		if hasPrefix(word, position, "GGY") {
			return position + TrigraphLength
		}
		return position + DigraphLength
	}

	return handleDoubleChar(word, position, 'G', "G", result)
}

// handleH dispatches the phonetic encoding for the letter H.
//
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the updated position after processing.
func handleH(_ string, position int, result *strings.Builder) int {
	_, _ = result.WriteString("H")
	return position + 1
}

// handleI processes the letter 'I' in a word and writes the result.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the output.
//
// Returns int which is the new position after processing.
func handleI(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'I', "I", result)
}

// handleJ dispatches the phonetic encoding for the letter J.
//
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the updated position after processing.
func handleJ(_ string, position int, result *strings.Builder) int {
	_, _ = result.WriteString("J")
	return position + 1
}

// handleK processes the letter K at the given position in a word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the next position to process.
func handleK(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'K', PhoneticK, result)
}

// handleL processes the letter L at the given position in a word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the next position to process after handling L.
func handleL(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "LY") {
		_, _ = result.WriteString(PhoneticJ)
		if hasPrefix(word, position, "LLY") {
			return position + TrigraphLength
		}
		return position + DigraphLength
	}

	return handleDoubleChar(word, position, 'L', "L", result)
}

// handleM processes the letter M at the given position in a word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the output.
//
// Returns int which is the next position to process.
func handleM(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'M', "M", result)
}

// handleN processes the letter N for phonetic conversion.
//
// Takes word (string) which is the input being converted.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the new position after processing.
func handleN(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "NY") {
		_, _ = result.WriteString(PhoneticNJ)
		if hasPrefix(word, position, "NNY") {
			return position + TrigraphLength
		}
		return position + DigraphLength
	}

	return handleDoubleChar(word, position, 'N', "N", result)
}

// handleO processes the letter O at the given position in the word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the output.
//
// Returns int which is the new position after processing.
func handleO(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'O', "O", result)
}

// handleP processes the letter P at the given position in a word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the output.
//
// Returns int which is the next position to process.
func handleP(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'P', "P", result)
}

// handleQ dispatches the phonetic encoding for the letter Q.
//
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the updated position after processing.
func handleQ(_ string, position int, result *strings.Builder) int {
	_, _ = result.WriteString(PhoneticK)
	return position + 1
}

// handleR processes the letter R at the given position in word.
//
// Takes word (string) which is the input being processed.
// Takes position (int) which is the current position in word.
// Takes result (*strings.Builder) which accumulates the output.
//
// Returns int which is the next position to process.
func handleR(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'R', "R", result)
}

// handleS processes the Hungarian letter S and its digraph SZ.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the new position after consuming the character(s).
func handleS(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "SZ") {
		_, _ = result.WriteString(PhoneticS)
		if hasPrefix(word, position, "SSZ") {
			return position + TrigraphLength
		}
		return position + DigraphLength
	}

	if position+1 < len(word) && word[position+1] == 'S' {
		_, _ = result.WriteString("X")
		return position + DigraphLength
	}

	_, _ = result.WriteString("X")
	return position + 1
}

// handleT processes the letter T and its combinations in a word.
//
// Takes word (string) which is the word being transliterated.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the new position after consuming the character(s).
func handleT(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "TY") {
		_, _ = result.WriteString(PhoneticTJ)
		if hasPrefix(word, position, "TTY") {
			return position + TrigraphLength
		}
		return position + DigraphLength
	}

	return handleDoubleChar(word, position, 'T', "T", result)
}

// handleU processes the letter U at the given position in word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the output.
//
// Returns int which is the new position after processing.
func handleU(word string, position int, result *strings.Builder) int {
	return handleDoubleChar(word, position, 'U', "U", result)
}

// handleV dispatches the phonetic encoding for the letter V.
//
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the updated position after processing.
func handleV(_ string, position int, result *strings.Builder) int {
	_, _ = result.WriteString("V")
	return position + 1
}

// handleW dispatches the phonetic encoding for the letter W.
//
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the updated position after processing.
func handleW(_ string, position int, result *strings.Builder) int {
	_, _ = result.WriteString("V")
	return position + 1
}

// handleX dispatches the phonetic encoding for the letter X.
//
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the updated position after processing.
func handleX(_ string, position int, result *strings.Builder) int {
	_, _ = result.WriteString("KS")
	return position + 1
}

// handleY dispatches the phonetic encoding for the letter Y.
//
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the updated position after processing.
func handleY(_ string, position int, result *strings.Builder) int {
	_, _ = result.WriteString("I")
	return position + 1
}

// handleZ processes the letter Z and its phonetic variants in Hungarian.
//
// Takes word (string) which is the input word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the phonetic output.
//
// Returns int which is the new position after consuming the processed chars.
func handleZ(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "ZS") {
		_, _ = result.WriteString(PhoneticZ)
		if hasPrefix(word, position, "ZZS") {
			return position + TrigraphLength
		}
		return position + DigraphLength
	}

	return handleDoubleChar(word, position, 'Z', "Z", result)
}

// handleDoubleChar handles characters that may double (consonants and vowels).
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes letter (byte) which is the character to check for doubling.
// Takes code (string) which is the phonetic code to append.
// Takes result (*strings.Builder) which accumulates the output.
//
// Returns int which is the next position to process, skipping doubled letters.
func handleDoubleChar(word string, position int, letter byte, code string, result *strings.Builder) int {
	_, _ = result.WriteString(code)
	if position+1 < len(word) && word[position+1] == letter {
		return position + DigraphLength
	}
	return position + 1
}

func init() {
	linguistics_domain.RegisterPhoneticEncoderFactory(Language, Factory)
}
