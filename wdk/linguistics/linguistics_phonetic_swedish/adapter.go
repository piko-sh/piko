...
const (
	...
	// DefaultMaxLength is the default maximum length for Swedish phonetic codes.
	DefaultMaxLength = 6
...
// Encoder provides phonetic encoding using Swedish phonetic rules. It implements the
...
// NewWithMaxLength creates a new Swedish phonetic encoder with a custom maximum code
// length.
//
...
// Encode returns the phonetic code for a Swedish word.
//
// Takes word (string) which is the word to encode. The word should be normalised for best
// results.
//
...
// Factory creates a new Swedish phonetic encoder instance.
//
// Use this with linguistics_domain.RegisterPhoneticEncoderFactory for explicit
// registration.
//
// Returns linguistics_domain.PhoneticEncoderPort which is the configured encoder ready
// for use.
...
// New creates a Swedish phonetic encoder with default settings.
//
// Returns *Encoder which is ready for encoding Swedish text.
...
// handleB processes the letter B at the given position in the word.
//
...
func handleG(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "GN") {
		_, _ = result.WriteString(PhoneticGN)
		return position + DigraphLength
	}
...
// handleH processes the letter H, treating it as silent before J.
//
...
// handleK processes the letter K at the given position and writes the phonetic
...
// handleL processes the letter L at the given position in the word.
//
// Takes word (string) which is the word being processed.
// Takes position (int) which is the current position in the word.
// Takes result (*strings.Builder) which accumulates the output.
...
func handleS(word string, position int, result *strings.Builder) int {
	if hasPrefix(word, position, "STJ") {
		_, _ = result.WriteString(PhoneticX)
