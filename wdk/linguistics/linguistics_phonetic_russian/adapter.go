...
const (
	...
	// cyrillicAlphabetSize is the number of letters in the main Cyrillic block (A through
	// Ya, Cyrillic).
...
// runeHandler processes a rune at the given position and returns the next position.
type runeHandler func(runes []rune, position int, result *strings.Builder) int

var (
	// cyrillicHandlers is a dispatch table for Cyrillic character processing. Index is
	// calculated as character - cyrillicBaseRune for uppercase Cyrillic letters (A=0, Be=1,
	// ..., Ya=31).
	cyrillicHandlers = [cyrillicAlphabetSize]runeHandler{
		0:  handleA,
		1:  handleB,
		2:  handleV,
		3:  handleG,
...
// Encoder provides phonetic encoding using Russian phonetic rules. It implements the
...
// NewWithMaxLength creates a new Russian phonetic encoder with a custom maximum code
// length.
//
...
//
// Takes word (string) which is the word to encode phonetically. The word should be in
// Cyrillic script.
//
...
//
// When the letter appears at word start or after a vowel, writes PhoneticYE. Otherwise,
// writes PhoneticE.
//
// Takes runes ([]rune) which is the input text as a slice of runes.
// Takes position (int) which is the current position in the rune slice.
...
// Takes result (*strings.Builder) which accumulates the transliterated output.
//
// Returns int which is the next position to process after handling the character.
