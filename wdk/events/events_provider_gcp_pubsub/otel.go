...
var (
	// log is the package-level logger for the GCP Pub/Sub events provider.
	log = logger.GetLogger("piko/wdk/events/events_provider_gcp_pubsub")

	// meter is the OpenTelemetry meter for the GCP Pub/Sub events provider.
	meter = otel.Meter("piko/wdk/events/events_provider_gcp_pubsub")
...
func init() {
	...
	providerStartDuration, err = meter.Float64Histogram(
		"events.adapters.gcp_pubsub.provider_start_duration",
		metric.WithDescription("Duration of GCP Pub/Sub provider start operations"),
		metric.WithUnit("ms"),
	)
	if err != nil {
		otel.Handle(err)
	...
	providerCloseDuration, err = meter.Float64Histogram(
		"events.adapters.gcp_pubsub.provider_close_duration",
		metric.WithDescription("Duration of GCP Pub/Sub provider close operations"),
		metric.WithUnit("ms"),
	)
	if err != nil {
		otel.Handle(err)
	...
	providerStartCount, err = meter.Int64Counter(
		"events.adapters.gcp_pubsub.provider_start_count",
		metric.WithDescription("Number of GCP Pub/Sub provider start operations"),
	)
	if err != nil {
		otel.Handle(err)
	}
	...
	providerCloseCount, err = meter.Int64Counter(
		"events.adapters.gcp_pubsub.provider_close_count",
		metric.WithDescription("Number of GCP Pub/Sub provider close operations"),
	)
	if err != nil {
		otel.Handle(err)
	}
	...
	providerStartErrorCount, err = meter.Int64Counter(
		"events.adapters.gcp_pubsub.provider_start_error_count",
		metric.WithDescription("Number of GCP Pub/Sub provider start errors"),
	)
	if err != nil {
		otel.Handle(err)
	}
	...
	providerCloseErrorCount, err = meter.Int64Counter(
		"events.adapters.gcp_pubsub.provider_close_error_count",
		metric.WithDescription("Number of GCP Pub/Sub provider close errors"),
	)
	if err != nil {
		otel.Handle(err)
	}
	...
	providerConnectionAttempts, err = meter.Int64Counter(
		"events.adapters.gcp_pubsub.provider_connection_attempts",
		metric.WithDescription("Number of GCP Pub/Sub connection attempts"),
	)
	if err != nil {
		otel.Handle(err)
	}
