...
// readFileViaOpen reads a file's entire contents using the provided open function, up to
// DefaultReadFileMaxBytes.
//
// The cap protects callers from unbounded allocation when a file has grown unexpectedly.
// Callers that need a tighter or looser cap should use readFileLimitViaOpen.
//
// Takes opener (func(string) (FileHandle, error)) which opens the file for reading.
...
// readFileLimitViaOpen reads up to maxBytes from a file using the provided open and stat
// functions. The size cap is enforced before allocation by statting first; the read
// itself is wrapped in an io.LimitReader as belt and braces against a file growing
// between stat and read.
//
// Takes opener (func(string) (FileHandle, error)) which opens the file for reading.
// Takes statter (func(string) (fs.FileInfo, error)) which returns the file's metadata.
// Takes name (string) which is the relative path within the sandbox.
// Takes maxBytes (int64) which caps the byte count read into memory; must be positive.
//
// Returns []byte containing the file content (up to maxBytes).
// Returns int64 reporting the stat-observed size taken at stat time.
// Returns error wrapping ErrFileExceedsLimit when the file is larger than maxBytes,
// ErrInvalidLimit when maxBytes is non-positive, or any underlying stat / open / read
// error.
