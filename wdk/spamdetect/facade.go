...
type ServiceOption = spamdetect_domain.ServiceOption
...
var (
	...
	// MustNewSchema creates a schema and panics on validation failure.
	MustNewSchema = spamdetect_dto.MustNewSchema
...
var (
	...
	WithScoreThreshold = spamdetect_domain.WithScoreThreshold

	// WithTimeout sets the per-analysis timeout option.
	WithTimeout = spamdetect_domain.WithTimeout

	// WithFeedbackStore sets the feedback store option.
	WithFeedbackStore = spamdetect_domain.WithFeedbackStore

	// WithClock sets the clock option for deterministic testing.
	WithClock = spamdetect_domain.WithClock

	// WithHealthCheckTimeout sets the per-detector readiness deadline.
	WithHealthCheckTimeout = spamdetect_domain.WithHealthCheckTimeout

	// DefaultServiceConfig returns a ServiceConfig with sensible defaults.
	DefaultServiceConfig = spamdetect_dto.DefaultServiceConfig

	// NewSpamDetectError wraps a detector error with operation context.
	NewSpamDetectError = spamdetect_dto.NewSpamDetectError
)
...
var (
	...
	// ErrDetectorUnavailable indicates a named detector could not be resolved.
	ErrDetectorUnavailable = spamdetect_dto.ErrDetectorUnavailable

	// ErrSubmissionNil indicates Analyse was called with a nil submission.
	ErrSubmissionNil = spamdetect_dto.ErrSubmissionNil

	// ErrSchemaNil indicates Analyse was called with a nil schema.
	ErrSchemaNil = spamdetect_dto.ErrSchemaNil
)
...
func Analyse(ctx context.Context, submission *Submission, schema *Schema) (*AnalysisResult, error) {
	if submission == nil {
		return nil, safeerror.NewError(
			"spam check failed: invalid request",
			fmt.Errorf("spamdetect.Analyse: %w", spamdetect_dto.ErrSubmissionNil),
		)
	}
	if schema == nil {
		return nil, safeerror.NewError(
			"spam check failed: invalid configuration",
			fmt.Errorf("spamdetect.Analyse: %w", spamdetect_dto.ErrSchemaNil),
		)
	}
	...
	if err != nil {
		return nil, safeerror.NewError(
			"spam check is unavailable",
			fmt.Errorf("spamdetect.Analyse: %w", err),
		)
	}

	result, err := service.Analyse(ctx, submission, schema)
	if err != nil {
		return result, wrapAnalyseErrorForUser(err)
	}
	return result, nil
}
...
//
// Returns error which implements safeerror.Error.
func wrapAnalyseErrorForUser(err error) error {
	switch {
	case errors.Is(err, spamdetect_dto.ErrSpamDetectDisabled):
		return safeerror.NewError("spam check is not configured", err)
	case errors.Is(err, spamdetect_dto.ErrNoMatchingDetectors):
		return safeerror.NewError("spam check has no compatible detectors for this form", err)
	case errors.Is(err, spamdetect_dto.ErrAllDetectorsFailed):
		return safeerror.NewError("spam check temporarily unavailable; please try again", err)
	default:
		return safeerror.NewError("spam check failed", err)
	}
}
