...
//
// Returns <-chan llm_dto.StreamEvent which yields streaming events as tokens are
...
// streamIterFunc is the type signature for the range-based stream iterator returned by
...
//
// Takes stream (streamIterFunc) which provides the streaming response data from the
// Gemini API as a range-based iterator.
// Takes events (chan<- llm_dto.StreamEvent) which receives converted stream events.
...
// Takes response (*genai.GenerateContentResponse) which contains the response to process.
...
//
// Takes fc (*genai.FunctionCall) which contains the function call data to process.
...
//
// Takes response (*genai.GenerateContentResponse) which contains the API response with
