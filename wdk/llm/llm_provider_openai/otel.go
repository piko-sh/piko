...
var (
	...
	meter = otel.Meter("piko/llm/llm_provider_openai")
...
func init() {
	...
	completeCount, err = meter.Int64Counter(
		"piko.llm.provider.openai.complete.count",
		metric.WithDescription("Number of OpenAI Complete calls."),
	...
	completeDuration, err = meter.Float64Histogram(
		"piko.llm.provider.openai.complete.duration",
		metric.WithDescription("Duration of OpenAI Complete calls."),
		metric.WithUnit("ms"),
	)
	if err != nil {
		otel.Handle(err)
	...
	completeErrorCount, err = meter.Int64Counter(
		"piko.llm.provider.openai.complete.errors",
		metric.WithDescription("Number of OpenAI Complete errors."),
	...
	streamCount, err = meter.Int64Counter(
		"piko.llm.provider.openai.stream.count",
		metric.WithDescription("Number of OpenAI Stream calls."),
		metric.WithUnit("{call}"),
	)
	if err != nil {
		otel.Handle(err)
	...
	streamDuration, err = meter.Float64Histogram(
		"piko.llm.provider.openai.stream.duration",
		metric.WithDescription("Duration of OpenAI Stream calls."),
		metric.WithUnit("ms"),
	)
	if err != nil {
		otel.Handle(err)
	...
	streamErrorCount, err = meter.Int64Counter(
		"piko.llm.provider.openai.stream.errors",
		metric.WithDescription("Number of OpenAI Stream errors."),
		metric.WithUnit("{error}"),
	)
	if err != nil {
		otel.Handle(err)
	...
	embedCount, err = meter.Int64Counter(
		"piko.llm.provider.openai.embed.count",
		metric.WithDescription("Number of OpenAI Embed calls."),
		metric.WithUnit("{call}"),
	)
	if err != nil {
		otel.Handle(err)
	...
	embedDuration, err = meter.Float64Histogram(
		"piko.llm.provider.openai.embed.duration",
		metric.WithDescription("Duration of OpenAI Embed calls."),
		metric.WithUnit("ms"),
	)
	if err != nil {
		otel.Handle(err)
	...
	embedErrorCount, err = meter.Int64Counter(
		"piko.llm.provider.openai.embed.errors",
		metric.WithDescription("Number of OpenAI Embed errors."),
		metric.WithUnit("{error}"),
	)
	if err != nil {
		otel.Handle(err)
