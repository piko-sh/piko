...
func TestConfig_WithDefaults(t *testing.T) {
	t.Run("applies default model", func(t *testing.T) {
		...
		assert.Equal(t, "gpt-4.1", result.DefaultModel)
	})
	...
	t.Run("preserves custom model", func(t *testing.T) {
		...
		assert.Equal(t, "gpt-4.1-mini", result.DefaultModel)
	})
}
...
func TestNew(t *testing.T) {
	...
	t.Run("applies defaults", func(t *testing.T) {
		...
		assert.Equal(t, "gpt-4.1", op.defaultModel)
	})
	...
	t.Run("uses organisation", func(t *testing.T) {
		config := Config{
			APIKey:       "test-api-key",
			Organisation: "org-123",
		}
