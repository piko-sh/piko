...
// Config holds the settings for the multilevel cache provider. This is re-exported from
// the internal adapter package.
type Config = provider_multilevel.Config

// NewMultiLevelAdapter creates a new multi-level cache provider that coordinates between
// an L1 (local/fast) and L2 (remote/distributed) cache.
//
...
// propagation.
// Takes name (string) which is a descriptive name for this multilevel cache instance,
// used in metrics.
// Takes l1 (cache.ProviderPort[K, V]) which is the L1 local cache provider (e.g. Otter).
// Takes l2 (cache.ProviderPort[K, V]) which is the L2 remote cache provider (e.g. Redis).
// Takes cbConfig (Config) which configures the circuit breaker for L2 resilience.
//
