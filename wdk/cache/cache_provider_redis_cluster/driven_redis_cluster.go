...
const (
	...
	// logKeyField is the attribute key used when logging Redis keys.
	logKeyField = "key"
...
type RedisClusterAdapter[K comparable, V any] struct {
	...
	// client is the Redis Cluster client used for cluster operations.
...
//
// Returns string which is the encoded Redis key with optional namespace.
// Returns error when no encoder is registered or marshalling fails.
func (a *RedisClusterAdapter[K, V]) encodeKey(key K) (string, error) {
...
func (a *RedisClusterAdapter[K, V]) GetIfPresent(ctx context.Context, key K) (V, bool, error) {
	if err := ctx.Err(); err != nil {
		return *new(V), false, err
	}

	timeoutCtx, cancel := context.WithTimeoutCause(ctx, a.operationTimeout, fmt.Errorf("redis cluster GetIfPresent exceeded %s timeout", a.operationTimeout))
	defer cancel()
	...
	if err != nil {
		...
		return *new(V), false, fmt.Errorf("redis cluster get failed: %w", err)
	}
	...
	if err != nil {
		return *new(V), false, fmt.Errorf("failed to unmarshal value from Redis Cluster: %w", err)
	}
...
func (a *RedisClusterAdapter[K, V]) Set(ctx context.Context, key K, value V, tags ...string) error {
	...
	timeoutCtx, cancel := context.WithTimeoutCause(ctx, a.operationTimeout, fmt.Errorf("redis cluster Set exceeded %s timeout", a.operationTimeout))
	defer cancel()
	...
	if a.needsJSONStorage() {
		...
		if err := a.client.Expire(timeoutCtx, keyString, ttl).Err(); err != nil {
			return fmt.Errorf("failed to set TTL on JSON document: %w", err)
		}
	} else {
		...
		if err != nil {
			return fmt.Errorf("failed to marshal value for Redis Cluster: %w", err)
		}
...
func (a *RedisClusterAdapter[K, V]) SetWithTTL(ctx context.Context, key K, value V, ttl time.Duration, tags ...string) error {
	...
	timeoutCtx, cancel := context.WithTimeoutCause(ctx, a.operationTimeout, fmt.Errorf("redis cluster SetWithTTL exceeded %s timeout", a.operationTimeout))
	defer cancel()
	...
	valBytes, err := encoder.MarshalAny(value)
	if err != nil {
		l.Warn("Failed to marshal value for Redis Cluster", logger.String(logKeyField, keyString), logger.Error(err))
		return fmt.Errorf("marshal failed: %w", err)
	}

	if err := a.client.Set(timeoutCtx, keyString, valBytes, ttl).Err(); err != nil {
		l.Warn("Redis Cluster Set with TTL failed", logger.String(logKeyField, keyString), logger.Error(err))
...
func (a *RedisClusterAdapter[K, V]) prepareBulkSetItem(ctx context.Context, key K, value V, defaultTTL time.Duration) (string, []byte, time.Duration, bool) {
	...
	valBytes, err := encoder.MarshalAny(value)
	if err != nil {
		l.Warn("Failed to marshal value for Redis Cluster in BulkSet",
			logger.String(logKeyField, keyString), logger.Error(err))
		return "", nil, 0, false
	}
...
func (a *RedisClusterAdapter[K, V]) BulkSet(ctx context.Context, items map[K]V, tags ...string) error {
	...
	timeoutCtx, cancel := context.WithTimeoutCause(ctx, a.bulkOperationTimeout, fmt.Errorf("redis cluster BulkSet exceeded %s timeout", a.bulkOperationTimeout))
	defer cancel()
	...
	for key, value := range items {
		...
		for _, tag := range tags {
			pipe.SAdd(timeoutCtx, tagPrefix+clusterHashTag(a.namespace+tag), keyString)
		}
...
//
// Returns error when the operation fails.
func (a *RedisClusterAdapter[K, V]) Invalidate(ctx context.Context, key K) error {
	ctx, l := logger.From(ctx, log)
	...
	timeoutCtx, cancel := context.WithTimeoutCause(ctx, a.operationTimeout, fmt.Errorf("redis cluster Invalidate exceeded %s timeout", a.operationTimeout))
	defer cancel()
...
// Returns error when the encoder cannot be found or encoding fails.
func (a *RedisClusterAdapter[K, V]) encodeValue(value V) ([]byte, error) {
...
func (a *RedisClusterAdapter[K, V]) bulkEncodeKeys(ctx context.Context, keys []K) ([]string, map[string]K) {
	_, l := logger.From(ctx, log)
...
// processBulkGetResult handles a single result from a Redis MGET response.
//
...
func (a *RedisClusterAdapter[K, V]) processBulkGetResult(ctx context.Context, value any, keyString string) (V, bool) {
	...
	if !ok {
		l.Warn("Unexpected value type from Redis Cluster MGET", logger.String(logKeyField, keyString))
		return zero, false
	}
	...
	if err != nil {
		l.Warn("Failed to unmarshal value from Redis Cluster MGET", logger.String(logKeyField, keyString), logger.Error(err))
		return zero, false
	}
...
func (a *RedisClusterAdapter[K, V]) storeLoadedValues(ctx context.Context, loaded map[K]V, results map[K]V) {
	ctx, l := logger.From(ctx, log)
	...
	for k, v := range loaded {
		...
		if marshalErr != nil {
			l.Warn("Failed to marshal loaded value for Redis Cluster", logger.String(logKeyField, keyString), logger.Error(marshalErr))
			continue
		}
...
func (a *RedisClusterAdapter[K, V]) BulkGet(ctx context.Context, keys []K, bulkLoader cache.BulkLoader[K, V]) (map[K]V, error) {
	...
	pipe := a.client.Pipeline()
	getCommands := make([]*redis.StringCmd, len(keyStrs))
	for i, keyString := range keyStrs {
		getCommands[i] = pipe.Get(ctx, keyString)
	}
	_, _ = pipe.Exec(ctx)

	var misses []K
	for i, command := range getCommands {
		keyString := keyStrs[i]
...
// Returns error when the operation fails.
func (a *RedisClusterAdapter[K, V]) InvalidateByTags(ctx context.Context, tags ...string) (int, error) {
	if err := ctx.Err(); err != nil {
		return 0, err
	}

	timeoutCtx, cancel := context.WithTimeoutCause(ctx, a.bulkOperationTimeout, fmt.Errorf("redis cluster InvalidateByTags exceeded %s timeout", a.bulkOperationTimeout))
	defer cancel()
...
func (a *RedisClusterAdapter[K, V]) flushClusterUnsafe(ctx context.Context) error {
	...
	l.Warn("Executing FLUSHDB on Redis Cluster. ALL keys on ALL nodes will be deleted.",
	...
	if err != nil {
		return fmt.Errorf("redis cluster FLUSHDB failed: %w", err)
	}
...
func (a *RedisClusterAdapter[K, V]) deleteBatch(ctx context.Context, batch []string) int {
	ctx, l := logger.From(ctx, log)
...
func (a *RedisClusterAdapter[K, V]) invalidateByNamespace(ctx context.Context) error {
	...
	err := a.client.ForEachMaster(ctx, func(ctx context.Context, client *redis.Client) error {
		...
		for scanIterator.Next(ctx) {
			...
			if len(batch) >= scanBatchSize {
				deletedCount.Add(int64(a.deleteBatch(ctx, batch)))
				batch = batch[:0]
			}
		}
	...
	l.Internal("InvalidateAll completed",
		logger.String("namespace", a.namespace),
		logger.Int("keys_deleted", int(deletedCount.Load())))
...
func (a *RedisClusterAdapter[K, V]) InvalidateAll(ctx context.Context) error {
	...
	timeoutCtx, cancel := context.WithTimeoutCause(ctx, a.flushTimeout, fmt.Errorf("redis cluster InvalidateAll exceeded %s timeout", a.flushTimeout))
	defer cancel()
...
func (a *RedisClusterAdapter[K, V]) BulkRefresh(ctx context.Context, keys []K, bulkLoader cache.BulkLoader[K, V]) {
	...
	go func() {
		defer goroutine.RecoverPanic(ctx, "cache.redisClusterBulkRefresh")
...
func (a *RedisClusterAdapter[K, V]) Refresh(ctx context.Context, key K, loader cache.Loader[K, V]) <-chan cache.LoadResult[V] {
	...
	go func() {
		...
		defer goroutine.RecoverPanic(ctx, "cache.redisClusterRefresh")
...
func (a *RedisClusterAdapter[K, V]) Search(ctx context.Context, query string, opts *cache.SearchOptions) (cache.SearchResult[K, V], error) {
	if a.schema == nil {
	...
	timeoutCtx, cancel := context.WithTimeoutCause(ctx, a.searchTimeout, fmt.Errorf("redis cluster Search exceeded %s timeout", a.searchTimeout))
	defer cancel()
...
func (a *RedisClusterAdapter[K, V]) Query(ctx context.Context, opts *cache.QueryOptions) (cache.SearchResult[K, V], error) {
	if a.schema == nil {
		return cache.SearchResult[K, V]{}, fmt.Errorf(
	...
	timeoutCtx, cancel := context.WithTimeoutCause(ctx, a.searchTimeout, fmt.Errorf("redis cluster Query exceeded %s timeout", a.searchTimeout))
	defer cancel()
...
func (a *RedisClusterAdapter[K, V]) SupportsSearch() bool {
	return a.schema != nil
}
...
func (a *RedisClusterAdapter[K, V]) GetSchema() *cache.SearchSchema {
	return a.schema
}
