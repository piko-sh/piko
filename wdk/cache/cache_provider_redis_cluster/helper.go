...
func createRedisClusterCache(p *RedisClusterProvider, namespace string, optionsAny any) (any, error) {
...
func createNamespaceGeneric[K comparable, V any](p *RedisClusterProvider, namespace string, options cache.Options[K, V]) (cache.Cache[K, V], error) {
	...
	if existing, exists := p.caches[namespace]; exists {
		if cachedEntry, ok := existing.(cache.Cache[K, V]); ok {
			l.Internal("Reusing existing Redis Cluster namespace",
	...
	adapter := &RedisClusterAdapter[K, V]{
	...
	l.Internal("Created new Redis Cluster namespace",
		logger.String("namespace", namespace),
