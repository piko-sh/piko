...
func createValkeyClusterCache(p *ValkeyClusterProvider, namespace string, optionsAny any) (any, error) {
	switch opts := optionsAny.(type) {
	case cache.Options[string, []byte]:
		return createNamespaceGeneric(p, namespace, opts)
	...
	default:
		...
		return nil, fmt.Errorf(
			"unsupported cache type %v - for custom domain types, register a ProviderFactoryBlueprint "+
				"via cache.RegisterProviderFactory()",
			optionsType,
		)
	}
}
...
func createNamespaceGeneric[K comparable, V any](p *ValkeyClusterProvider, namespace string, options cache.Options[K, V]) (cache.Cache[K, V], error) {
	...
	if existing, exists := p.caches[namespace]; exists {
		if cachedEntry, ok := existing.(cache.Cache[K, V]); ok {
			l.Internal("Reusing existing Valkey Cluster namespace",
	...
	adapter := &ValkeyClusterAdapter[K, V]{
	...
	l.Internal("Created new Valkey Cluster namespace",
		logger.String("namespace", namespace),
