...
func TestRedisAdapter_StructKeyEncoding(t *testing.T) {
	t.Parallel()
	...
	t.Run("Backward compatibility: primitive keys work without KeyRegistry", func(t *testing.T) {
		...
		val2, found2, err2 := adapter.GetIfPresent(ctx, "another-key")
...
func TestRedisAdapter_NamespaceIsolation(t *testing.T) {
	t.Parallel()
	...
	t.Run("Different namespaces isolate caches", func(t *testing.T) {
		...
		val1, found1, err1 := adapter1.GetIfPresent(ctx, "shared-key")
		require.NoError(t, err1)
		val2, found2, err2 := adapter2.GetIfPresent(ctx, "shared-key")
		require.NoError(t, err2)
	...
	t.Run("InvalidateAll respects namespace boundaries", func(t *testing.T) {
		...
		_, found1, err1 := adapter1.GetIfPresent(ctx, "key1")
		require.NoError(t, err1)
		val2, found2, err2 := adapter2.GetIfPresent(ctx, "key2")
		require.NoError(t, err2)
...
func TestRedisAdapter_InvalidateAllSafety(t *testing.T) {
	t.Parallel()
	...
	t.Run("InvalidateAll uses FLUSHDB with unsafe flag and no namespace", func(t *testing.T) {
		...
		t.Skip("Skipping dangerous FLUSHDB test - would delete entire database")
	})
...
func createTestAdapter[K comparable, V any](t *testing.T, config Config) *RedisAdapter[K, V] {
	t.Helper()
	...
	require.NoError(t, err, "Failed to create Redis adapter")

	adapter, ok := c.(*RedisAdapter[K, V])
	require.True(t, ok, "Cache should be RedisAdapter")
...
func (l *noopBulkLoader[K, V]) BulkLoad(_ context.Context, _ []K) (map[K]V, error) {
	return make(map[K]V), nil
}

func (l *noopBulkLoader[K, V]) BulkReload(_ context.Context, _ []K, _ []V) (map[K]V, error) {
	return make(map[K]V), nil
}
