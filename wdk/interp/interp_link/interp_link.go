...
type LinkedFunction struct {
	...
	//
	// The synthesiser uses these descriptors when rebuilding the generic's go/types
	// signature for user code to type-check against. Empty or nil indicates legacy Wrap
	// callers; the synthesiser falls back to reading the sibling's signature directly in
	// that case.
...
// This short form is retained for backwards compatibility; new registrations should
// prefer WrapFunc which also records the generic's original parameter and return shape.
//
...
//
// Extract's codegen emits WrapFunc whenever the generic's signature references type
// parameters outside of the direct T position, because inferring the shape from the
// sibling's reflect signature alone is ambiguous once slices, maps, pointers, or
// generic-typed returns are involved.
//
// Takes typeArgCount (int) which is the number of type parameters on the generic source.
// Takes target (any) which is the sibling function value.
