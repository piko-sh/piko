...
const (
	// defaultBatchSize is the number of events buffered before triggering a flush signal.
	defaultBatchSize = 25

	// maxEventsPerRequest is the hard limit imposed by the GA4 Measurement Protocol.
	...
	// paramPageLocation is the GA4 parameter key for the full page URL.
	paramPageLocation = "page_location"

	// paramsInitialCapacity is the pre-allocated size for GA4 event parameter maps, covering
	// the typical number of standard fields (page_location, page_referrer, language,
	// duration_ms, status_code, action_name, currency, value, timestamp_micros, plus
	// headroom for user properties).
...
// payload is the top-level JSON body sent to the GA4 Measurement Protocol endpoint.
type payload struct {
...
// WithBatchSize sets the maximum number of events buffered before triggering a flush. The
// value is clamped to the GA4 maximum of 25 events per request.
//
...
// WithTimeout sets the HTTP client timeout for GA4 POSTs. Defaults to 10 seconds.
//
...
// WithClientIDFunc sets a custom function to derive the GA4 client_id from request data.
// The default function hashes the client IP and user agent with SHA-256.
//
// Takes fn (func(clientIP, userAgent string) string) which derives the client ID.
//
...
// WithRetry enables retry with exponential backoff for failed batch sends. Only retryable
...
// WithDebug enables the GA4 debug endpoint which validates events but does not record
// them in Google Analytics.
//
...
// Collector sends analytics events to the Google Analytics 4 Measurement Protocol. Events
// are buffered internally and flushed when the batch reaches batchSize or the
...
type Collector struct {
	...
	// paramsPool recycles map[string]any instances to avoid per-event allocation in Collect.
	...
	// circuitBreakerConfig holds optional circuit breaker settings. Nil disables the circuit
	// breaker.
	...
	// endpoint is the full GA4 Measurement Protocol URL including measurement_id and
	// api_secret query parameters.
	endpoint string

	// flushInterval is the time between automatic timer-based flushes.
	flushInterval time.Duration

	// batchSize is the number of events that triggers an immediate flush signal.
	batchSize int
...
// NewCollector creates an analytics collector that sends events to the GA4 Measurement
// Protocol endpoint.
//
// Takes measurementID (string) which is the GA4 property measurement ID (e.g.
// "G-XXXXXXXXXX").
// Takes apiSecret (string) which is the Measurement Protocol API secret created in GA4
// Admin.
// Takes opts (...Option) which configure the collector.
//
// Returns analytics.Collector which sends events to GA4.
// Returns error when measurementID or apiSecret is empty, or when the batcher cannot be
// created.
...
// Start launches the background flush loop. Called by the analytics Service after
// registration.
...
// asynchronously; Collect itself never performs I/O.
//
...
// resolveEventName determines the GA4 event name from the analytics event type and
// fields.
//
// Takes event (*analytics_dto.Event) which carries the event data.
//
...
// mapParams populates the GA4 parameter map from standard event fields and revenue data.
//
...
// mapRevenue extracts currency and monetary value from a Money pointer into GA4
// parameters.
//
// Takes params (map[string]any) which is the target parameter map.
...
//
// Any remaining buffered events should be flushed via Flush before calling Close. Safe to
// call multiple times.
//
// Returns error which is always nil.
...
// sendBatch groups events by clientID and POSTs each group to the GA4 Measurement
// Protocol endpoint. GA4 requires a single client_id per request body, so different
// clients produce separate requests.
//
...
// sendChunk POSTs a single GA4 request body for the given clientID, userID, and event
// chunk.
//
...
// acquireParams returns a cleared map from the pool, or allocates a new one on pool miss.
//
