...
// Loader handles loading configuration from multiple sources into a struct. It runs a
// series of passes to fill in values step by step.
...
// Resolver is the interface for custom secret resolvers. Resolvers process placeholder
// strings (e.g., "aws-ssm:my-secret") and replace them with actual values.
type Resolver = config_domain.Resolver

// BatchResolver is an optimised resolver that can resolve multiple secrets in one
// operation.
...
// EnvResolver resolves placeholders like "env:MY_VAR" to environment variable values.
type EnvResolver = config_domain.EnvResolver

// Base64Resolver resolves placeholders like "base64:SGVsbG8=" by decoding base64 strings.
type Base64Resolver = config_domain.Base64Resolver

// FileResolver resolves placeholders like "file:/path/to/secret.txt" by reading file
// contents.
type FileResolver = config_domain.FileResolver

// RegisterFlags registers a config struct's flags with the global flag coordinator.
//
// In most cases, manual registration is unnecessary. Flag registration happens
// automatically when you call Load with a FlagPrefix. Manual registration is only needed
// in advanced scenarios where you need to register flags before calling Load, such as
// when implementing custom flag parsing logic.
//
// The prefix is prepended to all flag names. For example, if prefix is "app" and a field
// has `flag:"dbUrl"`, the resulting flag will be "--app.dbUrl". Pass an empty string for
// prefix to register flags without a prefix.
//
...
// ParseFlags parses os.Args using the global flag coordinator. This is called
// automatically during Load() if the PassFlags pass is enabled, but can be called
// manually if you need to parse flags before loading.
//
...
// RegisterResolver registers a resolver in the global resolver registry. Loaders with
// UseGlobalResolvers=true will automatically inherit all global resolvers.
//
...
// NewLoader creates a new loader with the given options. For most use cases, use the Load
...
// WithDefaultResolvers is a functional option that registers the built-in dependency-free
// resolvers (env:, base64:, file:).
//
...
// Load creates a new Loader and loads configuration into the provided struct pointer. It
// automatically includes default resolvers.
//
...
// Takes opts (LoaderOptions) which specifies file paths, prefixes, and other loading
// options.
//
...
//
// When sandbox is nil, the resolver creates per-file sandboxes for each file read. For
// improved security, provide a sandbox to restrict file access to a specific directory.
//
