...
var (
	// log is the package-level logger for the email_provider_mailgun package.
	log = logger.GetLogger("piko/email/email_provider_mailgun")

	// Meter is the package-level meter for Mailgun provider metrics.
	Meter = otel.Meter("piko/email/email_provider_mailgun")
...
func init() {
	...
	SendTotal, err = Meter.Int64Counter(
		"email.provider.mailgun.send.total",
	...
	SendDuration, err = Meter.Float64Histogram(
		"email.provider.mailgun.send.duration",
