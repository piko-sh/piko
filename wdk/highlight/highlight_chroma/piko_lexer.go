...
var (
	...
	//
	// It supports SFC blocks (<template>, <script>, <style>, <i18n>), script variations (Go
	// and TypeScript), Piko directives (p-if, p-for, p-bind, etc.) with modifiers, shorthand
	// syntax (:attr for binding, @event for events), interpolation ({{ expression }}), and
	// embedded language highlighting for Go, TypeScript, CSS, and JSON.
	PikoLexer = chromalib.MustNewLexer(
		&chromalib.Config{
			Name:            "Piko",
