...
const (
	subprocessEnvVar = "PIKO_EXAMPLES_SUBPROCESS"
...
func TestMain(m *testing.M) {
	...
	tlsMode := os.Getenv("PIKO_E2E_TLS") == "1"
...
func TestExamples_Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping examples integration tests in short mode")
	}
	...
	if err != nil {
		t.Fatalf("Critical test setup error: Failed to read scenarios directory at '%s': %v", scenariosRoot, err)
	}
	...
	for _, entry := range entries {
		...
		tc := testCase{
			Name: testCaseName,
			Path: filepath.Join(scenariosRoot, testCaseName),
		}
...
func runTestInSubprocess(t *testing.T, testName string) {
	...
	testPattern := "TestExamples_Integration/" + testName
	...
	specPath := filepath.Join("scenarios", testName, "testspec.json")
	if spec, specErr := browserpkg.LoadTestSpec(specPath); specErr == nil && spec.TLS {
		env = append(env, "PIKO_E2E_TLS=1")
	}
	command.Env = env
