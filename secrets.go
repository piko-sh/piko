...
// Secret provides lazy-loaded, secure secret handling for configuration values. Unlike
// regular config fields, Secret[T] stores the resolver reference and resolves on-demand
...
// registration, stats tracking, and graceful shutdown. SecretManager is a singleton; use
// GetSecretManager to access it.
...
var (
	...
	// ErrSecretClosed is returned when trying to acquire a secret that has been closed.
	...
	// ErrNoResolver is returned when no resolver is available for the secret's prefix.
	ErrNoResolver = config_domain.ErrNoResolver
)

// GetSecretManager returns the singleton secret manager. Use this to access secret
// statistics or manually trigger shutdown.
//
// Returns *SecretManager which provides access to secret management operations.
//
// Example: stats := piko.GetSecretManager().Stats() log.Info("Active secrets", "count",
// stats.TotalSecrets)
