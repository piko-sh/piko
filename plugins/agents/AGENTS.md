...
Piko is a Go-based server-side rendering framework with single-file components (.pk), client-side Web Components (.pkc), file-based routing, partials with slots, server actions, i18n, and integrated tooling.

Use this when generating, reviewing, or refactoring Piko templates, components, pages, partials, actions, or project configuration.
...
All reference files live in `references/` relative to this file.
