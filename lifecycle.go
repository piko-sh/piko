...
const (
	// zeroDurationText is the textual probe duration reported when a probe completes without
	// a measured elapsed time.
	zeroDurationText = "0s"
)

// LifecycleComponent defines the contract for components that require lifecycle
// management.
//
// Components implementing LifecycleComponent have their OnStart method called during
// server startup (before accepting HTTP traffic) and their OnStop method called during
// graceful shutdown.
//
// Lifecycle components are executed in the order they are registered. During shutdown,
// they are stopped in reverse order (last registered, first stopped).
//
...
type LifecycleComponent interface {
	// OnStart runs when the server starts up. Use this to set up resources, open
	// connections, run migrations, and so on.
	//
	// Returns error to stop the server from starting.
	//
	// The context has a timeout. If OnStart does not finish in time, the server will fail to
	// start.
...
// LifecycleStartTimeout is an optional interface that lifecycle components can implement
// to override the default 30-second startup timeout. Components performing long-running
// initialisation (such as document ingestion or database migrations) should implement
// this to request more time.
//
// Example:
//
...
// LifecycleHealthProbe provides health monitoring for components. It is an alias to the
// internal healthprobe_domain.Probe interface, allowing external packages to implement
// health probes without importing internal packages.
//
// Probes are called periodically by the health monitoring system and exposed via HTTP
// endpoints. Components can implement both LifecycleComponent and LifecycleHealthProbe to
// get both lifecycle management and health monitoring.
//
// Example implementation:
//
...
// LifecycleWithHealth combines lifecycle management and health probing for components
// that need both managed startup/shutdown and runtime health checks.
//
...
// lifecycleProbeAdapter adapts a LifecycleComponent to the internal Probe interface. If
// the component implements LifecycleHealthProbe, its Check method is used directly;
// otherwise a basic "component exists" check is provided.
type lifecycleProbeAdapter struct {
	// component is the lifecycle component being adapted as a health probe.
	component LifecycleComponent

	// probe is the health probe for the component; nil if the component does not implement
	// LifecycleHealthProbe.
	probe LifecycleHealthProbe
}

// Name returns the probe's display name, implementing healthprobe_domain.Probe.
//
...
// Check implements healthprobe_domain.Probe by returning the health status of the adapted
// component.
//
...
func (a *lifecycleProbeAdapter) Check(ctx context.Context, checkType healthprobe_dto.CheckType) healthprobe_dto.Status {
	if a.probe != nil {
		status, err := goroutine.SafeCall1(ctx, "lifecycle.probe."+a.component.Name(),
			func() (healthprobe_dto.Status, error) {
				return a.probe.Check(ctx, checkType), nil
			})
		if err != nil {
