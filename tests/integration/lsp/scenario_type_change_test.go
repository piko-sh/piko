...
func testScenarioTypeChange(t *testing.T) {
	...
	originalTypes := harness.readFile("types.go")
	modifiedTypes := strings.Replace(
		originalTypes,
		"Title string",
		"Title       string\n\tDescription string",
		1,
	...
	harness.writeFile("types.go", modifiedTypes)
	...
	require.NoError(t, err, "completion after type change should not error")

	_ = result

	assert.Empty(t, client.GetErrors(), "no protocol errors during type change")
}
