...
func TestMain(m *testing.M) {
	...
	var err error
	timescaleContainer, testConnectionString, err = startTimescaleDBContainer(ctx)
	if err != nil {
		log.Fatalf("starting timescaledb container: %v", err)
	}

	exitCode := m.Run()

	if timescaleContainer != nil {
		_ = timescaleContainer.Terminate(context.Background())
	}
...
func TestQuerierTimescaleDBIntegration(t *testing.T) {
	if testing.Short() {
