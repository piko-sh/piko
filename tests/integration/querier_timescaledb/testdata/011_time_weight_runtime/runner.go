...
func main() {
	...
	for _, sample := range samples {
		err = queries.InsertGaugeSample(ctx, db.InsertGaugeSampleParams{
			Ts:    baseline.Add(time.Duration(sample.offsetSeconds) * time.Second),
			Value: sample.value,
		})
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
	...
	row, err := queries.TimeWeightedAverage(ctx)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
