...
func runTestCase(t *testing.T, testCaseDirectory string) {
	...
	schemaName := createIsolatedSchema(t, testCaseDirectory)
	...
	output := executeRunner(t, tempDirectory, schemaName)
	goldenPath := filepath.Join(testCaseDirectory, "golden", "output.json")
	assertGoldenJSON(t, goldenPath, output)
}

func createIsolatedSchema(t *testing.T, testCaseDirectory string) string {
	t.Helper()

	schemaName := "test_" + strings.ReplaceAll(filepath.Base(testCaseDirectory), "-", "_")

	connection, err := sql.Open("pgx", testConnectionString)
	require.NoError(t, err)
	defer connection.Close()

	ctx := context.Background()

	_, err = connection.ExecContext(ctx, "CREATE EXTENSION IF NOT EXISTS timescaledb CASCADE")
	require.NoError(t, err)

	_, err = connection.ExecContext(ctx, fmt.Sprintf("DROP SCHEMA IF EXISTS %s CASCADE", schemaName))
	require.NoError(t, err)

	_, err = connection.ExecContext(ctx, fmt.Sprintf("CREATE SCHEMA %s", schemaName))
	require.NoError(t, err)

	_, err = connection.ExecContext(ctx, fmt.Sprintf("SET search_path TO %s, public", schemaName))
	require.NoError(t, err)
	...
	for _, entry := range entries {
		...
		migrationSQL := fmt.Sprintf("SET search_path TO %s, public;\n%s", schemaName, string(migrationContent))
		_, execError := connection.ExecContext(ctx, migrationSQL)
		require.NoError(t, execError, "executing migration %s", entry.Name())
	}

	t.Cleanup(func() {
		cleanupConnection, cleanupError := sql.Open("pgx", testConnectionString)
		if cleanupError != nil {
			return
		}
		defer cleanupConnection.Close()
		_, _ = cleanupConnection.ExecContext(context.Background(), fmt.Sprintf("DROP SCHEMA IF EXISTS %s CASCADE", schemaName))
	})
...
func writeGeneratedFiles(t *testing.T, tempDirectory string, files []querier_dto.GeneratedFile) {
	...
	if os.Getenv("PIKO_DEBUG_GENERATED") != "" {
		t.Logf("generated files in %s:", tempDirectory)
		for _, file := range files {
...
func executeRunner(t *testing.T, tempDirectory string, schemaName string) []byte {
	t.Helper()
...
func assertGoldenJSON(t *testing.T, goldenPath string, actual []byte) {
	...
	normalisedExpected := normaliseNonDeterministicFields(t, expectedBytes)
	normalisedActual := normaliseNonDeterministicFields(t, prettyActual)

	assert.JSONEq(t, string(normalisedExpected), string(normalisedActual), "golden file mismatch: %s", goldenPath)
}

func normaliseNonDeterministicFields(t *testing.T, data []byte) []byte {
	t.Helper()

	var parsed any
	require.NoError(t, json.Unmarshal(data, &parsed))

	normaliseIDFields(parsed)
...
func normaliseIDFields(value any) {
	switch typed := value.(type) {
	case map[string]any:
		for key, child := range typed {
			if key == "ID" || key == "JobID" || key == "ChunkName" {
				typed[key] = ""
			} else {
