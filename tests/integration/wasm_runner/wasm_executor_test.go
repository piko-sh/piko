...
func (e *WASMExecutor) DynamicRender(ctx context.Context, request *wasm_dto.DynamicRenderRequest) (*wasm_dto.DynamicRenderResponse, error) {
	result, err := e.callWASM(ctx, "dynamicRender", request)
	if err != nil {
		return nil, err
	}

	var response wasm_dto.DynamicRenderResponse
	if err := mapToStruct(result, &response); err != nil {
		return nil, fmt.Errorf("failed to parse dynamicRender response: %w", err)
	}
