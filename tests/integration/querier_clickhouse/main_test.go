...
func TestMain(m *testing.M) {
	...
	var err error
	clickhouseContainer, testConnectionString, err = startClickHouseContainer(ctx)
	if err != nil {
		log.Fatalf("starting clickhouse container: %v", err)
	}
...
func TestQuerierClickHouseIntegration(t *testing.T) {
	if testing.Short() {
	...
	for _, name := range names {
		t.Run(name, func(t *testing.T) {
			runTestCase(t, filepath.Join(testdataDirectory, name))
		})
	}
}
