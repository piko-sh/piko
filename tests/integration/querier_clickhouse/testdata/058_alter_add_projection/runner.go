...
func run() error {
	...
	events := []db.InsertEventParams{
		...
		{ID: 2, UserID: 10, Amount: 50},
		{ID: 3, UserID: 20, Amount: 200},
		{ID: 4, UserID: 30, Amount: 5},
		{ID: 5, UserID: 20, Amount: 25},
	}
	...
	rows, err := queries.UserTotals(ctx)
	if err != nil {
		return fmt.Errorf("user totals: %w", err)
	}
