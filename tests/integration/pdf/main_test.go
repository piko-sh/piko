...
const (
	subprocessEnvVar  = "PIKO_PDF_SUBPROCESS"
	subprocessTimeout = 5 * time.Minute
)

func TestMain(m *testing.M) {
	if os.Getenv("SKIP_BROWSER_TESTS") == "1" {
		fmt.Println("SKIP_BROWSER_TESTS=1, skipping PDF browser comparison tests")
		os.Exit(0)
	}
...
func TestPdfRendering_Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping PDF rendering integration tests in short mode")
	}
...
func runTestInSubprocess(t *testing.T, testName string) {
	...
	testPattern := "TestPdfRendering_Integration/" + testName
