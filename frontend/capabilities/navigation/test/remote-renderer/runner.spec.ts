...
import { discoverRemoteRendererTestCases, runRemoteRendererTestCase } from './harness';

describe('RemoteRenderer regression tests', () => {
    const testCases = discoverRemoteRendererTestCases();
    ...
    for (const tc of testCases) {
        describe(tc.name, () => {
            it(tc.spec.description, async () => {
                const result = await runRemoteRendererTestCase(tc);
                ...
                if (!result.passed) {
                    ...
                    if (result.sideEffectResults.length > 0) {
                        console.log('\nSide Effect Results:');
                        result.sideEffectResults.forEach(sr => {
                            const status = sr.passed ? '✓' : '✗';
                            console.log(`  ${status} ${sr.description}: ${sr.message}`);
                        });
                    }
