...
/**
 * Test-only hook manager that supports both `emit` (satisfying `HookManagerLike`)
 * and a fluent `api.on` / `setReady` for registering callbacks during assertions.
 */
interface TestHookManager {
...
function createHookManager(): TestHookManager {
...
let globalPageContext: PageContextLike & {loadModule: ReturnType<typeof vi.fn>} = createTestPageContext();

function createTestPageContext(): PageContextLike & {loadModule: ReturnType<typeof vi.fn>} {
    return {
        loadModule: vi.fn().mockResolvedValue(undefined),
        clear: vi.fn(),
        registerPartialInstance: vi.fn(),
    };
}

function resetGlobalPageContext(): void {
    globalPageContext = createTestPageContext();
}

function getGlobalPageContext(): PageContextLike & {loadModule: ReturnType<typeof vi.fn>} {
    return globalPageContext;
}

type HookManager = TestHookManager;
