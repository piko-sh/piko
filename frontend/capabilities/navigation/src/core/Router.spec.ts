...
import type {ErrorDisplayLike, FormStateManagerLike, HookManagerLike} from '@/coreServices';

/**
 * Test-only type aliases that extend the minimal `*Like` interfaces with the
 * extra methods the tests need to mock. The real services live in `@piko/core`,
 * but the navigation capability only depends on the slimmer `Like` contracts.
 */
interface ErrorDisplay extends ErrorDisplayLike {}

interface FormStateManager extends FormStateManagerLike {
    trackForm(): void;
    untrackForm(): void;
    markFormClean(): void;
...
interface HookManager extends HookManagerLike {
