...
/** Adapter for framework navigation, set by the capability entry point. */
let frameworkNavigate: ((url: string, evt: Event | undefined, options: RouterNavigateOptions) => Promise<void>) | null = null;

/**
 * Sets the framework navigation adapter.
 *
 * Called by the navigation capability entry point after the Router is created.
 *
 * @param navigateFn - The Router's navigateTo function.
 */
export function _setNavigateAdapter(navigateFn: (url: string, evt: Event | undefined, options: RouterNavigateOptions) => Promise<void>): void {
 ...
 *
 * @returns The navigation adapter, or null if not yet initialised.
 */
    ...
    if (frameworkNavigate) {
        return {
            navigate: (url: string, options?: NavigateOptions) => {
                ...
                return frameworkNavigate!(url, undefined, routerOptions);
            }
        };
    }
    return null;
