---
title: Directives
description: Complete reference for all PK directives
nav:
  sidebar:
    section: "guide"
    subsection: "concepts"
...
Directives are special attributes that control rendering, handle events, bind data, and add behaviour to elements in your PK templates.

## Conditional rendering

For advanced conditional and loop patterns, see [conditionals & loops](/docs/guide/conditionals-loops).

### `p-if`

Conditionally render an element based on a boolean expression. When the condition is false, the element is removed from the DOM entirely.
...
> **Key Point**: `p-else-if` and `p-else` must immediately follow a `p-if` or another `p-else-if` at the same nesting level. Only the first matching condition renders.
...
Toggle element visibility using CSS `display` property. Unlike `p-if`, the element remains in the DOM but is hidden.
...
> **Note**: Map iteration order is deterministic (sorted by key) in Piko for consistent rendering.
...
Provide a unique identifier for list items to enable efficient DOM reconciliation. Always use `p-key` when iterating over lists where items may be added, removed, or reordered.
...
> **Security Warning**: Never use `p-html` with user input or untrusted content. This bypasses HTML escaping and can lead to XSS (Cross-Site Scripting) attacks. Only use with content you fully control and trust.
...
Create two-way data binding between form inputs and state. The input value is synchronised with the specified state field.
...
For `p-collection`, `p-provider`, and content-driven routing, see [Collections](/docs/guide/collections).

## Directive combinations

Directives can be combined for flexible patterns.
...
Now that you understand the directive system, explore:

- [Template syntax](/docs/guide/template-syntax) → Expressions and interpolation
- [Partials](/docs/guide/partials) → Reusable components
- [Collections](/docs/guide/collections) → Content-driven pages
