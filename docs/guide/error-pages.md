---
title: Error pages
description: Custom error pages using the ! prefix convention
nav:
  sidebar:
    section: "guide"
...
Piko supports custom error pages using a convention-based `!` prefix. When an error occurs, whether a route doesn't match, a Render function fails, or you explicitly return a typed error, Piko renders the appropriate error page with the correct HTTP status code.
...
Error pages are **not** routable; they don't create URL routes. They are only rendered when an error occurs.

> **Note**: The `!` prefix was chosen because it's valid on all platforms (Linux, macOS, Windows), visually signals "not a normal page", and doesn't conflict with `_` (private partials) or `{param}` (dynamic routes). Only the three formats above are valid; any other `!`-prefixed `.pk` file will cause a build error.
...
When a visitor navigates to a URL that doesn't match any route, Piko renders this page with HTTP status 404.
...
> **Note**: `piko.GetErrorContext()` returns `nil` when the page is not being rendered as an error page. This allows you to write error pages that also work as regular pages during development.
...
Piko picks `pages/app/!404.pk` because it's an exact match **and** has the most specific scope.
...
This lets you create different error experiences for different sections of your application, such as a branded error page for the marketing site and a more functional one for the application dashboard.
...
This is useful when you want a single error page rather than one per status code. Exact matches (`!404.pk`) and ranges (`!400-499.pk`) take priority over catch-all.
...
When using `p-collection` with a dynamic route (e.g., `pages/blog/{slug}.pk`), navigating to a slug that doesn't exist in the collection automatically triggers a 404 error, which renders the appropriate `!404.pk` error page.

This means you don't need to manually handle missing collection items. Piko detects the missing item and falls through to your error page hierarchy automatically.
...
|--------|-------------|----------|
| `piko.NotFound(resource, id)` | 404 | Resource doesn't exist |
...
| `piko.Teapot(message)` | 418 | I'm a teapot (RFC 2324) |
...
Returning a plain `error` (or any error that doesn't implement `ActionError`) results in HTTP 500 and renders `!500.pk`:
...
When no custom error page exists for a given status code and scope:

- **404**: Returns plain text `404 page not found`
- **500**: Returns plain text `Internal Server Error`
- **Other codes**: Returns the appropriate HTTP status with a plain text message

Custom error pages are entirely optional. Your application works fine without them; they simply let you provide a better user experience.

## Scaffold default

When you create a new Piko project with `piko new`, a `pages/!404.pk` is included in the scaffold. It provides a simple, styled 404 page that uses `piko.GetErrorContext()` to display relevant details. You can customise it or delete it as needed.

## Next steps

- [Routing](/docs/guide/routing) → File-based routing and dynamic routes
- [Metadata](/docs/guide/metadata) → HTTP status codes and redirects
