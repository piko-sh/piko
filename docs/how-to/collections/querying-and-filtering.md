...
This guide covers advanced collection operations not found in the [basic Collections guide](../../reference/collections-api.md). You should be familiar with `piko.GetData[T](r)` and basic collection usage before reading this.
...
## Hybrid caching with Incremental Static Regeneration

Hybrid mode combines Incremental Static Regeneration, also known as `ISR`, with runtime revalidation for optimal performance.
...
| `content-hash` | Hash of file contents | When modtimes are not reliable |
...
Piko supports three locale patterns:
...
Piko links items across locales with a translation key:
...
The `FetchOptions` struct configures how Piko fetches collections:
...
- [Collections](../../reference/collections-api.md) -> Basic collection setup, declarative collections, piko:content
