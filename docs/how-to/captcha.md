...
The server action reads the token field from the input struct and passes it to `piko.Verify`:
...
func (a *SubmitAction) Call(input SubmitInput) (SubmitResponse, error) {
    if err := captcha.Verify(a.Ctx(), input.CaptchaToken, a.RemoteIP(), "contact_submit"); err != nil {
...
`DefaultScoreThreshold` and the two rate-limit fields on `ServiceConfig` apply across the service. Tune them when bootstrap builds the default captcha service. For example:

```go
piko.WithCaptchaServiceConfig(&captcha.ServiceConfig{
    DefaultScoreThreshold: 0.7,
    VerifyRateLimit:       10,
    ChallengeRateLimit:    15,
})
```

A higher threshold rejects more traffic. A lower threshold lets more through. The defaults (0.5 threshold, 20 verifies per IP per minute, 30 challenge calls per IP per minute) work for most sites.

## Troubleshoot verification failures

- `ErrTokenMissing`. The client did not include the token. Check the widget loaded and the form handler copied the value to the expected field.
- `ErrTokenExpired`. The user waited too long between solving and submitting. Re-challenge.
- `ErrScoreBelowThreshold`. The provider returned a score below `DefaultScoreThreshold`. Re-challenge the user, or tune the threshold for the action.
- `ErrProviderUnavailable`. The backend HTTP call failed. Piko treats this as a soft failure; surface a retry to the user, or fall back to a secondary challenge.

The action is free to inspect the underlying `VerifyResponse` for richer policy decisions by calling the service directly instead of the `Verify` shortcut.
