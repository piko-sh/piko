...
`Wait` accepts a duration directly (it sleeps for that long), and `WaitFor` polls for a selector to appear. `WaitForNetworkIdle(idleDuration, opts...)` requires the idle window. Pass `WaitOption`s (`WithTimeout`, etc.) to the polling waits as varargs:
