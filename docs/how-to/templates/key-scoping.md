...
The `p-context` directive declares a key-namespace boundary. Inside the boundary, the framework interprets `p-key` values relative to the context, so two sections of a template can use the same key without collision. For the full directive list see [directives reference](../../reference/directives.md).
