...
description: Use ${expression}, @key.path, and backslash escaping inside translation strings.
nav:
  sidebar:
    section: "how-to"
    subsection: "i18n"
...
Translation strings support three template constructs. The constructs are variable interpolation with `${expression}`, key references with `@key.path`, and backslash escaping for literal special characters. For the surrounding API see [i18n API reference](../../reference/i18n-api.md). For the rationale behind a small DSL inside translations see [about i18n](../../explanation/about-i18n.md).
...
Use `@key.path` to embed one translation inside another. The framework resolves linked keys recursively, capped at depth 10 to break circular references. Variables from the outer scope flow through to the linked translation:
...
Use a backslash to escape characters that would otherwise start an interpolation or reference:
...
The double backslash in JSON encodes a single backslash in the resulting string. The `support\@example.com` form prevents the parser from treating `@example` as a key reference.
...
`amount` and `name` resolve from binders. `@common.currency` resolves from another translation. The `\\$` keeps the literal dollar sign separate from the interpolation that follows.
