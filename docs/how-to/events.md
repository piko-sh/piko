...
The default provider is in-process GoChannel. For distributed messaging, pass an explicit provider:

```go
ssr := piko.New(
    piko.WithEventsProvider(events_provider_nats.New("nats://localhost:4222")),
)
```

The NATS, PostgreSQL, and SQLite providers live under `wdk/events/events_provider_*`.
