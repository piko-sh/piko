...
// Split 1:2:1 between three line items.
ratios := []maths.BigInt{
    maths.NewBigIntFromInt(1),
    maths.NewBigIntFromInt(2),
    maths.NewBigIntFromInt(1),
}

parts, err := totalMinor.Allocate(ratios...)
if err != nil {
    return err
}
...
total, err := cart.Add(shipping).Err()
if err != nil {
    return err
}
...
```go
matrix := maths.NewMatrixConverter(matrixConfig)
...
maths.SortDecimalsReverse(prices) // in-place descending sort
```
...
```go
label := total.Round(2).String()
```

`Money` also offers `RoundedDefaultFormat()` which applies locale-aware thousands separators and the currency symbol.

## Register a non-standard currency

```go
err := maths.RegisterCurrency("XTK", maths.CurrencyDefinition{
    DecimalPlaces: 8,
    Symbol:        "XTK",
})
if err != nil {
