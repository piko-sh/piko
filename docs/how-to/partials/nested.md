...
Specify `p-key` when looping partials so Piko can keep identities stable across updates. The leading colon evaluates each value as an expression. Piko matches the bare attribute name against the `Props` struct of the `product-card` partial. See [how to passing props to partials](passing-props.md) for the prefix-versus-bare attribute rules.
...
- [About PK files](../../explanation/about-pk-files.md) for the partial composition model.
