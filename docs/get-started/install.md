...
The scaffolded tree contains the folders a typical Piko project uses (`pages/`, `components/`, `actions/`, `content/`, `partials/`, `emails/`, `pdfs/`, `i18n/`, `cmd/`). See [Project structure](project-structure.md) for a folder-by-folder map.
...
|---|---|---|
| `dev` | `go run ./cmd/main/main.go dev` | Development mode with compiled templates and dev widgets. |
| `dev-i` | `go run ./cmd/main/main.go dev-i` | Interpreted mode. Fastest feedback loop for template edits. See [about interpreted mode](../explanation/about-interpreted-mode.md). |
| `prod` | `go run ./cmd/main/main.go prod` | Production mode. Dev widgets off, caching on, assets minified. |

The scaffolded `.air.toml` points at `dev-i` by default so every template save refreshes without a rebuild.
...
- Browse the [Showcase overview](../showcase/overview.md) for runnable example projects that match what you want to build.
