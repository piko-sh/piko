...
A PK file (`.pk` extension) is the single-file component format for server-rendered pages. Up to five sections live in one file. An HTML `<template>`, a Go `<script type="application/x-go">`, an optional client-side `<script lang="ts">`, a `<style>` block, and an `<i18n lang="json">` block. The Go script declares types, the template binds to those types, and the whole thing compiles to one Go function that returns HTML. See [pk-file format reference](../reference/pk-file-format.md) for the full grammar and [about PK files](../explanation/about-pk-files.md) for why Piko chose the single-file shape.

### PKC components

A PKC file (`.pkc` extension) is the client-side counterpart to a PK file. It compiles to a Web Component with reactive state, shadow DOM, and lifecycle hooks. Mount one inside a PK template by writing a custom-element tag (`<pp-counter />`) and the browser upgrades it on load. PKC handles everything the server cannot. Local state, click handlers, two-way input binding. See [client components reference](../reference/client-components.md) for the file format and [about reactivity](../explanation/about-reactivity.md) for the PK/PKC split.
...
> **Note:** Three filename conventions drive routing: `{slug}.pk` captures one URL segment, `{...slug}.pk` captures the trailing path, and a leading `!` flags an error page (`!404.pk`, `!500.pk`, `!error.pk`). Any other `!`-prefixed `.pk` is a build error. See [routing rules reference](../reference/routing-rules.md).
