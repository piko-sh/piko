...
Piko's `Get(ctx, key, loader)` serialises concurrent loads for the same key. The first caller runs the loader. Every other caller waiting for that key blocks on the first loader's result. When the loader finishes, all callers receive the value. Only one database query ran.

This is why application code should prefer `Get` (the loader-taking form) over the naive `GetIfPresent` / populate pattern. The latter works but requires the caller to re-implement the serialisation logic. The former gets it for free.
...
Transformers chain. `Options.Transformers` is an ordered list. Writes run forward through the list, and reads run backward. A natural chain puts compression first, then encryption. Small data ciphertext stays small. Reversing the order (encryption first, then compression) is pointless because encrypted output is incompressible.
...
Most caches are pure key-value. Piko's cache supports an optional search index because some use cases conflate the two. A product catalogue cache benefits from a `SearchOptions.Query` over `title` alongside the ability to fetch a single product by ID. Splitting the two (a cache for by-ID reads and a separate search index) duplicates the data and the invalidation logic.

Providers that ship a search index (Redis with RediSearch, some Valkey builds) expose it through Piko's `Search` method. Providers without search return `ErrSearchNotSupported`. Callers declare the schema at cache creation, and the provider indexes on write. Typed constructors (`Eq`, `Gt`, `Between`, `SortDesc`) build filters and sort orders instead of raw strings, keeping the query vocabulary small and verifiable at compile time.
...
- [Scenario 016: cached API](../showcase/016-cached-api.md) for a runnable example.
- [About the hexagonal architecture](about-the-hexagonal-architecture.md) for how the provider pattern fits the rest of the framework.
