...
A backend collector implements `analytics.Collector` (alias for `analytics_domain.Collector`, exported via [`wdk/analytics/facade.go`](https://github.com/piko-sh/piko/blob/master/wdk/analytics/facade.go)). The interface gives a collector its own lifecycle so it can batch, flush on shutdown, and identify itself in logs:

```go
type Collector interface {
    Start(ctx context.Context)
    Collect(ctx context.Context, event *analytics.Event) error
...
Piko does not prescribe how collectors handle batching, retry, or credentials. The GA4 and Plausible collectors in-tree share the same `analytics_domain.Batcher` pipeline. Each buffers events in `Collect` and flushes either when `batchSize` fills or `flushInterval` elapses. They apply exponential backoff with retry on send failures. The stdout collector takes a different shape. It forwards each event straight through to the structured logger as a single `Info` record with one `logger_domain.Attr` per event field. The on-disk format then matches whatever the logger sink produces (text under the default sink, JSON under a JSON sink). A custom collector picks whichever strategy matches its destination.
...
The frontend channel does not expose a global `track()` function. Instead the client runtime emits an `analytics:track` hook, and the bundled analytics extension subscribes through `pk.hooks.on('analytics:track', payload => ...)`. Application code forwards custom browser events through the same hook surface. This keeps the backend and frontend pipelines symmetrical without committing to a global API.

## What automatic enrichment adds

A collector does not need to populate every field of `analytics.Event` (alias for `analytics_dto.Event`). Piko fills in the fields it knows about before the collector runs:
...
The first choice. IP anonymisation is opt-in at the collector level, not at the middleware level. Piko passes the full client IP to the collector. Each collector decides whether to trim it before shipping. GA4 has its own IP anonymisation setting. Plausible never receives the IP in the first place. A stdout collector logs whatever Piko passes in. Centralising the choice inside each collector keeps each destination's contract with the application explicit.

The second choice. Piko never generates a persistent user identifier. Page views are anonymous by default. A value for `UserID` only appears if the application sets one, typically from an authenticated session. This avoids accidental tracking of logged-out visitors across sessions through a synthetic identifier.
...
- Whether to comply with region-specific consent regimes (GDPR, `CCPA`). Compliance is application-level, not Piko-level. A common pattern registers the collector conditionally, so declining consent skips the registration.

## Collector failure is non-fatal

A failed `Track` call does not fail the request. Analytics is observability, not business logic. A dropped event is acceptable. A dropped order is not. Piko logs collector errors and moves on. Collectors that need retry logic implement it internally. The GA4 adapter is the reference pattern.
