...
A request arrives. The router finds the page file. The page's `Render` function runs, reads data, and returns a typed `(Response, piko.Metadata, error)` tuple. The template engine substitutes the response into the `.pk` template and produces HTML, attaching the metadata as `<head>` tags and using the error to set the status code. The HTTP handler writes the HTML back.
