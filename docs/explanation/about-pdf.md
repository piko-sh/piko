...
The trade-off is that not every PDF feature maps onto CSS. Precise coordinate drawing, multi-column interleaving, and some advanced typographic controls need special handling. The framework supports these through SVG embedding (`SVGWriterPort`) and a low-level painter config, but they are escape hatches, not the default path.
...
Redaction permanently removes content from a document. The renderer burns out the original text and replaces it with a fill colour (typically black). This differs from visual overlays. Overlays hide the content in a reader but leave it in the PDF stream. A reader that ignores overlays, or a script that extracts text from the stream, sees the original. Redaction burns the content at the byte level.

The use case covers documents that need to reach wider audiences with specific fields removed. Legal filings, freedom-of-information responses, internal memos going to wider audiences. The caller specifies redacted regions in coordinate space because the caller knows where the sensitive text sits in the rendered output.
