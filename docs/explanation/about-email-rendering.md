...
Any product that wants to help with email has to accept all four constraints. The question is whether to build the tools inside the toolkit or pull them in.
...
| VML fallbacks | Auto-generated for `pml-button` pills, `pml-row` background images, and `pml-hero` backgrounds. | Covers buttons; backgrounds are less thorough. |
| Responsive breakpoint | Defaults to 480 px; configurable per project through the PML config (`Breakpoint` field in YAML or Go). | Defaults to 480 px; configurable through the `<mj-breakpoint>` head tag. |
...
Piko trades component breadth for Piko integration. MJML trades Piko integration for being a mature, specialised tool for email-only projects.

## How the premailer differs

Piko's premailer runs in ten stages (see the [premailer reference](../reference/premailer.md)). The stages are collection, variable resolution, parsing and cascade, shorthand expansion, colour normalisation, inlining, link-parameter injection, leftover placement, pseudo-element resolution, and cleanup. Each stage lives in Piko-internal code that reuses Piko's existing CSS tooling.
...
The premailer reuses the Piko generator's CSS parser, so projects that generate templates and bundle CSS in the same build pay no duplicated parse cost. Inlining is O(rules x matched-nodes) per template. For typical transactional email (fewer than 200 nodes and fewer than 100 rules) the stage runs in under a millisecond.
