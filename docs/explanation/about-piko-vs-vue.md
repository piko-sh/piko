...
**PKC components compile to imperative JavaScript wrapped in a web-component shell, not a component-framework runtime.** A PKC file produces a custom element, and the browser treats the custom element as a Web Component. The component's render and update logic, however, lowers to direct JavaScript DOM operations, not to a web-components-native declarative shape. Our reasoning runs like this. Benchmarks show that raw JavaScript DOM work outperforms web components' native rendering for Piko's workloads. The resulting bundle is also smaller because the runtime footprint stays small. The web-component wrapper still gives Piko lifecycle hooks, shadow DOM, and form association. The performance comes from what runs inside that wrapper.
