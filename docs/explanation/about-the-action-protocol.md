...
A server action is a Go struct that embeds `piko.ActionMetadata` and implements a `Call` method. The generator discovers the struct, mounts it at a conventional URL (`/actions/<package>.<name>`), and wires the dispatch code that binds an HTTP request to the `Call` method's parameters.

From the browser:

```typescript
const result = await piko.actions.customer.Upsert.call({ name: 'Alice', email: 'a@example.com' });
```

The browser posts form-encoded data to `/actions/customer.Upsert`. The dispatch layer parses the body, validates against the Go parameter types, runs `Call`, and returns the response as JSON.
...
Second, **dispatch is mechanical, not handcrafted**. The framework handles parameter binding, CSRF validation, rate limiting, caching, and response shaping. Application code writes only the `Call` body. Contrast that with hand-writing `r.FormValue("name")`, `r.FormValue("email")`, a validator, a response writer, and a JSON encoder. Every action would be ten lines of boilerplate, repeated everywhere, each copy subtly different.

Third, **the URL convention is discoverable**. `/actions/customer.Upsert` reads plainly. The `customer` package has an `Upsert` action. No route registration file. No list of URLs to maintain. The file system (`actions/customer/upsert.go`) determines the URL.
...
- `piko.ValidationField(field, message)` gives 400 with the field error attached; the client renders it inline next to the matching input.
- `piko.NewValidationError(fields)` gives 400 with multiple field errors at once.
...
- `Cacheable` with `CachePolicy()` attaches a TTL. Repeated calls with the same arguments return cached responses.
- `RateLimitable` with `RateLimit()` attaches a token bucket. Calls over the budget return HTTP 429.

These are declarations, not code. The framework implements the behaviour. Application code says "this action caches for 60 seconds", and the cache layer handles the rest.
...
Actions do not support arbitrary HTTP verbs. They are POST by convention. They do not support path parameters. Parameters always travel in the form or JSON body. They do not expose the underlying `http.Request`, and application code receives only the decoded arguments. All three are deliberate limits. Actions are narrower than `http.Handler`, and the narrowness is the point.
