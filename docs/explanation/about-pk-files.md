...
The multi-language shape confuses tooling. Editors need dedicated support to colour tokens and navigate a PK file correctly. The Piko LSP and the bundled extensions handle this, but a plain text editor shows a PK file as a single big HTML blob. Treat tooling support as a hard requirement, not a nice-to-have.

Tests cannot import one section without the others. When a Go test wants to test the `Render` function's business logic, it imports the whole file. Piko's test harness handles this. The [testing how-to](../how-to/testing.md) shows the pattern. But a function-level unit test is less ergonomic than it would be if the Go lived in a plain `.go` file.
...
- [About the hexagonal architecture](about-the-hexagonal-architecture.md) for the surrounding framework design.
