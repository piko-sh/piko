...
Piko wraps [Watermill](https://watermill.io) for typed in-process or distributed event messaging. Backends include in-memory GoChannel, NATS, and SQL-backed buses. The facade exposes shared router, publisher, and subscriber instances that Piko builds once and shares across the application. For task recipes see the [background tasks how-to](../how-to/background-tasks.md). Source file: [`wdk/events/facade.go`](https://github.com/piko-sh/piko/blob/master/wdk/events/facade.go).
...
| `events_provider_postgres` | PostgreSQL-backed bus. |
| `events_provider_sqlite` | SQLite-backed bus. |
