...
```go
func CreateNamespace[K, V](ctx context.Context, service Service, providerName, namespace string, options Options[K, V]) (Cache[K, V], error)
func NewCache[K, V](service Service, options Options[K, V]) (Cache[K, V], error)
func NewCacheFromDefault[K, V](options Options[K, V]) (Cache[K, V], error)
func NewCacheBuilder[K, V](service Service) (*Builder[K, V], error)
func NewCacheBuilderFromDefault[K, V]() (*Builder[K, V], error)
```

`CreateNamespace` is the recommended entry point. It maps one namespace per value type, which isolates entries and keeps key spaces from colliding. `NewCacheBuilderFromDefault` is the fluent shortcut when the framework is already bootstrapped.
...
| Time | `Expiration(time.Duration)`, `ExpiryCalculator(...)`, `RefreshCalculator(...)` |
...
| `BulkSet(ctx, items, tags...) error` | Batch store; on Redis/Valkey this uses pipeline/MSET. |
...
| Method | Purpose |
|---|---|
| `Refresh(ctx, key, loader) <-chan LoadResult[V]` | Asynchronously reload one key; old value served until the refresh completes. |
...
`Cache[K, V]` also exposes search when the cache is configured with a `SearchSchema`:
...
Providers that support multi-operation rollback (DAL-level transactions) implement the `Transactional[K, V]` interface in addition to `Cache[K, V]`:
...
`TransactionCache[K, V]` embeds the cache plus `Commit(ctx) error` and `Rollback(ctx) error`. Providers that don't implement `Transactional` still participate in transactions via the framework's generic journal-based fallback.
