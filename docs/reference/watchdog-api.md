...
When the application calls `WithDiagnosticDirectory` on the container, profile files land at `<dir>/profiles/` regardless of `WithWatchdogProfileDirectory`.
...
The CLI's `piko watchdog events --type <type>` flag filters by `WatchdogEventType`, with values such as `heap_threshold_exceeded`, `rss_threshold_exceeded`, `goroutine_threshold_exceeded`, `fd_pressure`, `scheduler_latency`, and continuous and contention variants.
