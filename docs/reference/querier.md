...
The project's scaffolded generator (`cmd/generator/main.go`) discovers `migrations/` and `queries/`, reads every `.sql` file, and writes type-safe Go under `generated/`. Re-run after any change:
...
### `NewSeedService(executor SeedExecutorPort, fileReader FileReader, directory string) SeedService`
