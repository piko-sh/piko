...
| `components.M3E()` | Material 3 Expressive library (46 components) | See [M3E components reference](m3e-components.md) |
...
The component declares no other props. The `+` and `-` buttons mutate `state.count` directly. Source: [`components/piko/piko-counter.pkc`](https://github.com/piko-sh/piko/blob/master/components/piko/piko-counter.pkc).
...
See [Scenario 006: data table](../../examples/scenarios/006_data_table/) for a runnable walkthrough. Source files live under [`components/m2/`](https://github.com/piko-sh/piko/tree/master/components/m2).
