...
Type alias for `safeerror.Error`. Any error in a wrap chain can implement the interface. The framework discovers it via `errors.As`. Sentinels and `errors.Is` checks propagate because `Error` implements `Unwrap()`.

Interface:

| Method | Returns |
|---|---|
| `Error() string` | Full internal string (logged server-side). |
...
|---|---|---|
| `piko.ValidationField(field, message)` | Attach a validation message to a specific form field. | 400 |
| `piko.NewValidationError(fields map[string]string)` | Multi-field validation error. | 400 |

These integrate with action forms. The dispatch layer maps each field error to the matching input.

## HTTP status mapping

Action `Call` and page `Render` return an `error` that the framework maps to an HTTP status:

| Error shape | HTTP status |
|---|---|
| `nil` | 200 |
| `piko.ValidationField`, `piko.NewValidationError` | 400 |
...
Piko populates `InternalMessage` only in development mode.
