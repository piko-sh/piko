...
const (
	...
	testPrimaryURI       = protocol.DocumentURI("file:///site/pikopls/primary/source.pk.go")
	testSatelliteU       = protocol.DocumentURI("file:///site/pikopls/satellite/source.pk.go")
...
func TestIsDroppableSynthetic(t *testing.T) {
	...
	assert.True(t, request.isDroppableSynthetic(testSatelliteU), "a foreign pikopls overlay is dropped")
	assert.True(t, request.isDroppableSynthetic("file:///site/pikopls/other/source.pk.go"), "an unknown pikopls overlay is dropped")
}
