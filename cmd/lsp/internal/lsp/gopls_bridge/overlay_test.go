...
func TestSyncOverlay_EnforcesPerChildLimit(t *testing.T) {
	...
	require.NoError(t, child.SyncOverlay(ctx, testOwner, "file:///m/piko-lsp/a/source.pk.go", []byte("a")))
	require.NoError(t, child.SyncOverlay(ctx, testOwner, "file:///m/piko-lsp/b/source.pk.go", []byte("b")))

	err := child.SyncOverlay(ctx, testOwner, "file:///m/piko-lsp/c/source.pk.go", []byte("c"))
	require.ErrorIs(t, err, ErrOverlayLimit)
	assert.False(t, child.hasOverlay("file:///m/piko-lsp/c/source.pk.go"))

	require.NoError(t, child.SyncOverlay(ctx, testOwner, "file:///m/piko-lsp/a/source.pk.go", []byte("a edited")))
}
...
func TestSyncOverlayLifecycle(t *testing.T) {
	...
	child := newTestChild(server)
	ctx := context.Background()
	uri := protocol.DocumentURI("file:///m/piko-lsp/abc/source.pk.go")
