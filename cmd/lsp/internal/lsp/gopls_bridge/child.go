...
// Child is one gopls process serving a single Go module root. piko-lsp drives it as an
...
// buildGoplsInitializeParams constructs the initialise request piko-lsp sends to its
...
// buildGoplsClientCapabilities advertises piko-lsp's capabilities to gopls so it returns
