...
// piko-lsp can offer full Go intelligence inside the embedded <script
// type="application/x-go"> blocks of .pk files.
//
// piko-lsp stays the single language server an editor talks to. For requests whose cursor
// lands in a Go block, piko-lsp acts as gopls's LSP client: it presents each block to
