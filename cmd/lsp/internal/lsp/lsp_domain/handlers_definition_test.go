...
func TestDefinition_AnalysisFails_ReturnsNil(t *testing.T) {
...
func TestDefinition_ValidDocument_ReturnsLocations(t *testing.T) {
	uri := protocol.DocumentURI("file:///test.pk")
	content := []byte(`<template><div>{{ name }}</div></template>`)
