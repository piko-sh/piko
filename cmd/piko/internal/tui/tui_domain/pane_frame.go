...
// PaneFrameOpts configures the appearance of a pane frame rendered by RenderPaneFrame.
// Values left at their zero defaults render with sensible fallbacks; callers must always
// supply a Theme.
type PaneFrameOpts struct {
	...
	// Indicator is an optional small marker shown alongside the title (e.g. an icon for
	// focus or status).
	Indicator string
...
// RenderPaneFrame composes a pane's title bar, border, and body into the final string.
// Panels that defer chrome to the layout call this from their View; panels that own their
// own chrome (the migration path during the theme rollout) continue to use
// BasePanel.RenderFrame.
//
// Takes opts (PaneFrameOpts) which configures the frame.
//
// Returns string which is the framed pane sized to opts.Width by opts.Height.
...
// paneFrameStyle picks the focused or inactive border style from the theme, falling back
// to the legacy globals when no theme is supplied so the frame remains visible in tests
// that do not configure a theme.
//
// Takes opts (PaneFrameOpts) which carries the theme and focus flag.
//
// Returns lipgloss.Style which is the border style ready to render the pane.
...
// paneFrameTitle composes the title-and-indicator row drawn inside the frame. Returns the
