...
// WatchdogEventType identifies the category of an emitted watchdog event. Mirrors the
// wire-level constants in the monitoring domain so panels can switch on them without
// depending on internal packages.
type WatchdogEventType string

const (
	// WatchdogEventHeapThresholdExceeded fires when heap allocation surpasses the configured
	// threshold.
	WatchdogEventHeapThresholdExceeded WatchdogEventType = "heap_threshold_exceeded"

	// WatchdogEventGoroutineThresholdExceeded fires when goroutine count exceeds the
	// configured threshold.
	WatchdogEventGoroutineThresholdExceeded WatchdogEventType = "goroutine_threshold_exceeded"

	// WatchdogEventGoroutineSafetyCeiling fires when goroutine count is at or above the
	// safety ceiling and capture is suppressed to avoid destabilising the runtime.
	WatchdogEventGoroutineSafetyCeiling WatchdogEventType = "goroutine_safety_ceiling"

	// WatchdogEventGCPressureWarning fires when the GC CPU fraction exceeds the configured
	// threshold.
	WatchdogEventGCPressureWarning WatchdogEventType = "gc_pressure_warning"

	// WatchdogEventCaptureError fires when an asynchronous capture failed and produced no
	// profile.
	WatchdogEventCaptureError WatchdogEventType = "capture_error"

	// WatchdogEventGomemlimitNotConfigured warns once that GOMEMLIMIT is unset and the heap
	// threshold is using a fallback.
	WatchdogEventGomemlimitNotConfigured WatchdogEventType = "gomemlimit_not_configured"

	// WatchdogEventRSSThresholdExceeded fires when resident-set size exceeds the configured
	// percentage of the cgroup limit.
	WatchdogEventRSSThresholdExceeded WatchdogEventType = "rss_threshold_exceeded"

	// WatchdogEventHeapTrendWarning fires when linear regression on the heap projects a
	// threshold breach within the warning horizon.
	WatchdogEventHeapTrendWarning WatchdogEventType = "heap_trend_warning"

	// WatchdogEventGoroutineLeakDetected fires when the runtime's goroutine-leak detector
	// reports an unbounded growth signature.
	WatchdogEventGoroutineLeakDetected WatchdogEventType = "goroutine_leak_detected"

	// WatchdogEventPreDeathSnapshot is emitted just before the watchdog captures a final
	// pre-death set of profiles on terminating signals.
	WatchdogEventPreDeathSnapshot WatchdogEventType = "pre_death_snapshot"

	// WatchdogEventLoopPanicked fires when the watchdog evaluation loop recovered from a
	// panic.
	WatchdogEventLoopPanicked WatchdogEventType = "loop_panicked"

	// WatchdogEventFDPressureExceeded fires when open file descriptors exceed the configured
	// percentage of the soft limit.
	WatchdogEventFDPressureExceeded WatchdogEventType = "fd_pressure_exceeded"

	// WatchdogEventSchedulerLatencyHigh fires when p99 scheduler latency exceeds the
	// configured threshold.
	WatchdogEventSchedulerLatencyHigh WatchdogEventType = "scheduler_latency_high"

	// WatchdogEventCrashLoopDetected fires on start-up when the recent startup history
	// contains enough unclean exits to constitute a crash loop.
	WatchdogEventCrashLoopDetected WatchdogEventType = "crash_loop_detected"

	// WatchdogEventPreviousCrashClassified records a classification of the most recent prior
	// unclean exit (panic, OOM, signal).
	WatchdogEventPreviousCrashClassified WatchdogEventType = "previous_crash_classified"

	// WatchdogEventRoutineProfileCaptured fires when continuous profiling captures a routine
	// profile on its periodic schedule.
	WatchdogEventRoutineProfileCaptured WatchdogEventType = "routine_profile_captured"

	// WatchdogEventContentionDiagnostic fires while a contention diagnostic is running and
...
// WatchdogEventCategory groups events into broad subject areas. Panels use this to colour
// and filter events without enumerating every event type.
type WatchdogEventCategory int

const (
	// WatchdogEventCategoryOther groups events that do not fit any other category and acts
	...
	// WatchdogEventCategoryGoroutine groups goroutine-count and goroutine-leak events.
	WatchdogEventCategoryGoroutine

	// WatchdogEventCategoryGC groups garbage-collector pressure events.
	WatchdogEventCategoryGC

	// WatchdogEventCategoryProcess groups crash-loop, pre-death, and process-lifetime
	// events.
	WatchdogEventCategoryProcess

	// WatchdogEventCategoryDiagnostic groups continuous profiling, capture errors, scheduler
	// latency, and FD-pressure events.
	WatchdogEventCategoryDiagnostic
)

// UtilisationGauge captures a "X of Y" measurement plus a derived percentage. Used in the
// Overview panel for budget and threshold meters.
...
// Severity returns the severity band corresponding to the gauge's utilisation. The
// thresholds are 60% (warning), 80% (critical), and 100% (saturated).
//
...
// WatchdogStatus is the TUI-friendly view of the watchdog's current state. Sub-categories
// are flattened into named gauges and durations so panels can render specific aspects
...
type WatchdogStatus struct {
	...
	// ContentionDiagnosticLastRun is the wall-clock instant the last contention diagnostic
	// completed.
	...
	// ContinuousProfilingTypes lists the profile types captured on the continuous schedule.
	...
	// FDPressureThresholdPercent is the fraction of the FD soft limit that triggers an FD
	// pressure event.
	...
	// avoid destabilising the runtime.
	GoroutineSafetyCeiling int

	// WarmUpRemaining is how much of the post-startup warm-up window is still pending.
	...
	// ContinuousProfilingRetention is the count of continuous profiles retained per type.
	ContinuousProfilingRetention int

	// ContentionDiagnosticWindow is the duration of a single contention diagnostic capture.
	ContentionDiagnosticWindow time.Duration

	// ContentionDiagnosticCooldown is the minimum gap between contention diagnostic runs.
	...
	// CrashLoopThreshold is the unclean-exit count that constitutes a crash loop within
	// CrashLoopWindow.
	...
	// ContentionDiagnosticAutoFire is true when contention diagnostics fire automatically on
	// threshold breaches.
	ContentionDiagnosticAutoFire bool
}

// WatchdogProfile describes a stored profile artefact on the server side of the watchdog.
...
// AgeFromNow returns the time since the profile was captured according to the supplied
// clock.
//
...
// DisplaySize returns a human-friendly representation of the profile's compressed size.
//
...
// WatchdogStartupEntry describes a single process start/stop record from the
// startup-history ring on the server.
type WatchdogStartupEntry struct {
	// StartedAt is the wall-clock instant the watchdog began monitoring.
	StartedAt time.Time

	// StoppedAt is the wall-clock instant of clean shutdown. Zero when the process did not
	// exit cleanly or is still running.
	StoppedAt time.Time
	...
	// Reason is the free-form reason recorded at stop time ("clean", "unclean", "panic").
	// Empty when running.
	Reason string
...
// IsRunning reports whether the entry describes the live process (no stopped timestamp
// recorded).
//
...
// IsUnclean reports whether the entry describes an exit that was not flagged clean.
//
...
// Duration returns how long the process ran. For still-running processes this measures
// the elapsed time at the supplied clock.
//
// Takes c (clock.Clock) which yields the current time.
//
...
// WatchdogEvent is the TUI-friendly view of a single watchdog event with its emission
// timestamp.
...
// Category returns the broad subject area of the event for filtering and colouring.
//
...
type WatchdogEventQuery struct {
	// Since filters out events emitted before this instant; zero disables the filter.
	Since time.Time
