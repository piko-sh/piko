...
// PanelSnapshot captures the per-panel state that should survive panel switches and
// refreshes. Panels opt into snapshotting by implementing the Snapshotter interface; the
// model saves/restores snapshots through the configured PanelStateStore.
type PanelSnapshot struct {
	// LastRefresh is when the panel last received a data update. Used by the store for
	// diagnostics; not used for cursor/scroll restoration.
	LastRefresh time.Time

	// ExpandedKeys is the set of currently-expanded item identifiers in a tree-style panel.
	// Stable across refreshes when the same identifiers reappear.
	ExpandedKeys map[string]bool

	// SearchQuery is the currently-applied filter or search query for the panel.
	SearchQuery string

	// DataIdentity is a fingerprint of the items the panel is showing. Restoring a snapshot
	// whose DataIdentity matches the panel's current items preserves the cursor; differing
	// identities cause the cursor to be clamped to the new range to avoid pointing at a
	// removed item.
	DataIdentity uint64
...
// Snapshotter is implemented by panels that participate in persistent per-panel state.
// The model calls Snapshot when focus leaves a panel and Restore when focus returns;
...
// PanelStateStore persists per-panel snapshots keyed by panel ID. The store is owned by
...
// InMemoryPanelStateStore is the default PanelStateStore used by the model. Snapshots
// live for the lifetime of the service and are discarded when the process exits.
...
// NewInMemoryPanelStateStore creates a thread-safe in-memory store suitable for the
// duration of a TUI session.
//
// Returns *InMemoryPanelStateStore which is empty and ready to receive snapshots.
...
// HashIdentifiers returns a stable FNV-1a 64 hash of the supplied identifiers. Panels use
// this to compute a DataIdentity for their current item set; matching identities indicate
// the cursor can be restored verbatim.
//
// Takes ids ([]string) which are the stable identifiers of the displayed items.
//
