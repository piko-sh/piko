...
// nextVisiblePanelMessage signals that focus should move to the next panel currently
// visible in the layout, skipping panels that are hidden.
type nextVisiblePanelMessage struct{}

// previousVisiblePanelMessage signals that focus should move to the previous panel
// currently visible in the layout.
...
// filterApplyMessage carries a filter query submitted via the command bar's filter mode.
// Panels that consume filtered queries observe this message and apply the query to their
// items list.
...
// searchApplyMessage carries a search query submitted via the command bar's search mode.
// Panels that support substring search observe this message and apply the query to their
// content.
...
// pushOverlayMessage asks the model to push an Overlay onto the overlay-manager stack.
// Panels emit this when they need to show a modal, for example the profiling
...
// popOverlayMessage asks the model to pop the topmost overlay if its ID matches.
// ID-matching avoids accidentally dismissing an overlay the user pushed in the meantime.
type popOverlayMessage struct {
	// ID is the overlay identifier that must match the top of the stack for the pop to take
	// effect.
	ID string
}

// WatchdogEventReceivedMsg is emitted by the EventDispatcher when an event has been
// delivered to one or more subscribers. Panels subscribing to the dispatcher can also use
// this as a wake signal even though they receive events through their own subscription
// channel.
...
// WatchdogStreamStateMsg announces a change in the dispatcher's connection state to the
// underlying provider stream. Panels render the change in their live indicators.
