...
const (
	// ProfileTopDefault is the default number of top entries kept by ParseProfileSummary
	// when topN is non-positive. The TUI capture detail pane shows roughly this many rows.
	ProfileTopDefault = 30

	// profileMaxSamples bounds the number of samples AggregateProfile will accumulate.
	//
	// A hostile or runaway server returning a profile with millions of samples would
	// otherwise allocate O(samples) work and O(unique-frames) map entries per call. The cap
	// is chosen to be high enough to never trigger for realistic Go pprof captures while
	// still producing a clean error path on degenerate inputs.
	profileMaxSamples = 1_000_000
)

var (
	// ErrProfileTooManySamples is returned by AggregateProfile when the profile contains
	// more than profileMaxSamples samples. Callers can detect it with errors.Is.
	ErrProfileTooManySamples = errors.New("profile sample count exceeded budget")
)

// profileKey identifies a unique location in the aggregation maps.
//
// FileName and Line are populated only when ProfileAggOpts.ByLine is true; otherwise the
// key is the function name alone.
type profileKey struct {
	// FunctionName is the fully-qualified function name at this location.
	FunctionName string

	// FileName is the source file path; populated only when grouping by line.
	FileName string

	// Line is the source line number; populated only when grouping by line.
	Line int64
}

// label returns the display label for a key. Includes "file:line" when the key was built
// with ByLine grouping.
//
...
//
// Flat is the value attributable to this frame's own work; Cum is the value across the
// call subtree rooted at this frame.
type ProfileEntry struct {
	// Label is the display label - either a function name or "function file:line" depending
	// on ByLine.
	Label string
...
//
// SampleType / SampleUnit name the column the values came from (e.g. "cpu" /
// "nanoseconds"). Total is the sum of Flat values across every entry in the unfiltered,
// unsliced aggregation, used to compute percentages.
type ProfileSummary struct {
	// SampleType describes what each numeric value represents.
	SampleType string

	// SampleUnit is the SampleType's unit; rendered alongside values for context.
	SampleUnit string

	// Entries is the ordered top-N slice; first element is the highest-Flat location in the
	// profile.
...
// ProfileAggOpts controls how raw pprof data is aggregated into a ProfileSummary.
type ProfileAggOpts struct {
	// FocusRegex, when non-nil, includes only samples whose call stack contains a function
	// matching the pattern.
	FocusRegex *regexp.Regexp

	// SampleIndex selects the column inside the profile's SampleType slice. CPU profiles
	// only have one column; heap profiles distinguish allocs vs in-use.
	SampleIndex int

	// TopN caps the returned Entries slice. Values <= 0 fall back to ProfileTopDefault.
	TopN int

	// ByLine, when true, groups by source file + line; otherwise groups by function name.
	ByLine bool
}

// ParseProfileSummary parses raw pprof bytes and aggregates the top-N locations for the
// supplied sample-type column. The TUI uses this to render a "top frames" view; the CLI
// uses it to print a profile report.
//
// Takes data ([]byte) which is the raw pprof bytes.
// Takes opts (ProfileAggOpts) which controls aggregation.
//
// Returns *ProfileSummary which holds the top-N entries sorted by flat descending and
// total across the unsliced aggregation.
...
// AggregateProfile walks all samples and aggregates flat/cum values using the supplied
// options. Caller-supplied prof retains ownership; AggregateProfile only reads from it.
//
// Takes prof (*profile.Profile) which is the parsed pprof data.
// Takes opts (ProfileAggOpts) which controls aggregation.
//
// Returns *ProfileSummary which holds the top-N entries sorted by flat descending.
...
// sampleMatchesFocus reports whether at least one location in the sample matches
// focusRegex, or true when focusRegex is nil.
//
...
// accumulateSample adds a single sample's contribution to the flat and cumulative
// aggregation maps, keyed by profileKey.
//
