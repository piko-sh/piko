...
// RequestData encapsulates all information about an incoming HTTP request. It is made
// available within a component's Render function and includes the request context, URL,
// headers, and parsed form/query data.
type RequestData = templater_dto.RequestData

// Metadata allows a component's Render function to return SEO information, caching
// policies, and other page-level metadata that influences the final HTTP response.
type Metadata = templater_dto.Metadata

// OGTag represents an Open Graph protocol <meta> tag for social media sharing (e.g.,
// <meta property="og:title" content="...">).
type OGTag = templater_dto.OGTag

// MetaTag represents an HTML <meta> tag used for page metadata (e.g., <meta name="author"
// content="...">).
type MetaTag = templater_dto.MetaTag

// NoProps is a placeholder type for components that do not accept any properties. It is
// used in the Render function signature when a component is self-contained.
type NoProps = templater_dto.NoProps

// NoResponse is a placeholder type for Render functions that do not return any data for
// the template. The template can still access global state such as the request object.
...
// AuthContext represents the resolved authentication state for a request. Users implement
// AuthContext to expose their auth system's data to Piko pages and actions.
type AuthContext = daemon_dto.AuthContext

// AuthProvider resolves authentication state from an HTTP request. Piko calls
// Authenticate on every request when a provider is registered.
type AuthProvider = daemon_dto.AuthProvider

// AuthPolicy declares authentication requirements for a page. Returned by the optional
// AuthPolicy() function in PK files.
type AuthPolicy = daemon_dto.AuthPolicy

// PreviewScenario describes a single named preview scenario for a component. The
// Preview() convention function returns a slice of these, providing sample data for
// rendering the component in the dev tools preview panel.
...
// ActionMetadata is embedded in action structs to provide access to request context,
...
// limits for protection against resource exhaustion.
...
// SpamProtected is an interface for requiring spam detection before action execution.
// Actions implement SpamSchema() to declare which fields should be checked and with which
// signals.
...
// per-action spam detection settings.
...
var (
	// RateLimitByIP is a rate limiting strategy that limits requests by IP address.
	...
	// RateLimitBySession is the rate limiting strategy that tracks requests per session.
	RateLimitBySession = daemon_domain.RateLimitBySession

	// NewFileUpload creates a FileUpload from a multipart.FileHeader. This is called by
	// generated wrapper code; users typically don't need this.
	NewFileUpload = daemon_dto.NewFileUpload

	// NewRawBody creates a RawBody from raw data. This is called by the action handler;
	// users typically don't need this.
	...
	// ValidationField creates a single-field validation error. This is a helper for the
	// common case of one field failing validation.
	//
	// Example: return nil, piko.ValidationField("email", "invalid email format")
	ValidationField = daemon_dto.ValidationField

	// NotFound creates a not found error for a resource.
	//
	// Example: return nil, piko.NotFound("user", userID)
	NotFound = daemon_dto.NotFound

	// NotFoundResource creates a not found error without a specific ID. Use this when the
	// resource type is known but there's no specific identifier.
	//
	// Example: return nil, piko.NotFoundResource("configuration")
	NotFoundResource = daemon_dto.NotFoundResource

	// Conflict creates a conflict error with the default CONFLICT code.
	//
	// Example: return nil, piko.Conflict("email already registered")
	Conflict = daemon_dto.Conflict

	// ConflictWithCode creates a conflict error with a custom error code. Use this when the
	// client needs to discriminate between different conflict types.
	//
	// Example: return nil, piko.ConflictWithCode("email already registered", "EMAIL_EXISTS")
	ConflictWithCode = daemon_dto.ConflictWithCode

	// Forbidden creates a forbidden error.
	//
	// Example: return nil, piko.Forbidden("you do not have permission to delete this
	// resource")
	Forbidden = daemon_dto.Forbidden

	// Unauthorised creates an unauthorised error.
	//
	// Example: return nil, piko.Unauthorised("session expired")
	Unauthorised = daemon_dto.Unauthorised

	// BadRequest creates a bad request error.
	//
	// Example: return nil, piko.BadRequest("missing required header: X-Request-ID")
	BadRequest = daemon_dto.BadRequest

	// PageError creates a generic page error with an arbitrary HTTP status code. Use this
	// when none of the specific error helpers match.
	//
	// Example: return Response{}, piko.Metadata{}, piko.PageError(429, "too many requests")
	...
	// SessionCookieInsecure is a session cookie option that allows HTTP connections. Use
	// this only for local development where HTTPS is not available.
	//
	...
	// ClearCookieInsecure creates a cookie deletion instruction that works over HTTP. Use
	// this only for local development where HTTPS is not available.
	...
	// based on the runtime environment. In production Secure is true; in development Secure
	// is false.
	...
	// Cookie is a helper that creates a cookie with custom settings. Use this when you need
	// more control than SessionCookie gives.
	//
	// Example:
	//
	// 	return piko.ActionResponse{
	...
	// WithInsecure allows the cookie to be sent over HTTP. WARNING: Only use for local
	// development.
	WithInsecure = daemon_dto.WithInsecure

	// WithJavaScriptAccess allows JavaScript to access the cookie (removes HttpOnly).
	...
	// CSRF protection but may stop some normal website features from working correctly.
	...
	// but requiring Secure to be true.
	WithSameSiteNone = daemon_dto.WithSameSiteNone
)

// ActionHandlerEntry describes a registered action for the runtime. Generated code
// creates entries for each action and registers them at startup.
type ActionHandlerEntry = daemon_adapters.ActionHandlerEntry

// FileUpload represents an uploaded file from a multipart form request. Use FileUpload in
// action Call parameters to receive file uploads.
//
// Example:
//
...
// RawBody provides access to the unparsed request body. Use this when you need to verify
// signatures or parse custom formats.
//
// Example:
//
...
// implementing ActionError are automatically discriminated by the action handler to
...
// none of the specific error types match the status code you need.
...
// RegisterActions registers multiple actions with the global action registry. This is
// called by generated code in init() functions to auto-register actions.
//
// Takes entries (map[string]ActionHandlerEntry) which maps action names (e.g.,
// "email.contact") to their handler entries.
